...
class CustomColorPanel(tk.Frame):
    ...
    def _reset_colors(self):
        ...
        if not exe or not os.path.isfile(exe):
            ...
            return
        reset_bytes = bytes.fromhex("F0CFD9E5FF0EDDE8FF03A1FFFCA74CFFFF4339FFF5DE0CFDB4F2ACFF73DF52FF123F4DFFE80770FFD217FCFF3030D0")
        try:
            with open(exe, "r+b") as f:
                f.seek(MDT_COLOR_OFFSET)
                f.write(reset_bytes)
...
class AVLEditorPanel(tk.Frame):
    ...
    def _save(self):
        ...
        p2 = 0
        while True:
            idx = raw.find(header, p2)
            if idx < 0: break
            all_pos.append(idx)
            p2 = idx + 16
        for ep in all_pos:
            if ep + 15 < len(raw):
                raw[ep + 15] = total_count
