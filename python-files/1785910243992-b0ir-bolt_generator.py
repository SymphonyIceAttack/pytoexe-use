# -*- coding: utf-8 -*-
"""
Fastener Assembly TXT Generator
...
# Reference: M20 base values for vertex pattern scaling
REF_SIZE = 'M20'
REF_S = 30.0          # hex flat-to-flat for M20
REF_E = 34.7          # hex circumscribed diameter for M20 (from reference file)
...
# ============================================================
# Reference Vertex Patterns (M20 base)
...
# Shaft NREVOLUTION (5 vertices, all have POS)
SHAFT_NREVOLUTION_VERTS = [
    (0, -10.01, 0),
    (0, -18, 0),
    (-130, -18, 0),
    (-130, -10.01, 0),
...
def calc_z_positions(bolt_size, bolt_length, t_thickness):
    ...
    Reference: bolt +Z end is at Z = REF_BOLT_TIP_Z (25mm).

    Stacking from -Z to +Z:
      bolt head -> flat washer 1 -> [T connection] -> flat washer 2
      -> spring washer -> nut

    Returns dict with all Z positions (global, center-based for cylinders).
    """
    ...
    # Flat washer 2: nut side, T distance from FW1 (center to center)
    fw2_center_z = fw1_center_z + t_thickness
...
def generate_fastener_txt(bolt_size, bolt_length, t_thickness):
    ...
    # Scaling factors (relative to M20 reference)
    hex_scale_bolt = s_bolt / REF_S
    hex_scale_nut = s_nut / REF_S
    length_scale = L / REF_LENGTH
    ...
    shaft_v = scale_vertices(SHAFT_NREVOLUTION_VERTS,
                             scale_xy=1.0,
                             scale_x=length_scale,
                             scale_y=hex_scale_bolt)
    ...
    # Left NXTRUSION Y offset (tiny offset from reference: -0.01mm for M20)
    left_y_offset = -0.01 * hex_scale_bolt

    # Date string (matches reference format: "4  Aug 2026 13:27")
...
class BoltGeneratorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Fastener TXT Generator")
    ...
    def create_widgets(self):
        ...
        # Title
        title_label = ttk.Label(main_frame, text="Fastener Assembly Parameters",
                                font=("Arial", 14, "bold"))
        title_label.grid(row=0, column=0, columnspan=2, pady=(0, 20))

        # Bolt size (dropdown)
        ttk.Label(main_frame, text="Bolt Size:").grid(
        ...
        # Connection thickness (positive numbers only)
        ttk.Label(main_frame, text="Connection Thickness:").grid(
        ...
        ttk.Label(main_frame, text="Bolt Length:").grid(
        ...
        # Info frame
        self.info_frame = ttk.LabelFrame(main_frame, text="Auto-Matched Dimensions", padding="10")
        ...
        self.generate_btn = ttk.Button(btn_frame, text="Generate Fastener",
    ...
    def update_info(self):
        """Update matched dimension info"""
        bolt_size = self.bolt_size_var.get()
        if bolt_size not in BOLT_DATA:
            self.info_text.set("Select a bolt size to see dimensions")
            return
        ...
        info = f"Bolt: hex {bolt['s']}mm across flats, head {bolt['h']}mm thick\n"
        info += f"Nut: hex {nut['s']}mm across flats, {nut['h']}mm thick\n"
        info += f"Flat Washer: dia {fw['d']}mm, thick {fw['h']}mm\n"
        if sw:
            info += f"Spring Washer: dia {sw['d']}mm, thick {sw['h']}mm"
        else:
            info += f"Spring Washer: not available for {bolt_size}"
    ...
    def generate_fastener(self):
        ...
        bolt_size = self.bolt_size_var.get()
        if not bolt_size:
            messagebox.showwarning("Warning", "Please select a bolt size")
            return

        t_str = self.t_thickness_var.get().strip()
        if not t_str:
            messagebox.showwarning("Warning", "Please enter connection thickness")
            return

        try:
            t_thickness = float(t_str)
            if t_thickness <= 0:
                messagebox.showwarning("Warning", "Connection thickness must be positive")
                return
        except ValueError:
            messagebox.showwarning("Warning", "Connection thickness must be a valid number")
            return

        bolt_length_str = self.bolt_length_var.get()
        if not bolt_length_str:
            messagebox.showwarning("Warning", "Please select a bolt length")
            return
        ...
        if bolt_size not in SPRING_WASHER_DATA:
            messagebox.showerror("Error",
                f"No spring washer data for {bolt_size}.\n"
                f"Supported sizes: M6 ~ M48")
            return
        ...
        try:
            ...
            messagebox.showinfo("Success",
                f"Fastener TXT file generated successfully!\n\n"
                f"File: {file_path}\n"
                f"Bolt Size: {bolt_size}\n"
                f"Bolt Length: {bolt_length} mm\n"
                f"Connection Thickness: {t_thickness} mm")

        except Exception as e:
            messagebox.showerror("Error", f"Error generating file:\n{str(e)}")
