...
try:
    ...
    if currenttime - float(buffer.read()) >= 17990:
        ...
        buffer2.close()
        openwebpage("https://discord.com/invite/K8gdWHthVw")
        buffer.close()
except:
...
def load():
    ...
    try:
        ...
        COLORS = {
            ...
            "aimblox": (Fore.BLUE, np.array((4, 225, 206), dtype="uint8"), np.array((0, 175, 119), dtype="uint8")),
...
def print_banner(bot: trbot):
    try:
        ...
        print(Style.BRIGHT + Fore.RED + """     
                 ...
                 `7MM                 *MM                  mm    
                   MM                  MM                  MM    
 ,p6"bo   ,pW"Wq.  MM  ,pW"Wq.`7Mb,od8 MM,dMMb.   ,pW"Wq.mmMMmm  
6M'  OO  6W'   `Wb MM 6W'   `Wb MM' "' MM    `Mb 6W'   `Wb MM    
8M       8M     M8 MM 8M     M8 MM     MM     M8 8M     M8 MM    
YM.    , YA.   ,A9 MM YA.   ,A9 MM     MM.   ,M9 YA.   ,A9 MM    
 YMbmd'   `Ybmd9'.JMML.`Ybmd9'.JMML.   P^YbmdP'   `Ybmd9'  `Mbmo

""" + Style.RESET_ALL)
        
        print("--- CONTROLS ---")
        print(f"Activate colorbot     : {Fore.BLUE}{AIM_KEY}{Style.RESET_ALL}")
        if SWITCH_MODE_KEY != "disabled":
            print(f"Activation Key        : {Fore.BLUE}{SWITCH_MODE_KEY}{Style.RESET_ALL}")
        if UPDATE_KEY != "disabled":
            print(f"Update Config         : {Fore.BLUE}{UPDATE_KEY}{Style.RESET_ALL}\n")

        print("--- SETTINGS ---")
        print(f"Activation Mode       : {Fore.LIGHTCYAN_EX}{toggleholdmodes[bot.switchmode]}{Style.RESET_ALL}")
        print(f"Aim FOV               : {Fore.LIGHTYELLOW_EX}{AIM_FOV}{Style.RESET_ALL}")
        print(f"Cam FOV               : {Fore.LIGHTYELLOW_EX}{CAM_FOV}{Style.RESET_ALL}")
        ...
        if TRIGGERBOT_DELAY != 0:
            print(f"Triggerbot Delay      : {Fore.LIGHTYELLOW_EX}{TRIGGERBOT_DELAY}{Style.RESET_ALL}")
        if SMOOTHENING != "disabled":
            print(f"Smoothening           : {Fore.LIGHTCYAN_EX}On{Style.RESET_ALL}")
            print(f"Smoothening Factor    : {Fore.YELLOW}{SMOOTH_FACTOR}{Style.RESET_ALL}")
        else:
            print(f"Smoothening           : {Fore.RED}Off{Style.RESET_ALL}")
        if TRIGGERBOT_DELAY != 0:
            print(f"Triggerbot Delay      : {Fore.GREEN}{TRIGGERBOT_DELAY}{Style.RESET_ALL}")
        print(
            f"Aim Speed             : {Fore.LIGHTYELLOW_EX}X: {AIM_SPEED_X} Y: {AIM_SPEED_Y}{Style.RESET_ALL}"
        )
        print(
            f"Aim Offset            : {Fore.LIGHTYELLOW_EX}X: {AIM_OFFSET_X} Y: {AIM_OFFSET_Y}{Style.RESET_ALL}"
        )
        print(
            f"On/Off                : {(Fore.GREEN if bot.aimtoggled else Fore.LIGHTBLACK_EX)}{bot.aimtoggled}{Style.RESET_ALL}"
        )
        print(f"Aim color             : {str(colorname + COLOR)}{Style.RESET_ALL}")
