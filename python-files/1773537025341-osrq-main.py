...
def fix_ar(text):
    """
    Tkinter renders Arabic LTR so we reverse word order.
    English words/tokens inside Arabic text stay as-is but their
    position in the reversed list puts them in the right visual spot.
    Tokens that are purely ASCII get wrapped in Unicode RTL marks so
    they don't visually flip inside the Arabic string.
    """
    if not text:
        return text
    if not any("\u0600" <= c <= "\u06ff" for c in text):
        return text
    # Split on spaces, reverse order for RTL display
    words = text.split(" ")
    words.reverse()
    return " ".join(words)
