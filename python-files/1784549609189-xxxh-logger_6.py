...
def get_usb_devices_ordered():
    """Получает список USB устройств в порядке их обнаружения системой."""
    usb_list = []
...
def scan_disks_ordered():
    """Сканирует диски и возвращает список в порядке их перечисления WMI."""
    results = []
...
def save_to_log(info, usb_devices, disk_results, log_file):
    ...
    with open(log_file, "a", encoding="utf-8") as f:
        # Определяем максимальное количество записей, чтобы пройтись по всем
        max_count = max(len(usb_devices), len(disk_results))
        
        for i in range(max_count):
            # Берем данные USB, если индекс существует, иначе заглушка
            if i < len(usb_devices):
                usb_data = usb_devices[i]
                usb_part = format_usb(usb_data)
            else:
                usb_part = "usb_devices=none"
            
            # Берем данные диска, если индекс существует, иначе заглушка
            if i < len(disk_results):
                disk = disk_results[i]
                paths_str = "^".join(disk["paths"])
                paths_str = paths_str.replace(";", "^") # Экранирование
            ...
            # Формируем ОДНУ строку: Система + USB + Диск
            full_line = f"{sys_info_base};{usb_part};{disk_part}\n"
            f.write(full_line)
