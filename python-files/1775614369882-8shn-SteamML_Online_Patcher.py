...
def get_game_exe(gp,en=""):
    if en:
        ...
        if os.path.isfile(p):return en
    exes=glob.glob(os.path.join(gp,"*.exe"))
    if exes:return os.path.basename(exes[0])
    return""
class App(ctk.CTk):
    def __init__(self):
        super().__init__()
    ...
    def check_ready(self,*a):
        if os.path.isdir(self.gp.get().strip()) and self.iv.get().strip():self.pb.configure(state="normal")
        else:self.pb.configure(state="disabled")
    def patch_game(self):
        gp=self.gp.get().strip()
        en=get_game_exe(gp,"")
        pf=["unsteam64.dll","winhttp.dll","winmm.dll"]
        for f in pf:
            s=os.path.join(os.getcwd(),"PatcherFiles",f)
            if os.path.isfile(s):shutil.copy(s,gp)
        cfg=configparser.ConfigParser()
        cfg["loader"]={"exe_file":en,"dll_file":"unsteam64.dll"}
        cfg["game"]={"offline_mode":0,"steam_id":"","player_name":"By OPV","real_app_id":self.iv.get().strip(),"fake_app_id":480,"language":"english","beta_name":"beta","saves_path":"","game_launch_options":""}
        cfg["dlcs"]={}
        with open(os.path.join(gp,"unsteam.ini"),"w")as f:cfg.write(f)
