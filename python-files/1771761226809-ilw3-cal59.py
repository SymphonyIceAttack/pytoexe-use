...
def start_timer():
    ...
    if timer_running:
        return
    timer_running = True
    root.after(1000, lambda: countdown(60))  # يبدأ بعد ثانية واحدة
...
view_menu.add_command(label="Transparency", command=lambda: root.attributes("-alpha", 0.5 if root.attributes("-alpha")>0.5 else 0.9))
