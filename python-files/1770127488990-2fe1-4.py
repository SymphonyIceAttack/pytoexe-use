...
class SnowflakeCalculator:
    ...
    def create_compatible_styles(self):
        """创建兼容性样式"""
        style = ttk.Style()
        
        # 使用系统主题以获得最好的兼容性
        try:
            style.theme_use('vista')  # Windows Vista/7/8/10/11 通用主题
        except:
            try:
    ...
    def create_snowflake_tab(self):
        ...
        # 输入区域 - 第0行
        input_frame = ttk.LabelFrame(container, text="第一步：输入第一瓣雪花的角度数据", padding=10)
        input_frame.grid(row=0, column=0, sticky="ew", padx=5, pady=(0, 5))
        input_frame.columnconfigure(0, weight=1)
        
        tk.Label(input_frame, text="请输入角度数据（用逗号分隔）：", 
                font=('微软雅黑', 9)).grid(row=0, column=0, sticky="w", pady=(0, 5))
        
        self.angle_entry = tk.Text(input_frame, height=3, font=('Consolas', 9))
        self.angle_entry.grid(row=1, column=0, sticky="ew", pady=(0, 5))
        
        # 配置区域 - 第1行
        config_frame = ttk.LabelFrame(container, text="第二步：配置雪花参数", padding=10)
        config_frame.grid(row=1, column=0, sticky="ew", padx=5, pady=(0, 5))
        
        # 瓣数配置 - 第0行
        ttk.Label(config_frame, text="雪花瓣数：", 
                 font=('微软雅黑', 9)).grid(row=0, column=0, sticky="w", padx=(0, 5), pady=(0, 5))
        ...
        petal_combo.grid(row=0, column=1, sticky="w", padx=(0, 15), pady=(0, 5))
        petal_combo.bind("<<ComboboxSelected>>", self.on_petal_change)
        
        # 自定义瓣数
        ttk.Label(config_frame, text="或自定义瓣数：", 
                 font=('微软雅黑', 9)).grid(row=0, column=2, sticky="w", padx=(0, 5), pady=(0, 5))
        
        self.custom_petal_var = tk.StringVar()
        self.custom_petal_entry = ttk.Entry(config_frame, textvariable=self.custom_petal_var, width=8)
        self.custom_petal_entry.grid(row=0, column=3, sticky="w", pady=(0, 5))
        self.custom_petal_entry.configure(state='disabled')
        
        # 旋转方向 - 第1行
        ttk.Label(config_frame, text="旋转方向：", 
                 font=('微软雅黑', 9)).grid(row=1, column=0, sticky="w", padx=(0, 5))
        
        self.rotation_var = tk.StringVar(value="negative")
        ttk.Radiobutton(config_frame, text="负角度（顺时针）", 
                       variable=self.rotation_var, value="negative").grid(row=1, column=1, sticky="w", padx=(0, 15))
        ttk.Radiobutton(config_frame, text="正角度（逆时针）", 
                       variable=self.rotation_var, value="positive").grid(row=1, column=2, columnspan=2, sticky="w")
        
        # 输出区域 - 第2行
        output_frame = ttk.LabelFrame(container, text="第三步：生成结果", padding=10)
        output_frame.grid(row=2, column=0, sticky="nsew", padx=5, pady=(0, 5))
        container.rowconfigure(2, weight=1)  # 输出区域可以扩展
        
        output_frame.columnconfigure(0, weight=1)
        output_frame.rowconfigure(1, weight=1)  # 文本区域可以扩展
        
        # 按钮区域 - 第0行
        button_frame = ttk.Frame(output_frame)
        button_frame.grid(row=0, column=0, sticky="ew", pady=(0, 10))
        
        self.generate_btn = tk.Button(button_frame, text="生成雪花数据", 
                                     font=('微软雅黑', 9),
        ...
        self.copy_btn = tk.Button(action_frame, text="复制到剪贴板", 
                                 font=('微软雅黑', 9),
                                 command=self.copy_to_clipboard, 
                                 state='disabled')
        self.copy_btn.pack(side=tk.LEFT, padx=(0, 10))
        
        self.save_btn = tk.Button(action_frame, text="保存到文件", 
                                 font=('微软雅黑', 9),
                                 command=self.save_to_file, 
                                 state='disabled')
        self.save_btn.pack(side=tk.LEFT, padx=(0, 10))
        
        self.transfer_btn = tk.Button(action_frame, text="传输到角度加减器", 
                                     font=('微软雅黑', 9),
    ...
    def create_angle_calculator_tab(self):
        ...
        # 输入区域 - 第0行
        input_frame = ttk.LabelFrame(container, text="输入角度数据", padding=10)
        input_frame.grid(row=0, column=0, sticky="ew", padx=5, pady=(0, 5))
        input_frame.columnconfigure(0, weight=1)
        ...
        self.calc_angle_entry.grid(row=1, column=0, sticky="ew", pady=(0, 5))
        
        # 操作按钮
        calc_button_frame = ttk.Frame(input_frame)
        calc_button_frame.grid(row=2, column=0, sticky="w", pady=(5, 0))
        
        tk.Button(calc_button_frame, text="从雪花生成器导入", 
                 font=('微软雅黑', 9),
                 command=self.import_from_snowflake).pack(side=tk.LEFT, padx=(0, 10))
        
        tk.Button(calc_button_frame, text="清空", 
                 font=('微软雅黑', 9),
                 command=self.clear_calculator).pack(side=tk.LEFT)
        
        # 计算配置区域 - 第1行
        calc_frame = ttk.LabelFrame(container, text="角度加减配置", padding=10)
        calc_frame.grid(row=1, column=0, sticky="ew", padx=5, pady=(0, 5))
        
        # 加减角度输入
        ttk.Label(calc_frame, text="要加上的角度：", 
                 font=('微软雅黑', 9)).grid(row=0, column=0, sticky="w", padx=(0, 5), pady=(0, 5))
        
        self.add_angle_var = tk.StringVar(value="45")
        add_angle_entry = ttk.Entry(calc_frame, textvariable=self.add_angle_var, width=8)
        add_angle_entry.grid(row=0, column=1, sticky="w", padx=(0, 5), pady=(0, 5))
        
        ttk.Label(calc_frame, text="度").grid(row=0, column=2, sticky="w", pady=(0, 5))
        
        # 角度归一化选项
        self.normalize_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(calc_frame, text="将结果归一化到0-360度", 
                       variable=self.normalize_var).grid(row=0, column=3, sticky="w", padx=(10, 0), pady=(0, 5))
        
        # 计算按钮
        self.calc_btn = tk.Button(calc_frame, text="计算", 
                                 font=('微软雅黑', 9),
                                 command=self.calculate_angles)
        self.calc_btn.grid(row=1, column=0, columnspan=4, sticky="w", pady=(5, 0))
        
        # 结果显示区域 - 第2行
        result_frame = ttk.LabelFrame(container, text="计算结果", padding=10)
        result_frame.grid(row=2, column=0, sticky="nsew", padx=5, pady=(0, 5))
        container.rowconfigure(2, weight=1)  # 结果区域可以扩展
        
        result_frame.columnconfigure(0, weight=1)
        result_frame.rowconfigure(1, weight=1)  # 文本区域可以扩展
        
        # 结果统计
        self.calc_stats_label = tk.Label(result_frame, text="就绪", font=('微软雅黑', 9))
        self.calc_stats_label.grid(row=0, column=0, sticky="w", pady=(0, 5))
        ...
        self.calc_result_text.grid(row=1, column=0, sticky="nsew", pady=(0, 5))
        
        # 操作按钮 - 第2行
        calc_action_frame = ttk.Frame(result_frame)
        calc_action_frame.grid(row=2, column=0, sticky="ew", pady=(5, 0))
        
        self.calc_copy_btn = tk.Button(calc_action_frame, text="复制到剪贴板", 
                                      font=('微软雅黑', 9),
                                      command=self.copy_calc_to_clipboard, 
                                      state='disabled')
        self.calc_copy_btn.pack(side=tk.LEFT, padx=(0, 10))
        
        self.calc_save_btn = tk.Button(calc_action_frame, text="保存到文件", 
                                      font=('微软雅黑', 9),
                                      command=self.save_calc_to_file, 
                                      state='disabled')
        self.calc_save_btn.pack(side=tk.LEFT, padx=(0, 10))
        
        tk.Button(calc_action_frame, text="传输到雪花生成器", 
                 font=('微软雅黑', 9),
    ...
    def create_status_bar(self):
        ...
        self.status_bar = tk.Label(status_frame, text="就绪", 
                                  ...
                                  bg='#F0F0F0',
                                  fg='black')
        self.status_bar.pack(side=tk.LEFT, padx=(5, 0))
        ...
        tk.Label(status_frame, text="作者: iDQhonefully", 
                font=('微软雅黑', 8),
                bg='#F0F0F0',
                fg='gray').pack(side=tk.RIGHT, padx=(0, 5))
