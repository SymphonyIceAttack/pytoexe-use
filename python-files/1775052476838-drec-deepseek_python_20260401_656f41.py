...
def init_db():
    ...
    ''')
    # Добавляем новые столбцы, если их нет (для совместимости со старой БД)
    try:
        c.execute("ALTER TABLE clients ADD COLUMN rate_prev_year REAL")
    except sqlite3.OperationalError:
        pass
    try:
        c.execute("ALTER TABLE clients ADD COLUMN rate_prev2_year REAL")
    except sqlite3.OperationalError:
        pass
...
def add_client(name, doc_type, specialist='', rate=0.0, rate_prev_year=0.0, rate_prev2_year=0.0, comment=''):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("""
        INSERT INTO clients (name, doc_type, specialist, rate, rate_prev_year, rate_prev2_year, comment)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    """, (name, doc_type, specialist, rate, rate_prev_year, rate_prev2_year, comment))
    conn.commit()
    conn.close()

def update_client(client_id, name, doc_type, specialist, rate, rate_prev_year, rate_prev2_year, comment):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("""
        UPDATE clients SET name=?, doc_type=?, specialist=?, rate=?, rate_prev_year=?, rate_prev2_year=?, comment=?
        WHERE id=?
    """, (name, doc_type, specialist, rate, rate_prev_year, rate_prev2_year, comment, client_id))
    conn.commit()
    conn.close()
...
def get_all_months():
    """Возвращает список строк 'YYYY-MM' начиная с текущего месяца по декабрь 2030"""
    today = date.today()
...
def get_total_visits(client_ids, year):
    """Суммарное количество выездов за год для списка client_ids"""
    if not client_ids:
        return 0
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
...
def get_client_info(client_id):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("SELECT name, specialist FROM clients WHERE id=?", (client_id,))
    row = c.fetchone()
    conn.close()
    return row
...
def format_month(month_str):
    """Преобразует 'YYYY-MM' в 'Месяц YYYY года'"""
    year, month = map(int, month_str.split('-'))
    month_name_ru = calendar.month_name[month]
    # Для красоты можно перевести на русский, но оставим английские названия,
    # так как в calendar.month_name[1] = 'January'
    # Чтобы получить русские, нужна дополнительная библиотека, оставим английские.
    # Пользователь на скрине использовал русские названия, но это легко поправить.
    # Для простоты оставим английские.
    return f"{month_name_ru} {year} года"
...
class ClientEditDialog(tk.Toplevel):
    ...
    def save(self):
        ...
        self.result = (name, self.doc_type_var.get(), self.specialist_entry.get().strip(),
                       rate, rate_prev, rate_prev2, self.comment_text.get('1.0', 'end').strip())
        self.destroy()

class MonthEditDialog(tk.Toplevel):
    def __init__(self, parent, month_year, data):
        super().__init__(parent)
        self.title(f"Редактирование {format_month(month_year)}")
        self.data = data  # (invoice, act, docs_returned, visits)
        self.result = None
        self.geometry("300x250")
        self.resizable(False, False)

        tk.Label(self, text=f"Месяц: {format_month(month_year)}", font=('', 10, 'bold')).pack(pady=10)
        ...
        self._enable_copy_paste(self.visits_entry)

    def _enable_copy_paste(self, widget):
        def copy(e): widget.event_generate('<<Copy>>')
        def cut(e): widget.event_generate('<<Cut>>')
        def paste(e): widget.event_generate('<<Paste>>')
        widget.bind('<Control-c>', copy)
        widget.bind('<Control-x>', cut)
        widget.bind('<Control-v>', paste)
...
class MainApplication:
    ...
    def set_background(self):
        if PIL_AVAILABLE:
            size = 400
            img = Image.new('RGB', (size, size), '#20B2AA')
    ...
    def create_journal_tab(self):
        ...
        ttk.Button(begin_frame, text="Добавить", command=lambda: self.add_client('begin')).pack(side='left', padx=5, pady=2)
        ttk.Button(begin_frame, text="Удалить", command=lambda: self.delete_client('begin')).pack(side='left', padx=5, pady=2)
        ...
        ttk.Button(end_frame, text="Добавить", command=lambda: self.add_client('end')).pack(side='left', padx=5, pady=2)
        ttk.Button(end_frame, text="Удалить", command=lambda: self.delete_client('end')).pack(side='left', padx=5, pady=2)
        ...
        # Комментарий – увеличен в 2 раза по ширине
        ...
        # Настройка Treeview с увеличенным шрифтом и цветами
        style = ttk.Style()
        ...
        # Теги для подсветки текущего месяца
        self.month_tree.tag_configure('current_month', background='#90EE90')  # светло-зелёный

    def create_stats_tab(self):
        stats_frame = ttk.Frame(self.notebook)
        self.notebook.add(stats_frame, text="Статистика выездов")

        # Верхняя панель выбора клиентов
    ...
    def refresh_stats_clients(self):
        ...
        self.clients_list = clients  # сохраняем для быстрого доступа
        for c in clients:
            self.stats_listbox.insert(tk.END, f"{c[1]} (спец: {c[3] if c[3] else 'не указан'})")
    ...
    def save_client_field(self, field):
        ...
        if field == 'specialist':
            ...
            update_client(cid, self.current_client[1], self.current_client[2], new_val,
                          self.current_client[4], self.current_client[5], self.current_client[6], self.current_client[7])
        elif field == 'rate':
            try:
            ...
            update_client(cid, self.current_client[1], self.current_client[2], self.current_client[3],
                          new_val, self.current_client[5], self.current_client[6], self.current_client[7])
            ...
            try:
                ...
                new_val = 0.0
            update_client(cid, self.current_client[1], self.current_client[2], self.current_client[3],
                          self.current_client[4], new_val, self.current_client[6], self.current_client[7])
            ...
            try:
                ...
                new_val = 0.0
            update_client(cid, self.current_client[1], self.current_client[2], self.current_client[3],
                          self.current_client[4], self.current_client[5], new_val, self.current_client[7])
        elif field == 'comment':
            new_val = self.comment_text.get('1.0', 'end').strip()
            update_client(cid, self.current_client[1], self.current_client[2], self.current_client[3],
                          self.current_client[4], self.current_client[5], self.current_client[6], new_val)
    ...
    def edit_current_client(self):
        ...
        if dlg.result:
            name, doc_type, specialist, rate, rate_prev, rate_prev2, comment = dlg.result
            update_client(self.current_client[0], name, doc_type, specialist, rate, rate_prev, rate_prev2, comment)
            self.refresh_client_lists()
            conn = sqlite3.connect(DB_NAME)
            c = conn.cursor()
    ...
    def add_client(self, doc_list_type):
        ...
        if dlg.result:
            name, doc_type, specialist, rate, rate_prev, rate_prev2, comment = dlg.result
            add_client(name, doc_type, specialist, rate, rate_prev, rate_prev2, comment)
            self.refresh_client_lists()
    ...
    def refresh_months_table(self):
        ...
        for m in months_to_show:
            ...
            item_id = self.month_tree.insert('', 'end', values=(month_display, inv_str, act_str, docs_str, visits))
    ...
    def edit_month(self, event):
        ...
        # Найдём исходный month_year по отображаемому названию
        month_year = None
        all_months = get_all_months()
        for m in all_months:
        ...
        row = c.fetchone()
        conn.close()
        current_data = row if row else (0, 0, 0, 0)
