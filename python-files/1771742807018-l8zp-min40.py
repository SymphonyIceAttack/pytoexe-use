...
PRE_BLUE = {2,8,14,20,26,32,38,44,50,56}
...
# ================= FUNCTIONS =================
def calculate(event=None, ai_input=False):
    txt = entry.get().strip()
    ...
    middle_lbl_text = f"{m} {sign}"

    # ======= ألوان المربع الأوسط مع AI =======
    if ai_input and v in PRE_BLUE:
        middle_lbl.config(bg="lightblue")
        special_next = min([x for x in SPECIAL_MIDDLES if x > v], default=m)
        middle_lbl_text = f"<{special_next}"
    elif v in SPECIAL_MIDDLES:
        middle_lbl.config(bg="orange")
    elif any(v == x-1 for x in SPECIAL_MIDDLES):
        middle_lbl.config(bg="yellow")
    else:
        middle_lbl.config(bg=night_bg if dark_mode else "white")

    middle_lbl.config(text=middle_lbl_text)
...
def auto_update():
    if not ai_mode:
        return
    now = datetime.now()
    ai_number = (now.hour*60 + now.minute) % 60
    entry.delete(0,"end")
    entry.insert(0, ai_number)
    calculate(ai_input=True)
