...
class TypingTest:
    def __init__(self, master):
        ...
        # Надпись внизу
        self.bottom_label = tk.Label(master, text='ГБУ "Система 112" г. Москва 2026', font=("Arial", 10))
        self.bottom_label.pack(pady=10)

        # Инициализация счетчиков ошибок
        self.error_counts = defaultdict(int)
        self.total_errors = 0
        self.all_errors = defaultdict(int)  # Общие ошибки по всему тексту
    ...
    def start_test(self):
        ...
        self.error_counts = defaultdict(int)
        self.total_errors = 0
        self.all_errors = defaultdict(int)
    ...
    def check_text(self, event=None):
        ...
        for i in range(min_length):
            ...
            if typed_text[i] == original_text[i]:
                ...
                self.error_counts[original_text[i]] += 1
                self.all_errors[original_text[i]] += 1
        ...
        if len(typed_text) > len(original_text):
            for i in range(len(original_text), len(typed_text)):
                ...
                self.text_display.tag_add("error", start_pos, end_pos)
                self.text_display.tag_config("error", foreground="red")
                # Можно считать ошибку за отсутствие символа или пропустить
                # Тут пропускаем, так как ввод больше оригинала

        # Итоговое число ошибок
        self.total_errors = sum(self.error_counts.values())
        ...
        if self.correct_letters == self.total_letters:
            ...
            # Формируем строку с ошибками по символам
            errors_summary = "\n".join([f"Символ '{char}': {count} ошибок" for char, count in self.all_errors.items() if count > 0])
