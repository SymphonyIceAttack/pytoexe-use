...
# GUID для класса DiskDrive (стандартный для съемных дисков)
...
def parse_usb_device_id(device_id_string):
    """
    Парсит строку DeviceID вида:
    USBSTOR\DISK&VEN_KINGSTON&PROD_DATATRAVELER_3.0&REV_0000\E0D55EA573FD19C15844006D&0
    
    Возвращает словарь с полями:
    bus_class, dev_type, vendor, product, rev, serial, guid
    """
    result = {
        "bus_class": "",
        "dev_type": "",
    ...
    # Ищем часть после "USBSTOR\"
    match = re.search(r'USBSTOR\\([A-Z]+)&(.*?)\\(.+)', device_id_string)
    if not match:
        # Если формат не совпадает, возвращаем то, что есть, или пытаемся найти хоть что-то
        result["bus_class"] = "USBSTOR"
        return result

    dev_type_raw = match.group(1)       # DISK
    props_part = match.group(2)         # VEN_KINGSTON&PROD_...&REV...
    serial_part = match.group(3)        # E0D5...&0

    result["bus_class"] = "USBSTOR"
    result["dev_type"] = dev_type_raw

    # Парсим свойства: VEN_..., PROD_..., REV_...
    props = {}
    for item in props_part.split('&'):
        if '=' in item:
            ...
            # Обработка формата VEN_NAME, PROD_NAME
    ...
    # Серийный номер - это всё до последнего &0 или просто остаток, если &0 нет
    # Обычно формат: SERIAL\SERIAL_NUMBER&0
    # Но в твоем примере: ...15844006D&0. Берем всё до последнего '&'
    if '&' in serial_part:
...
def save_to_log(info, usb_output, android_results, log_file):
    ...
    with open(log_file, "a", encoding="utf-8") as f:
        ...
        # Здесь мы парсим вывод и пишем его в красивом виде
        if isinstance(usb_output, str):
            ...
            for line in lines:
                ...
                # Извлекаем DeviceID из строки вида:
                # \\PC...\root\cimv2:Win32_PnPEntity.DeviceID="USBSTOR\..."
