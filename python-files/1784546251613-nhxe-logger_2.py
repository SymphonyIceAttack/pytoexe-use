...
def format_parsed_usb(parsed_data):
    # Теперь это просто набор полей через ;
    return (
...
def save_to_log(info, usb_output, android_results, log_file):
    ...
    # Готовим Android-результаты как одну строку (экранируем внутренние ; через ^)
    ...
    # Если устройств нет — пишем одну строку с none
    if not usb_devices:
        full_line = f"{sys_info_str};usb_devices=none;android_results={android_str}\n"
        with open(log_file, "a", encoding="utf-8") as f:
            f.write(full_line)
        return
    ...
    for parsed in usb_devices:
        ...
        # Каждая строка: sys_info;usb_info;android_info
        full_line = f"{sys_info_str};{usb_part};android_results={android_str}\n"
