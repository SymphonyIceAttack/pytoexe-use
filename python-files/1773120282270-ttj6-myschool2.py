...
class SchoolManagementApp:
    ...
    def setup_database(self):
        ...
        cursor.execute('''CREATE TABLE IF NOT EXISTS students 
                          (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, class TEXT, admission_date TEXT, contact TEXT)''')
        # Finance Table
        cursor.execute('''CREATE TABLE IF NOT EXISTS finance 
                          (id INTEGER PRIMARY KEY AUTOINCREMENT, type TEXT, amount REAL, category TEXT, date TEXT, description TEXT)''')
        conn.commit()
        conn.close()
    ...
    def add_student(self):
        ...
        if name and s_class:
            conn = sqlite3.connect('school_data.db')
            cursor = conn.cursor()
            cursor.execute("INSERT INTO students (name, class, admission_date) VALUES (?, ?, ?)", (name, s_class, date))
            conn.commit()
            conn.close()
    ...
    def setup_accounts_tab(self):
        ...
        btn_income = tk.Button(frame, text="Add as Income", bg="#4CAF50", fg="white", command=lambda: self.record_finance("Income"))
        btn_income.grid(row=1, column=1, pady=10)

        btn_expense = tk.Button(frame, text="Add as Spending", bg="#f44336", fg="white", command=lambda: self.record_finance("Expense"))
    ...
    def record_finance(self, f_type):
        ...
        if amount and cat:
            try:
                ...
                conn = sqlite3.connect('school_data.db')
                cursor = conn.cursor()
                cursor.execute("INSERT INTO finance (type, amount, category, date) VALUES (?, ?, ?, ?)", (f_type, amount_val, cat, date))
                conn.commit()
                conn.close()
                messagebox.showinfo("Success", f"{f_type} of {amount} recorded.")
    ...
    def show_report(self, period):
        ...
        if period == "monthly":
            date_filter = date_now.strftime("%Y-%m")
            query = f"SELECT type, SUM(amount) FROM finance WHERE date LIKE '{date_filter}%' GROUP BY type"
        else:
            date_filter = date_now.strftime("%Y-%m-%d")
            query = f"SELECT type, SUM(amount) FROM finance WHERE date = '{date_filter}' GROUP BY type"

        cursor.execute(query)
        results = cursor.fetchall()
        
        self.txt_report.delete(1.0, tk.END)
        self.txt_report.insert(tk.END, f"--- {period.upper()} FINANCIAL SUMMARY ({date_filter}) ---\n\n")
