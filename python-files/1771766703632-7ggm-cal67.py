...
current_group_idx = 0
ai_mode = False
ai_delay_sec = 0  # Timer delay for AI
...
def start_timer():
    ...
    root.after(ai_delay_sec*1000, _countdown, 60)
...
def calculate(event=None):
    ...
    if ai_mode and v in AI_NEXT_VALUES:
        # فقط العلامة في المربع اللبني تعكس حسب الرقم الأوسط
        ...
        stop_timer()
        sign = "<" if v < m else ">" if v > m else "="
        middle_lbl.config(text=f"{m} {sign}", bg="white")  # افتراضي أبيض
...
def draw_ai_lamp():
    ...
    size=int(BASE_FONT*current_scale*0.45*1.3)  # +30% أكبر
