...
class ScreenMotionDetector:
            ...
            if event == cv2.EVENT_LBUTTONDOWN:
                ...
                if self.start_point and self.end_point:
                    ...
                    if width > 20 and height > 20:
                        nonlocal roi_selected
                        roi_selected = (x1, y1, width, height)
    ...
    def process_frame(self, frame):
        ...
        if not motion_detected:
            ...
            # Обновление с задержкой для уменьшения шума
        ...
        if self.roi is not None:
            cv2.putText(display_frame, f"Область: {self.roi[2]}x{self.roi[3]}", (10, 90), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
        
        cv2.putText(display_frame, f"Звук: {'Вкл' if self.sound_enabled else 'Выкл'}", (10, 120), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
        
        cv2.putText(display_frame, "ESC-выход SPACE-выбор области R-сброс S-звук +/-чувствительность", (10, display_frame.shape[0]-10), 
