...
class RE4PatcherApp(tk.Tk):
    ...
    def _compare_exes(self, path_a, path_b, label_a="A", label_b="B"):
        """Core compare logic. Returns report string."""
        try:
        ...
        if len(data_a) != len(data_b):
            size_warn = (
                ...
                + label_a + ": " + str(len(data_a)) + " bytes\n"
                + label_b + ": " + str(len(data_b)) + " bytes\n\n"
            )
        else:
            size_warn = ""
        ...
        while i < limit:
            if data_a[i] != data_b[i]:
                # find end of differing run
                j = i
                while j < limit and data_a[j] != data_b[j]:
                    j += 1
                block_a = data_a[i:j].hex().upper()
                block_b = data_b[i:j].hex().upper()
                diffs.append((i, j - i, block_a, block_b))
                i = j
            else:
                i += 1

        if not diffs:
            return size_warn + "No differences found. Files are identical."

        lines = [
            size_warn +
            "Found " + str(len(diffs)) + " difference(s):\n" +
            "=" * 60
        ]
        for offset, length, val_a, val_b in diffs:
            lines.append(
                "\nOffset: 0x" + format(offset, "08X") +
                "  Length: " + str(length) + " byte(s)" +
                "\n  " + label_a + ": " + " ".join(val_a[i:i+2] for i in range(0, min(len(val_a), 64), 2)) +
                ("..." if len(val_a) > 64 else "") +
                "\n  " + label_b + ": " + " ".join(val_b[i:i+2] for i in range(0, min(len(val_b), 64), 2)) +
                ("..." if len(val_b) > 64 else "")
            )
        return "\n".join(lines)
    ...
    def _compare_two_exes(self):
        ...
        for label, var in [("EXE  A:", var_a), ("EXE  B:", var_b)]:
            ...
            make_button(row, "...", lambda v=var, l=label: pick(v, l),
        ...
        def run():
            ...
            dlg.destroy()
            report = self._compare_exes(a, b,
                                         label_a=os.path.basename(a),
                                         label_b=os.path.basename(b))
            if report:
                self._show_report("Compare: " + os.path.basename(a) +
                                  " vs " + os.path.basename(b), report)
    ...
    def _compare_with_original(self):
        ...
        if not os.path.isfile(ORIG_FILE):
            ...
            return
        report = self._compare_exes(
            ORIG_FILE, exe,
            label_a="bio4_original.exe",
            label_b=os.path.basename(exe)
        )
        if report:
            self._show_report(
