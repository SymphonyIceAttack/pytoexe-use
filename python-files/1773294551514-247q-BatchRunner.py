...
class BatchRunner:
    ...
    def scan_batch_files(self):
        ...
        try:
            ...
            # Can't print to terminal yet if it doesn't exist
    ...
    def refresh_buttons(self):
        ...
        for widget in self.button_frame.winfo_children():
            if isinstance(widget, tk.Button) and widget.cget('text') != "Refresh List":
                widget.destroy()
            ...
            def read_output():
                while True:
                    ...
                    if output and self.terminal:
                        self.root.after(0, self.print_to_terminal, output.strip(), 'output')
                
                # Check for errors
                stderr = self.process.stderr.read()
                if stderr and self.terminal:
                    if not self.is_windows and "fixme" in stderr.lower():
                        # Wine FIXME messages are usually harmless
