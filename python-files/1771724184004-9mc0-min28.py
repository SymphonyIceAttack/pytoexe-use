...
def show_trade(event=None):
    try:
        ...
        output.config(state="disabled")
        return
    # إيجاد أول مجموعة تحتوي الدقيقة
...
def next_group(event=None):
    if current_index is not None and current_index<len(trade_table)-1:
        show_group(current_index+1)
...
btn_generate = tk.Button(btn_frame,text="Generate", font=main_font, width=10, command=show_trade)
