...
class PersonaEditor:
    ...
    def generate_prompt(self):
        ...
        lines = []
        # 标题
        lines.append(f"# Role：{self.get_text_value('name')}")
        lines.append("\n## 基本信息")
        ...
        for key in identity_keys:
            if key == "name":
                lines.append(f"-姓名：{self.get_text_value('name')}。")
                lines.append(f"-昵称：{self.get_text_value('nickname')}。")
            elif key != "nickname":
        ...
        for key in personality_keys:
            lines.append(f"**{p_map[key]}**：\n{self.get_text_value(key)}。")

        lines.append(f"**语言风格**：\n{self.get_text_value('speech')}。")
