...
def init_db():
    ...
    for col in ('rate_prev_year', 'rate_prev2_year', 'is_summer'):
        try:
            c.execute(f"ALTER TABLE clients ADD COLUMN {col} REAL" if 'rate' in col else f"ALTER TABLE clients ADD COLUMN {col} INTEGER DEFAULT 0")
...
def get_total_cost(client_ids, year):
    """Возвращает общую стоимость выездов для клиентов за год (ставка * сумма_выездов)"""
    if not client_ids:
        return 0
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
...
def display_name(client):
    """Возвращает имя клиента для отображения (с приставкой 'Летний', если is_summer=1)"""
    name = client[1]
    if client[8] == 1:  # is_summer
