...
def calculate(event=None):
    ...
    # Day/Night white label behavior
...
def toggle_mode():
    ...
    for b in [calc_btn, clear_btn, prev_btn, next_btn, pin_btn, edit_btn, history_btn, view_btn, ai_btn]:
...
def apply_scale(val):
    ...
    for w in [calc_btn, clear_btn, prev_btn, next_btn, ai_btn]:
...
def toggle_ai():
    ...
    ai_mode = not ai_mode
    if ai_mode:
        ai_btn.config(relief="sunken")
        update_ai()
    else:
        ai_btn.config(relief="raised")

def update_ai():
    if ai_mode:
        # استخدم الوقت الحالي للحصول على الساعات والدقائق والثواني
        now = datetime.now()
        # نحصر الرقم في حدود 0-59 (مثال)
        v = now.minute  # يمكن تعديل الحساب حسب اللوجيك المطلوب
        entry.delete(0, tk.END)
        entry.insert(0,str(v))
        calculate()
        root.after(1000, update_ai)  # تحديث كل ثانية
...
ai_btn = tk.Menu(menubar, tearoff=0)
menubar.add_cascade(label="AI", menu=ai_menu)
ai_menu.add_command(label="Toggle AI Mode", command=toggle_ai)
