...
def fix_ar(text):
    ...
    return text  # no bidi lib: return as-is
...
class RE4PatcherApp(tk.Tk):
    ...
    def _dialog_punisher_pierce(self, code_id):
        """Two separate numeric inputs for punisher pierce count."""
        exe = self.exe_path.get().strip()
        if not exe or not os.path.isfile(exe):
            messagebox.showerror("Error", "Please select a valid bio4.exe first.")
            return
        ...
        for field in fields:
            ...
            make_label(row, lbl, fg=TEXT_DIM, bg="#111",
                       font=FONT_SMALL, width=28, anchor="w").pack(side="left")

            var = tk.StringVar(value=str(current))
            e = tk.Entry(row, textvariable=var, font=FONT_NORMAL,
                         fg=ACCENT2, bg="#1a1a1a", insertbackground=ACCENT2,
                         relief="flat", bd=0,
                         highlightthickness=1, highlightbackground=BORDER,
                         width=6, justify="center")
            e.pack(side="left", ipady=3, padx=4)
            self._add_paste_menu(e)
            ...
            def make_apply(v=var, f=field):
                def do():
                    try:
                        ...
                        return
                    # write: B8 + val as 4 bytes little-endian
            ...
            tk.Button(row, text="Apply" if CURRENT_LANG == "en" else "تطبيق",
                      font=FONT_TINY, fg=GREEN, bg="#1a2a0a",
                      activeforeground=GREEN, activebackground="#2a4a1a",
                      relief="flat", bd=0, cursor="hand2",
                      highlightthickness=1, highlightbackground="#2a5a2a",
                      command=make_apply(), padx=6, pady=2
                      ).pack(side="left")
        ...
        make_button(dlg, "Close" if CURRENT_LANG == "en" else "إغلاق",
                    ...
                    active_bg="#2a2a2a", width=8).pack(pady=12)
