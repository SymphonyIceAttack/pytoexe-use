...
def show_menu():
    ...
    print_colored("10. Remove ALL dots (complete) - no dots left")
    print_colored("11. Remove extension only (keep name)")
    print_colored("12. Change file extension")
    print_colored("13. Shorten filename")
    print_colored("14. Auto-rename duplicates only")
    print_colored("15. Remove ALL LETTERS (keep numbers, dots, symbols)")  # NEW OPTION
    print_colored("16. Exit")
    print_colored("="*60)
    return input(f"{BG_WHITE}{TEXT_BLACK}Choose (1-16): {RESET}")
...
def remove_all_letters():  # NEW FUNCTION
    """Remove ALL letters (a-z, A-Z) from filenames, keep numbers, dots, special chars"""
    folder, files = get_files()
    if not files: 
        input("No files found! Press Enter...")
        return
    
    print_colored("\nRemoving ALL letters from filenames...")
    new = []
    for _, f in files:
        # Remove all uppercase and lowercase letters
        new_name = re.sub(r'[a-zA-Z]', '', f)
        new.append(new_name)
    
    rename_files_with_auto((folder, files), new)
    print_colored("\n✅ All letters removed! Filenames now contain only:")
    print_colored("   - Numbers")
    print_colored("   - Dots")
    print_colored("   - Special characters (_, -, etc.)")
    input(f"{BG_WHITE}{TEXT_BLACK}Press Enter...{RESET}")
...
def main():
    while True:
        try:
            ...
            elif choice == '15': remove_all_letters()  # NEW OPTION
            elif choice == '16': 
                clear_screen()
