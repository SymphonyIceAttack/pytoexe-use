...
class SnowflakeCalculator:
    ...
    def create_modern_styles(self):
        ...
        try:
            ...
            style.theme_use('winnative')  # 如果vista不可用，使用winnative
        
        # 修复按钮样式 - 添加明确的背景色和边框设置
        style.configure('TButton',
                       font=('Segoe UI', 10),
                       padding=(10, 5),
                       background=self.colors['button_bg'],
                       borderwidth=1,
                       relief='solid')  # 使用solid而不是flat，避免显示问题
    ...
    def create_title_bar(self):
        ...
        title_frame.pack(fill=tk.X, padx=15, pady=(15, 8))
    ...
    def create_snowflake_tab(self):
        """创建雪花生成器标签页"""
        # 主容器框架
        container = ttk.Frame(self.tab1, padding=12)
        container.pack(fill=tk.BOTH, expand=True)
        
        # 使用Canvas和Frame实现可滚动区域（如果需要）
        ...
        # 输入区域
        input_frame = ttk.LabelFrame(scrollable_frame, text="第一步：输入第一瓣雪花的角度数据", padding=12)
        input_frame.pack(fill=tk.X, pady=(0, 12))
        
        ttk.Label(input_frame, text="请输入角度数据（用逗号分隔）：", 
                 font=('Microsoft YaHei', 10)).pack(anchor=tk.W, pady=(0, 6))
        
        self.angle_entry = tk.Text(input_frame, height=3, font=('Consolas', 10))
        self.angle_entry.pack(fill=tk.X, pady=(0, 8))
        
        # 配置区域
        config_frame = ttk.LabelFrame(scrollable_frame, text="第二步：配置雪花参数", padding=12)
        config_frame.pack(fill=tk.X, pady=(0, 12))
        
        # 瓣数配置
        petals_frame = ttk.Frame(config_frame)
        petals_frame.pack(fill=tk.X, pady=(0, 10))
        ...
        petal_combo.pack(side=tk.LEFT, padx=(0, 20))
        ...
        # 输出区域
        output_frame = ttk.LabelFrame(scrollable_frame, text="第三步：生成结果", padding=12)
        ...
        ttk.Label(result_frame, text="生成的角度数据：", 
                 font=('Microsoft YaHei', 11, 'bold')).pack(anchor=tk.W, pady=(0, 6))
    ...
    def create_angle_calculator_tab(self):
        """创建角度加减器标签页"""
        # 主容器框架
        container = ttk.Frame(self.tab2, padding=12)
        container.pack(fill=tk.BOTH, expand=True)
        ...
        # 输入区域
        input_frame = ttk.LabelFrame(scrollable_frame, text="输入角度数据", padding=12)
        input_frame.pack(fill=tk.X, pady=(0, 12))
        
        ttk.Label(input_frame, text="请输入角度数据（用逗号分隔）：", 
                 font=('Microsoft YaHei', 10)).pack(anchor=tk.W, pady=(0, 6))
        
        self.calc_angle_entry = tk.Text(input_frame, height=3, font=('Consolas', 10))
        self.calc_angle_entry.pack(fill=tk.X, pady=(0, 8))
        ...
        ttk.Button(calc_button_frame, text="⬅️ 从雪花生成器导入", 
                  command=self.import_from_snowflake).pack(side=tk.LEFT, padx=(0, 8))
        
        ttk.Button(calc_button_frame, text="清空", 
                  command=self.clear_calculator).pack(side=tk.LEFT)
        
        # 计算配置区域
        calc_frame = ttk.LabelFrame(scrollable_frame, text="角度加减配置", padding=12)
        calc_frame.pack(fill=tk.X, pady=(0, 12))
        ...
        add_angle_entry.pack(side=tk.LEFT, padx=(0, 8))
        ...
        ttk.Checkbutton(calc_frame, text="将结果归一化到0-360度", 
                       variable=self.normalize_var).pack(anchor=tk.W, pady=(5, 0))
        
        # 计算按钮
        calc_btn_frame = ttk.Frame(calc_frame)
        calc_btn_frame.pack(fill=tk.X, pady=(8, 0))
        ...
        # 结果统计
        self.calc_stats_label = ttk.Label(result_frame, text="就绪", font=('Microsoft YaHei', 10))
        self.calc_stats_label.pack(anchor=tk.W, pady=(0, 6))
    ...
    def create_status_bar(self):
        """创建现代化状态栏"""
        # 状态栏框架
        status_frame = ttk.Frame(self.main_frame, style='TFrame', height=26)
        status_frame.pack(fill=tk.X, side=tk.BOTTOM, padx=15, pady=(0, 10))
        status_frame.pack_propagate(False)
        
        # 状态文本
        self.status_bar = ttk.Label(status_frame, text="就绪", style='Status.TLabel')
        self.status_bar.pack(side=tk.LEFT, padx=(8, 0))
        
        # 添加分隔符
        ttk.Label(status_frame, text="|", style='Status.TLabel', 
                 foreground=self.colors['border']).pack(side=tk.LEFT, padx=(10, 10))
        ...
        # 添加作者信息
        ttk.Label(status_frame, text="作者: iDQhonefully", style='Status.TLabel').pack(side=tk.RIGHT, padx=(0, 8))
