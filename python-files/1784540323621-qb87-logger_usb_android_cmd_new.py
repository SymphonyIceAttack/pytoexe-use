...
def get_usb_devices_wmic():
    # Используем прямой запрос к Win32_PnPEntity для надежного получения DeviceID
...
def save_to_log(info, usb_devices, android_results, log_file):
    ...
    if usb_devices and len(usb_devices) > 0:
        ...
        dev_type = first_usb.get("dev_type", "Unknown")      # DISK
    ...
    # ГЛАВНОЕ ИЗМЕНЕНИЕ: объединяем ВСЕ найденные папки через запятую
    if android_results:
