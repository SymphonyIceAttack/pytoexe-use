...
# ----------------------------------------------------------------------
# Функции для определения кодировки (ручной метод, без внешних библиотек)
# ----------------------------------------------------------------------

def detect_encoding_simple(filepath: str) -> str:
    """
    Определяет кодировку файла ручным методом (без chardet).
    Проверяет наиболее популярные кодировки для русского языка.
    """
    encodings_to_try = ['utf-8-sig', 'utf-8', 'cp1251', 'windows-1251', 'cp866', 'koi8-r', 'latin-1']
    
    with open(filepath, 'rb') as f:
        raw_data = f.read(10000)  # Читаем первые 10KB для анализа
    
    for encoding in encodings_to_try:
        try:
            decoded = raw_data.decode(encoding)
            # Проверяем, есть ли русские буквы (для достоверности)
            if re.search(r'[а-яА-ЯёЁ]', decoded):
                return encoding
            # Если нет русских букв, но декодировалось без ошибок, тоже подходит
            if encoding in ['utf-8-sig', 'utf-8']:
                # Для UTF-8 проверим ещё раз без BOM
                continue
        except UnicodeDecodeError:
            continue
    
    # Если ничего не подошло, пробуем определить по первому байту
    if raw_data.startswith(b'\xef\xbb\xbf'):
        return 'utf-8-sig'
    
    # Последняя попытка: cp1251 (самая частая для русских Excel)
    try:
...
def read_csv_safe(filepath: str, skip_header=True, delimiter=';') -> List[List[str]]:
    """
    Безопасное чтение CSV с автоопределением кодировки (только стандартные библиотеки)
    """
    ...
    try:
        ...
        # Если не удалось с определённой кодировкой, пробуем другие популярные
...
class App:
    def __init__(self, root):
        ...
        dev_label = tk.Label(top_frame, text="Разработчик: Омельченко Алексей | Версия 2.3 (полностью переносимая, CSV)", 
        ...
        self.status_var.set("Готов к работе (CSV, автоопределение кодировки, без внешних библиотек)")
    ...
    def arshin_load_csv(self):
        ...
        try:
            ...
            messagebox.showerror("Ошибка", f"Не удалось загрузить CSV:\n{str(e)}\n\n"
                                          f"Совет: сохраните файл в кодировке UTF-8 или используйте нашу кнопку скачивания шаблона.")
