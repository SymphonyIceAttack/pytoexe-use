...
class RE4PatcherApp(tk.Tk):
    ...
    def _open_settings(self):
        ...
        make_label(dlg, "Recommended Codes",
                   fg=ACCENT2, bg="#111", font=FONT_SMALL
                   ).pack(pady=(4, 6))
        make_label(dlg,
                   fix_ar("احسن تجميعه لكودات bio4.exe"),
                   fg=MUTED, bg="#111", font=FONT_TINY
                   ).pack()

        def best_and_close():
            dlg.destroy()
            self._apply_best_settings()

        make_button(dlg, "Apply Best Settings", best_and_close,
                    fg=ACCENT2, bg="#2a2a0a", active_bg="#3a3a1a", width=18
                    ).pack(pady=10)
