...
def log_error(message):
    """Вспомогательная функция для записи ошибок в лог, если print нельзя использовать"""
    try:
...
def run_wmic_command(command):
    try:
        result = subprocess.run(
            ...
            text=False,
            timeout=10,
            # Это скрывает всплывающие окна от wmic/find
            creationflags=subprocess.CREATE_NO_WINDOW 
        )
        
        # Пробуем декодировать. Если кодировка другая (редко), можно попробовать 'cp437'
...
if __name__ == "__main__":
    try:
        ...
        # print() удалены! Вся информация только в логе.
