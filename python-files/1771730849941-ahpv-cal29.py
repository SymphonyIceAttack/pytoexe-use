...
def apply_scale(val):
    ...
    lamp_size = max(1,int(size*0.6))  # حجم اللمبة
    ...
    # تعديل حجم اللمبة
    ai_lamp.config(width=lamp_size, height=lamp_size)
...
def auto_update_time():
    ...
    minute = now.minute
    second = now.second
    # نحسب الرقم الحالي من الساعة: مجرد مثال
    current_number = (hour*60 + minute) % 60
    entry.delete(0, tk.END)
    entry.insert(0,str(current_number))
    calculate()
    root.after_id = root.after(1000, auto_update_time)  # كل ثانية
...
# ================= AI LAMP =================
ai_lamp = tk.Label(btns, width=2, height=1, bg="red", relief="raised", bd=1)
ai_lamp.pack(side="left", padx=3)
ai_btn = tk.Button(btns, text="AI", command=toggle_ai)
