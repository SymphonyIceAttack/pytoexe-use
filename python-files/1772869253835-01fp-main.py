...
def fetch_latest_release_zip(cfg: dict, log: callable) -> bytes:
    ...
    asset_name = str(gh.get("asset_name", "ltsteamplugin.zip")).strip()
        ...
        if "plugin.json" in files:
            try:
                ...
                with open(p, "r", encoding="utf-8") as f:
                    txt = f.read()
                if re.search(r'"common_name"\s*:\s*"LuaTools"', txt):
                    matches.append(root)
            except Exception:
                continue
    if matches:
        log(f"Found {len(matches)} LuaTools plugin location(s)")
        return matches
    # Fallback: create default target
    target = os.path.join(plugins_dir, "ST-Steam_Plugin")
    os.makedirs(target, exist_ok=True)
    log(f"No existing LuaTools plugin found; using {target}")
    return [target]
...
if __name__ == "__main__":
    ...
    if steam_path:
        ...
        wait_for_keypress("")
        os.startfile("steam://millennium/settings/plugins/enable/l