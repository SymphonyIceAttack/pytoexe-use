...
def http_get_json(url):
    # Force a fresh response from TSETMC/CDN on every poll.
    # Without this, a CDN/proxy may return the same status repeatedly,
    # making an IS -> IR transition invisible until a restart.
    cache_buster = f"_payesh_ts={time.time_ns()}"
    sep = "&" if "?" in url else "?"
    fresh_url = f"{url}{sep}{cache_buster}"
    ...
    def monitor_symbol(self, index, item):
        ...
        while self.running and not self.stop_event.is_set():
            try:
                ...
                self.root.after(
                    0,
                    lambda n=name, c=code:
                    self.write_log(f"{n}: وضعیت خام = {c}")
                )
