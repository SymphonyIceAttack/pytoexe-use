Вот полный код Python‑скрипта, который нужно скомпилировать в EXE‑файл для работы с контроллером **BLSD‑50** в качестве OPC UA‑сервера. Сохраните его в файл `blsd50_opc_server.py`:
...
class BLSD50OPCServer:
    ...
    def read_modbus_data(self):
        ...
        try:
            # Читаем 6 регистров (настройте под ваш контроллер)
            result = self.modbus_client.read_holding_registers(address=0, count=6, slave=1)
            if result.isError():
            ...
            data['Status'] = registers[6] if len(registers) > 6 else 0
            # Скорость (регистр 7 как int16)
            data['Speed'] = registers[7] if len(registers) > 7 else 0
            # Код ошибки (регистр 8 как int16)
            data['ErrorCode'] = registers[8] if len(registers) > 8 else 0
        ...
        if data:
            for name, value in data.items():
                if name in self.variables:
                    ...
                    _logger.debug(f"Обновлено {name}: {value}")
    ...
    def stop(self):
        ...
        _logger.info("OPC UA сервер остановлен")

# Запуск сервера
if __name__ == "__main__":
    # Создаём сервер для BLSD-50
    # Настройте IP-адрес контроллера под вашу сеть
    server = BLSD50OPCServer(
        opc_endpoint="opc.tcp://0.0.0.0:4840/blsd50",
        modbus_ip="192.168.1.100"  # Замените на реальный IP вашего BLSD-50