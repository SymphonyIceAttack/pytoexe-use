...
class RE4PatcherApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self._global_selected = set()  # persists across section switches
...
class SpeechColorPanel(tk.Frame):
    ...
    def _render(self, raw, colors):
        ...
        for m in token_re.finditer(raw):
            ...
            code  = token[1:-1]  # e.g. 0x0300
            if code == "0x0400":
                self._text.insert("end", " [New Page] ", ("newpage",))
            elif code == "0x0300":
                self._text.insert("end", "\n")
            elif code.startswith("0x060"):
                # color change: 0x0600 + next token is color index
                pass  # handled in pair below
            elif code == "0x0100":
                self._text.insert("end", "\n")
            elif code == "0x0100":
                self._text.insert("end", "\n" + "─"*40 + "\n", ("newpage",))
                current_color_tag = None
            elif re.match(r"0x06[0-9a-f]{2}", code):
                idx_str = code[-2:]
                try:
                    idx = int(idx_str, 16)
                    current_color_tag = f"col{idx}" if idx < MDT_COLOR_COUNT else None
                except Exception:
                    pass
            pos = m.end()
        ...
        # remaining text
        if pos < len(raw):
            self._text.insert("end", raw[pos:], ("hover_word",))
...
class LockedPanel(tk.Frame):
    ...
    def __init__(self, parent, tool_label, required_code_id,
                 required_code_name, master_app, **kw):
        super().__init__(parent, bg="#0b0b0b", **kw)
        self.master_app         = master_app
    ...
    def _check_lock(self):
        """Re-check if required code is now applied."""
        # look for code_manager app instance
        cm = getattr(self.master_app, "_cm_app", None)
        if cm and cm.applied.get(self.required_code_id, False):
            # unlock — show coming soon for now
            self._lbl_lock.configure(
                text="✓ Code is enabled. Module ready.",
                fg="#7cfc7c"
            )
        else:
            self._lbl_lock.configure(
    ...
    def _build(self):
        ...
        tk.Label(f, text="🔒", fg="#c8a035", bg="#0b0b0b",
    ...
    def _build(self):
        ...
        try:
            ...
            tk.Label(self,
                     ...
                     fg="#e05050", bg="#0b0b0b",
                     font=("Courier New", 9)).pack(pady=40, padx=20)
...
# ── RE4PatcherApp embedded mode support ──────────────────────────────────────
# Patch RE4PatcherApp.__init__ to support embedded=True and shared_exe_var

_original_patcher_init = None

def _patch_patcher_app():
    global _original_patcher_init
    if _original_patcher_init is not None:
        return
    _original_patcher_init = RE4PatcherApp.__init__

    def new_init(self, master=None, embedded=False, shared_exe_var=None, **kw):
        if embedded and master is not None:
            # Initialize as Frame, not Tk
            tk.Frame.__init__(self, master, bg="#0b0b0b", **kw)
            ...
            try:
                with open(INFO_FILE, encoding="utf-8") as f:
                    self.codes_info = json.load(f)
                with open(DATA_FILE, encoding="utf-8") as f:
                    self.codes_data = json.load(f)
            except Exception as e:
                tk.Label(self, text="Error loading codes:\n" + str(e),
                         fg="#e05050", bg="#0b0b0b",
                         font=("Courier New", 9)).pack(pady=20, padx=10)
                return
            self.sections_list = self.codes_info["sections"]
            self.all_codes     = self.codes_info["codes"]
            self.code_by_id    = {c["id"]: c for c in self.all_codes}
            ...
            for c in self.all_codes:
                self.codes_by_section.setdefault(c["section"], []).append(c)
            self._build_ui()
            # scan bar: just show Scan button (no path/browse)
            self._inject_scan_only_bar()
            self.select_section(self.sections_list[0]["id"])
            self.status_total_var.set(
                ("Total codes: " if CURRENT_LANG == "en" else "إجمالي الأكواد: ")
                + str(len(self.all_codes))
            ...
            if shared_exe_var:
                shared_exe_var.trace_add("write", self._on_exe_path_change)
            if APP_SETTINGS.get("remember_exe", True):
                last = APP_SETTINGS.get("last_exe", "")
                if last and os.path.isfile(last) and shared_exe_var:
                    shared_exe_var.set(last)
                    if APP_SETTINGS.get("auto_scan_startup", False):
                        self.after(400, self._scan)
            self._first_scan_done = False
        else:
            _original_patcher_init(self, **kw)

    RE4PatcherApp.__init__ = new_init

    def _inject_scan_only_bar(self):
        """Replace the path bar with a Scan-only bar for embedded mode."""
        # hide the original path_bar if it exists
        try:
            self.path_bar.pack_forget()
        except Exception:
            pass
        # add a compact scan bar at the top of the frame
        scan_bar = tk.Frame(self, bg="#0e0e0e")
        scan_bar.pack(fill="x", side="top", before=self.winfo_children()[0]
                      if self.winfo_children() else None)
        ...
        self.scan_status_var = tk.StringVar(value="")
        tk.Label(scan_bar, textvariable=self.scan_status_var,
                 fg="#555", bg="#0e0e0e",
                 font=("Courier New", 8)).pack(side="left", padx=8)

    RE4PatcherApp._inject_scan_only_bar = _inject_scan_only_bar
