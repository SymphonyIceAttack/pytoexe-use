...
def get_usb_devices_wmic():
    # Ищем устройства напрямую через Win32_PnPEntity для более чистого парсинга
                ...
                for entry in entries:
                    ...
                    if os.path.isdir(entry_path):
                        ...
                        if folder_name in {f.lower() for f in ANDROID_FOLDERS}:
                            ...
                            # Если нужна только ПЕРВАЯ найденная папка, раскомментируй строку ниже:
                            # return found_paths 
...
def save_to_log(info, usb_devices, android_results, log_file):
    ...
    if usb_devices and len(usb_devices) > 0:
        ...
        dev_type = first_usb.get("dev_type", "Unknown")      # В твоем примере это DISK
        product = first_usb.get("product", "Unknown")       # USB_DISK_2.0
        serial = first_usb.get("serial", "Unknown")         # Серийник
    else:
        dev_type, product, serial = "NoUSB", "NoUSB", "NoUSB"
    
    # Логика для Android: берем ТОЛЬКО первую найденную папку, как в твоем примере
    if android_results:
        android_str = android_results[0]  # Берем первый элемент списка
    else:
        android_str = "[Папки не найдены]"

    # ФОРМИРУЕМ ИТОГОВУЮ СТРОКУ ТОЧНО ПО ТВОЕМУ ШАБЛОНУ
    log_line = f"{timestamp} || {hostname} || {username} || {dev_type} || {product} || {serial} || {android_str}\n"
