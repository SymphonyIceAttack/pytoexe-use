...
class App:
    ...
    def search_button(self, index):
        ...
        self._mofid_diag("[DIAG] click thread is being created")
    ...
    def test_click(self):
        ...
        self._mofid_diag(f"[DIAG] before left_click | x={self.click_x}, y={self.click_y}")


        left_click(self.click_x, self.click_y)


        self._mofid_diag("[DIAG] after left_click")
        self.gentle_ding()
        self.write_log(
...
# ===== MOFID CLICK DIAGNOSTIC LOGGING =====
# Diagnostic markers are intentionally placed around the click path.
def _mofid_diag(self, message):
    try:
        self.log(message)
    except Exception:
        try:
    ...
    def delayed_click(self, symbol, detected_perf):
        ...
        self._mofid_diag(f"[DIAG] delayed_click: waiting for click_lock")

        with self.click_lock:

            self._mofid_diag(f"[DIAG] delayed_click: click_lock acquired")
            delay_s = self.delay_ms / 1000.0

            self._mofid_diag(f"[DIAG] delayed_click: before delay | delay_s={delay_s:.6f}")
            if self.stop_event.wait(delay_s):
                self._mofid_diag("[DIAG] delayed_click: STOP EVENT fired during delay -> NO CLICK")
                return
            self._mofid_diag("[DIAG] delayed_click: delay finished")
