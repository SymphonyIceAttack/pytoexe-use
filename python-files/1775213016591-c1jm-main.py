...
LOG_FILE = "realistic_sim_fixed.log"
...
# Real IPs (SIMULATION ONLY — no actual connection)
REAL_IPS = [
    "93.123.109.214",
    "60.205.233.234"
]
...
# =========================
# TCP Simulation (SAFE, sequential)
# =========================
def simulate_tcp_beacon():
    ...
    try:
        ...
        if is_real:
            ...
            log_event("tcp_simulated", {
                "ip": ip,
                "port": port,
                "note": "real IP - simulated only"
            })
            return

        # Only SAFE IPs are actually used
...
def run():
    global CYCLES
    print(f"{CYAN}Realistic Crypto-Mining Behavior Simulator (SAFE, Fixed){RESET}")
    log_event("start", {"env": "Windows 11 lab"})
