...
try:
    ...
    show_error_and_exit(f"无法导入 tkinter:\n{e}\n\n请确保 Python 安装了 tkinter 支持。")
except Exception as e:
    show_error_and_exit(f"tkinter 初始化失败:\n{e}")
...
class CythonCompilerGUI:
    ...
    def __init__(self, root):
        ...
        if not CYTHON_AVAILABLE:
            self.log(f"警告: Cython 未正确加载", "WARNING")
            self.log(f"错误信息: {CYTHON_ERROR}", "WARNING")
            self.log("请运行: pip install cython", "WARNING")
    ...
    def create_output_section(self, parent):
        ...
        default_output = os.path.join(os.path.expanduser("~"), "Desktop", "dist_pyd")
        ...
        default_build = os.path.join(os.path.expanduser("~"), "Desktop", "build_cython")
        self.build_path_var = tk.StringVar(value=default_build)
    ...
    def start_compilation(self):
        ...
        if not CYTHON_AVAILABLE:
            messagebox.showerror("错误", f"Cython 未安装，无法编译!\n\n错误信息: {CYTHON_ERROR}\n\n请运行: pip install cython")
            return
    ...
    def open_output_dir(self):
        ...
        if os.path.exists(output_dir):
            try:
                ...
                messagebox.showwarning("警告", f"无法打开目录:\n{output_dir}")
        else:
