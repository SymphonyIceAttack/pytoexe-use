...
Версия 6.3 (EXE-совместимая, без matplotlib, график заменён на таблицу динамики)
"""
...
# ------------------------------------------------------------
# Функция для корректного поиска файлов в EXE
...
class Database:
    ...
    def get_last_trend(self):
        """Возвращает изменение ВБР между последними двумя записями (в процентах) и стрелку"""
        self.cursor.execute("SELECT vbr_system FROM history ORDER BY id DESC LIMIT 2")
        rows = self.cursor.fetchall()
        if len(rows) < 2:
            return None, "—"
        prev, last = rows[1][0], rows[0][0]
        change = ((last - prev) / prev) * 100
        if change > 0:
            arrow = "▲ (+{:.1f}%)".format(change)
        elif change < 0:
            arrow = "▼ ({:.1f}%)".format(change)
        else:
            arrow = "▬ (0%)"
...
class SplashScreen:
    def __init__(self, root, callback, duration=2500):
        ...
        if os.path.exists(gif_path):
            try:
                image = tk.PhotoImage(file=gif_path)
                if image.width() > 400:
...
class UltimateApp:
    def __init__(self, root):
        ...
        self._create_history_tab()   # теперь таблица + анализ тренда
    ...
    def _create_history_tab(self):
        tab = ttk.Frame(self.notebook)
        self.notebook.add(tab, text="📜 ИСТОРИЯ + АНАЛИЗ")
        frame = ttk.Frame(tab, padding=10)
        frame.pack(fill=tk.BOTH, expand=True)
        ttk.Button(frame, text="Обновить", command=self._refresh_history).pack(anchor=tk.W, pady=5)
        # Таблица истории
        self.history_tree = ttk.Treeview(frame, columns=("date", "vbr", "category"), show="headings", height=10)
        ...
        # Панель анализа тренда
        self.trend_frame = ttk.LabelFrame(frame, text="Анализ динамики", padding=5)
        self.trend_frame.pack(fill=tk.X, pady=5)
        self.trend_label = ttk.Label(self.trend_frame, text="", font=("Segoe UI", 10))
    ...
    def _update_trend(self):
        change, arrow = self.db.get_last_trend()
        if change is None:
            self.trend_label.config(text="Недостаточно записей для анализа тренда (нужно минимум 2).")
        else:
            if change > 0:
                trend_text = f"📈 Тренд: {arrow} (надежность растёт)"
                self.trend_label.config(text=trend_text, foreground="green")
            elif change < 0:
                trend_text = f"📉 Тренд: {arrow} (надежность падает)"
                self.trend_label.config(text=trend_text, foreground="red")
            else:
                trend_text = f"➡️ Тренд: {arrow} (стабильно)"
                self.trend_label.config(text=trend_text, foreground="blue")
    ...
    def _about(self):
        msg = ("КСА 45Л6-1С – Оценка технического состояния\n"
               "Версия 6.3 (EXE-совместимая, график заменён на анализ тренда)\n"
               "Разработчик: Свирко В.А.\n"
               "Военная академия ВКО им. Г.К. Жукова, 2026\n\n"
               "Функции:\n"
               "- 4 готовых примера с корректными категориями\n"
               "- Расчёт надёжности, прогноз, история, тренд\n"
               "- Экспорт, копирование отчёта\n"
               "- Заставка с фото (splash.gif)\n\n"
               "График динамики намеренно заменён на текстовый анализ тренда для обеспечения 100% совместимости с EXE.")
