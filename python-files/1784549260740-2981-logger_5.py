...
def save_to_log(info, usb_devices, disk_results, log_file):
    ...
    # Формируем общую часть системы (она будет повторяться в каждой строке для удобства парсинга,
    # либо можно вынести отдельно. Здесь делаем по одной полной строке на запись)
    sys_info_base = (f"sys_timestamp={info['timestamp']};"
    ...
    with open(log_file, "a", encoding="utf-8") as f:
        ...
        # ==========================================
        # БЛОК 1: Информация о системе и USB-носителях
        # Формат: SysInfo; USB_Device_Data
        # ==========================================
        
        if not usb_devices:
            # Если USB нет, пишем заглушку
            f.write(f"{sys_info_base};usb_devices=none\n")
        else:
            for usb in usb_devices:
                usb_part = format_usb(usb)
                line = f"{sys_info_base};{usb_part}\n"
                f.write(line)

        # Добавляем разделитель между блоками (опционально, для читаемости человеком)
        f.write("---BLOCK_SEPARATOR---\n")

        # ==========================================
        # БЛОК 2: Результаты проверки папок на дисках
        # Формат: SysInfo; Disk_Data; Status; Paths
        # ==========================================
        
        if not disk_results:
            f.write(f"{sys_info_base};disk_report=none\n")
        else:
            for disk in disk_results:
                ...
                disk_part = (f"drive={disk['drive']};"
                             f"status={disk['status']};"
                             f"paths={paths_str}")
                
                line = f"{sys_info_base};{disk_part}\n"
                f.write(line)
