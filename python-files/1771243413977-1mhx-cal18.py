...
def toggle_minimal():
    ...
    widgets = [btns,res,history_btn,pin_btn,edit_btn,view_btn]
    if minimal_mode:
        for w in widgets:
            w.pack_forget()
    else:
        for w in widgets:
            w.pack(pady=5 if w != res else 10)
...
def change_font():
    global current_font
    fonts = ["Segoe UI","Arial","Consolas","Times New Roman"]
    f = simpledialog.askstring("Font","Enter font: "+", ".join(fonts),initialvalue=current_font)
    if f and f in fonts:
        current_font = f
        for w in [entry,left_lbl,middle_lbl,right_lbl]:
...
view_menu.add_command(label="Default Font",command=change_font)
