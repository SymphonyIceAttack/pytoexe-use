...
self.scan_btn = ttk.Button(self, text="Scan System", command=self.start_scan)
self.scan_btn.pack(pady=4)

self.clean_btn = ttk.Button(self, text="Clean Selected", command=self.start_clean)
self.clean_btn.pack(pady=4)
...
def scan():
results = {}
valid_paths = [(k, v) for k, v in PATHS.items() if v and os.path.exists(v)]

for i, (name, path) in enumerate(valid_paths):
if safe_path(path):
...
self.progress.set(int((i + 1) / len(valid_paths) * 100))
