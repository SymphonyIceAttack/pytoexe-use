...
class CodeRow(tk.Frame):
    ...
    def _on_numeric_apply(self):
        ...
        # check if this code is blocked by an active mutex partner
        conflicts = self.app.OFFSET_MUTEX.get(code_id, [])
        for other in conflicts:
            if self.app.applied.get(other, False):
                other_name = self.app.code_by_id.get(other, {}).get(
                    "name_en" if CURRENT_LANG == "en" else "name", other)
        ...
        if not self.app._is_unlocked(code_id):
            self.app.handle_toggle(code_id)
            return
    ...
    def refresh(self, applied, locked, detected):
        ...
        # for numeric codes: check mutex lock
        is_numeric = self.code.get("dialog") == "numeric_input"
        mutex_locked = False
        if is_numeric:
            for other in self.app.OFFSET_MUTEX.get(self.code["id"], []):
                if self.app.applied.get(other, False):
                    mutex_locked = True
                    break
