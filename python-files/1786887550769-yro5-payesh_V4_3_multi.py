...
_HTTP_HOST = urllib.parse.urlsplit(API_BASE).netloc
...
def http_get_json(url):
    """GET JSON while reusing one HTTPS connection per monitoring thread."""
    cache_buster = f"_payesh_ts={time.time_ns()}"
    parts = urllib.parse.urlsplit(url)
    query = parts.query
    query = f"{query}&{cache_buster}" if query else cache_buster
    ...
    try:
        ...
        # If a keep-alive connection was closed by the CDN/server,
        # discard it and make one clean reconnect attempt.
        try:
            conn.close()
        except Exception:
            pass
        _http_local.conn = http.client.HTTPSConnection(
            _HTTP_HOST,
            timeout=HTTP_TIMEOUT,
        )
        conn = _http_local.conn
        conn.request("GET", path, headers=headers)
        with conn.getresponse() as response:
            raw = response.read()
            status = response.status
    ...
    if status < 200 or status >= 300:
        raise RuntimeError(f"TSETMC HTTP {status}")

    text = raw.decode("utf-8-sig", errors="replace")
    data = json.loads(text)

    # Store the last network RTT for optional diagnostics/logging.
    _http_local.last_rtt_ms = elapsed_ms
    return data
...
class App:
    def __init__(self, root):
        ...
        # Multi-point click configuration.
        self.multi_count = tk.IntVar(value=1)
        self.multi_clicks = [
            {"x": None, "y": None, "delay_ms": 0, "delay_var": tk.StringVar(value="0")}
            for _ in range(4)
        ]
    ...
    def gentle_ding(self):
        """A short, soft Windows notification sound."""
        try:
    ...
    def build_ui(self):
        outer = ttk.Frame(self.root, padding=5)
        ...
        # ---------- Multi-point click controls ----------
        multi_frame = ttk.LabelFrame(outer, text="کلیک چندنقطه‌ای")
        multi_frame.pack(fill="x", pady=3)

        ttk.Label(multi_frame, text="تعداد نقاط:").grid(
            row=0, column=0, padx=4, pady=3, sticky="e"
        )
        ttk.Combobox(
            multi_frame,
            textvariable=self.multi_count,
            values=[1, 2, 3, 4],
            state="readonly",
            width=5,
        ...
        for i in range(4):
            ttk.Button(
                multi_frame,
                text=f"ثبت موقعیت {i + 1}",
                command=lambda j=i: self.register_multi_position(j),
            ).grid(row=1, column=i, padx=3, pady=3, sticky="ew")

            ttk.Label(
                multi_frame, text=f"Delay {i + 1} (ms)"
            ).grid(row=2, column=i, padx=3, pady=(1, 0))

            ttk.Entry(
                multi_frame,
                textvariable=self.multi_clicks[i]["delay_var"],
                width=8,
            ).grid(row=3, column=i, padx=3, pady=2)
        ...
        self.start_btn.grid(row=0, column=0, padx=6, pady=3, sticky="ew")

        self.stop_btn = ttk.Button(
            btns, text="■ توقف پایش", command=self.stop, state="disabled"
        )
        self.stop_btn.grid(row=0, column=1, padx=6, pady=3, sticky="ew")
        ...
        self.always_on_top_chk.grid(row=0, column=2, padx=6, pady=3, sticky="w")
    ...
    def register_multi_position(self, index):
        x, y = get_mouse_position()
        self.multi_clicks[index]["x"] = x
        self.multi_clicks[index]["y"] = y
        self.write_log(
            f"موقعیت {index + 1} ثبت شد: X={x}, Y={y}"
        )

    def get_active_multi_clicks(self):
        count = max(1, min(4, int(self.multi_count.get())))
        active = []

        for i in range(count):
            point = self.multi_clicks[i]
            if point["x"] is None or point["y"] is None:
                raise ValueError(f"موقعیت {i + 1} ثبت نشده است")

            try:
                delay = max(0, int(point["delay_var"].get().strip()))
            except Exception:
                raise ValueError(f"Delay نقطه {i + 1} معتبر نیست")
    ...
    def execute_multi_clicks(self):
        try:
            active = self.get_active_multi_clicks()
        except Exception as e:
            self.write_log(f"خطای تنظیم کلیک‌ها: {e}")
            return

        for i, point in enumerate(active):
            # Delay belongs to the click that follows it:
            # click 1 -> delay 2 -> click 2 -> delay 3 -> ...
            if i > 0:
                delay_ms = active[i]["delay_ms"]
                if delay_ms:
                    time.sleep(delay_ms / 1000.0)

            left_click(point["x"], point["y"])
            self.write_log(
                f"کلیک {i + 1}: X={point['x']}, Y={point['y']} | "
                f"Delay={point['delay_ms']}ms"
            )
            self.gentle_ding()
    ...
    def monitor_symbol(self, index, item):
        ...
        while self.running and not self.stop_event.is_set():
            try:
                ...
                # Log every successfully received raw TSETMC status.
                # This makes the live monitor observable without needing
                # Stop/Start to diagnose an IS -> IR transition.
                rtt_ms = getattr(_http_local, "last_rtt_ms", None)
                rtt_text = f" | RTT={rtt_ms:.0f}ms" if rtt_ms is not None else ""
                self.root.after(
                    0,
                    lambda n=name, c=code, r=rtt_text:
                    self.write_log(f"{n}: وضعیت خام = {c}{r}")
                )
                ...
                if code != previous:
                    ...
                    self.root.after(
                        0,
                        lambda n=name, old=previous, new=code:
                        self.write_log(
                            f"{n}: تغییر وضعیت خام {old} → {new}"
                        )
                    )
                    ...
                    if previous == TARGET_FROM and code == TARGET_TO:
                        ...
                        self.root.after(
                            ...
                            self.write_log(
                                f"*** {n}: وضعیت خام IS → IR در {ts} | اجرای دستور کلیک ***"
                            )
                        )
                        self.root.after(0, self.gentle_ding)

                        threading.Thread(
                            target=self.delayed_click_multi,
            ...
            next_time = time.perf_counter() + (self.poll_ms / 1000.0)
            self.stop_event.wait(max(0, next_time - time.perf_counter()))

    # ---------- Click sequence ----------

    def delayed_click_multi(self, symbol, detected_perf):
        # Keep the existing global trigger delay, then execute
        # the configured 1-4 point sequence.
        with self.click_lock:
            delay_s = self.delay_ms / 1000.0
            if delay_s > 0:
                time.sleep(delay_s)

            self.execute_multi_clicks()
