    ...
    if os.path.exists(SETTINGS_FILE):
        try:
            with open(SETTINGS_FILE, "r") as f:
                alpha, scale, dm, tm = f.readline().strip().split(",")
...
def edit_limits():
    global limits
    txt = simpledialog.askstring("Edit Limits", "Comma separated limits:", initialvalue=",".join(map(str, limits)))
    if txt:
        try:
...
def open_scale_popup():
    ...
    s = tk.Scale(popup, from_=5, to=200, orient="horizontal", command=apply_scale)
    s.set(current_scale*100)
    s.pack(fill="x", padx=10, pady=10)

# ===== TRANSPARENCY POPUP =====
def open_transp_popup():
    def apply_alpha(val):
    ...
    popup = tk.Toplevel(root)
    popup.title("Transparency (%)")
    popup.geometry("300x80")
    s = tk.Scale(popup, from_=10, to=100, orient="horizontal", command=apply_alpha)
    s.set(current_alpha*100)
    s.pack(fill="x", padx=10, pady=10)
...
def update_scale_ui():
    ...
    left_lbl.config(font=("Segoe UI", font_main), width=max(3,int(BASE_WIDTH_LR*current_scale)))
    middle_lbl.config(font=("Segoe UI", font_main), width=max(2,int(BASE_WIDTH_M*current_scale)))
    right_lbl.config(font=("Segoe UI", font_main), width=max(3,int(BASE_WIDTH_LR*current_scale)))
    for b in buttons:
...
view_menu.add_command(label="Transparency", command=open_transp_popup)
view_menu.add_command(label="Resize / Small", command=lambda: resize("small"))
view_menu.add_command(label="Resize / Medium", command=lambda: resize("medium"))
view_menu.add_command(label="Resize / Large", command=lambda: resize("large"))
...
left_lbl = tk.Label(res_frame, width=BASE_WIDTH_LR, font=("Segoe UI", BASE_FONT), bg="red", fg="white")
middle_lbl = tk.Label(res_frame, width=BASE_WIDTH_M, font=("Segoe UI", BASE_FONT), bg="white")
right_lbl = tk.Label(res_frame, width=BASE_WIDTH_LR, font=("Segoe UI", BASE_FONT), bg="green", fg="white")
