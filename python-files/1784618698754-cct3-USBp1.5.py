...
def run_powershell_command(script_block):
    ...
    try:
        ...
        if result.returncode != 0:
            return None
        # Очищаем вывод от лишних переносов и пробелов по краям
...
def get_usb_drives_info():
    ...
    Использует логику из предыдущих шагов.
    """
    ps_script = r"""
    ...
    # Разбиваем на строки, если их несколько
...
def check_android_folder(drive_letter):
    ...
    Возвращает статус-строку (✅ или ❌).
    """
    # Экранируем букву для безопасного использования в PS
    safe_letter = drive_letter.replace(':', ':\') # На самом деле просто подставляем
    ...
    if res and res.startswith("FOUND:"):
        path = res.split(":", 1)[1]
        return f"✅ Найдена папка: {path}"
    elif res and res.startswith("NOT_FOUND:"):
        return f"❌ На {drive_letter} папки Android нет"
    else:
        return "⚠️ Ошибка проверки папки"
...
def log_usb_devices():
    ...
    count = 0
    for dev in devices:
        # 2. Проверяем папку Android на этой букве
        status_msg = check_android_folder(dev["letter"])
        
        # 3. Формируем итоговую строку для лога
