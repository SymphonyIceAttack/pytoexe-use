...
class App(tk.Tk):
    ...
    def create_widgets(self):
        ...
        tk.Label(self, text="Enter real_app_id:").pack(pady=(10,0))
        self.id_entry = tk.Entry(self, width=20)
        self.id_entry.pack(pady=5)

        # Кнопка Patch — вот она!
        self.patch_btn = tk.Button(self, text="Patch", command=self.patch_game, state="disabled", bg="lightblue")
        self.patch_btn.pack(pady=15)

        self.log_text = tk.Text(self, height=5, width=60)
    ...
    def patch_game(self):
        ...
        if not en:
            ...
            self.log_text.insert("1.0", "Ошибка: не найден .exe файл игры в папке.")
            return

        pf = ["unsteam64.dll", "winhttp.dll", "winmm.dll"]
        patcher_dir = r"F:\dowland\SteamML_Online_Patcher\SteamML_Online_Patcher\PatcherFiles"
        missing = []
        for f in pf:
        ...
        if missing:
            self.log_text.delete("1.0", tk.END)
            self.log_text.insert("1.0", f"Ошибка: в папке PatcherFiles отсутствуют файлы: {', '.join(missing)}")
            return
