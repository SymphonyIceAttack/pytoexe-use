...
# ── quick EXE detection (no full scan needed) ─────────────────────────────────
_QUICK_DETECT = {
    "aev_osd": {
        "type": "find_replace",
        "find":    bytes.fromhex("81A0CC5200007FFFFFFF"),
        "replace": bytes.fromhex("E9892E000090909090"),
    },
    "aev_option": {
        "type": "find_replace",
        "find":    bytes.fromhex("E943BACEFFE9BF5B"),
        "replace": bytes.fromhex("E931FFFFFFE9BF5B"),
    },
}

def quick_detect_codes(exe_path):
    """
    Quickly check if specific codes are applied in the EXE
    by scanning for their replace bytes. Returns set of applied code IDs.
    """
    applied = set()
    ...
    try:
        with open(exe_path, "rb") as f:
            data = f.read()
        for code_id, info in _QUICK_DETECT.items():
            if info["replace"] in data:
                applied.add(code_id)
    except Exception:
        pass
    return applied
    ...
    def _reload_ui(self):
        ...
        if self.scanned:
            ...
            self.scan_status_var.set(
                ("Scanned -- " + str(n_det) + " codes detected") if CURRENT_LANG == "en"
                else ("تم المسح -- " + str(n_det) + " كود مكتشف")
            + orig_status)
...
class CodeManagerPanel(tk.Frame):
    ...
    def _build(self):
        ...
        try:
            ...
            # check if AEV codes already applied after scan
...
class AEVOptionPanel(tk.Frame):
    ...
    def _render(self, raw):
        ...
        while i <= len(tokens):
            ...
            if code_val in ("0X0000",):
                ...
                # end of an AEV option block — add separator line
                if last_was_0700:
                    self._text.insert("end", "\n" + chr(0x2502)*2 + "\n", ("separator",))
                    last_was_0700   = False
                    current_aev_tag = None
    ...
    def _show_aev_options(self, word, count):
        ...
        dlg.geometry("360x" + str(80 + count * 90))
        dlg.resizable(False, False)
        dlg.configure(bg="#111")
        dlg.grab_set()
        ...
        def do_apply():
            ...
            # build prefix codes
            prefix = ""
            for hide_var, idx_var in aev_configs:
        ...
        tk.Button(dlg, text="Apply" if CURRENT_LANG == "en" else "تطبيق",
                  font=("Courier New", 9, "bold"),
                  fg="#0a0a0a", bg="#7cfc7c",
                  activeforeground="#0a0a0a", activebackground="#5cdc5c",
                  relief="flat", bd=0, cursor="hand2",
                  highlightthickness=0,
                  command=do_apply, padx=14, pady=4
                  ).pack(pady=10)
...
AVL_ENTRY_BYTES = (
    "4E756D62657220 6F6620416576 2D2D XX "
    "4B657920 49445F 5F5F5F5F5F5F5F XX "
    "00 6C6F636B206D657373616765 2D2D2D XX "
    "6C6F636B20 736F756E64 2D2D2D2D2D XX "
    "6C6F636B2063616D657261 2D2D2D2D XX "
    "756E6C6F636B206D657373616765 2D XX "
    "756E6C6F636B20736F756E64 2D2D2D XX "
...
ITEM_LIST = [
    ...
    ("09", "Brown Chicken Egg"), ("0A", "Gold Chicken Egg"), ("0C", "Plaga Sample"),
    ("0D", "Krauser's Knife"), ("0E", "Flash Grenade"), ("0F", "Salazar Family Insignia"),
    ("10", "Bowgun"), ("11", "Bowgun Bolts"), ("17", "Rocket Launcher (Special)"),
    ("18", "Shotgun Shells"), ("19", "Red Herb"), ("1C", "Yellow Herb"),
    ...
    ("29", "Broken Butterfly"), ("2A", "Killer7"), ("2C", "Shotgun"),
    ("2D", "Striker"), ("2E", "Rifle"), ("2F", "Rifle (Semi-Auto)"),
    ("30", "TMP"), ("32", "TMP w/ Stock"), ("34", "Chicago Typewriter (Infinite)"),
    ("35", "Rocket Launcher"), ("36", "Mine Thrower"), ("37", "Handcannon"),
    ("38", "Combat Knife"), ("41", "P.R.L. 412"), ("47", "Shotgun"),
    ("51", "Rifle (Semi-Auto) w/ Infrared Scope"), ("52", "Krauser's Bow"),
    ("53", "Chicago Typewriter"), ("56", "Velvet Blue"), ("57", "Spinel"),
    ("58", "Pearl Pendant"), ("59", "Brass Pocket Watch"), ("5A", "Elegant Headdress"),
    ("5B", "Antique Pipe"), ("94", "Riot Gun"), ("9A", "Crown"),
    ("A1", "Emerald"), ("B8", "Butterfly Lamp"), ("D1", "Golden Lynx"),
    ("FE", "Tactical Vest"),
]
...
class AVLEditorPanel(tk.Frame):
    ...
    def _browse(self):
        p = filedialog.askopenfilename(
            ...
            filetypes=[("All files", "*.*"), ("AVL files", "*.avl")]
        )
        if p:
    ...
    def _render_entries(self):
        ...
        for n, entry in enumerate(self._entries):
            ...
            for i, (lbl_en, lbl_ar) in enumerate(field_labels):
                ...
                if lbl_en == "Key ID":
                    # dropdown for item list
                    item_labels = [f"{hx} - {nm}" for hx, nm in ITEM_LIST]
                    item_vals   = [hx for hx, nm in ITEM_LIST]
                    # find current
                    current_lbl = next(
                        (f"{hx} - {nm}" for hx, nm in ITEM_LIST if int(hx, 16) == val),
                        f"{hex_val} - Unknown"
                    )
                    var = tk.StringVar(value=current_lbl)
                    cb = ttk.Combobox(grid, textvariable=var,
                                      values=item_labels, state="readonly",
                                      font=("Courier New", 8), width=28)
                    cb.grid(row=row, column=1, padx=(0, 8), pady=3)
                    row_vars[i] = ("combo", var, item_vals, item_labels)
                else:
    ...
    def _save(self):
        ...
        # find all entry positions fresh
        entry_positions = []
        pos = 0
        while True:
            idx = data.find(header, pos)
            if idx < 0:
                break
            entry_positions.append(idx)
            pos = idx + 16

        # handle new entries (append)
        entry_template = bytes.fromhex(AVL_ENTRY_BYTES)

        for n, (row_vars, entry) in enumerate(
                zip(self._entry_vars, self._entries)):
            vals = {}
            for i, field_info in row_vars.items():
                kind, var, item_vals, item_labels = field_info
                if kind == "combo":
                    sel = var.get()
                    try:
                        idx2 = item_labels.index(sel)
                        vals[i] = int(item_vals[idx2], 16)
                    except Exception:
                        vals[i] = 0
                else:
                    try:
                        vals[i] = int(var.get().strip(), 16)
                    except Exception:
                        vals[i] = 0

            if n < len(entry_positions):
                # update existing
                start = entry_positions[n]
                for i, off in enumerate(offsets):
                    data[start + off] = vals.get(i, 0)
            else:
                # append new entry
                new_bytes = bytearray(entry_template)
                for i, off in enumerate(offsets):
                    new_bytes[off] = vals.get(i, 0)
                # update count byte (first XX = entry count)
                new_bytes[15] = len(self._entries)
                data.extend(new_bytes)

        # update count byte in all entries
        ...
        try:
            with open(path, "wb") as f:
                f.write(data)
            messagebox.showinfo("[+] Saved", f"AVL file saved: {os.path.basename(path)}")
        except Exception as e:
            messagebox.showerror("Error", str(e))
...
TOOLS_DEF = [
    ...
    {"id": "osd_editor",      "label": "OSD EDITOR",        "lock": ("aev_osd",    "AEV-OSD")},
    {"id": "cns_editor",      "label": "CNS EDITOR",        "lock": None},
    {"id": "snd_editor",      "label": "SND EDITOR",        "lock": None},
    {"id": "aev_editor",      "label": "AEV OPTION EDITOR", "lock": ("aev_option", "AEV OPTION"), "panel": "aev"},
    {"id": "mdt_color_editor","label": "MDT COLOR EDITOR",  "lock": None},
    {"id": "room_init_editor","label": "ROOM INIT EDITOR",  "lock": None},
    {"id": "avl_editor",      "label": "AVL EDITOR",        "lock": None,                                "panel": "avl"},
]
...
class RE4MasterEditor(tk.Tk):
    ...
    def __init__(self):
        ...
        self._panels   = {}
        self._nav_btns = {}
        self._cm_app   = None  # set by CodeManagerPanel
    ...
    def _build_ui(self):
        ...
        for tool in TOOLS_DEF:
            ...
            if tid == "code_manager":
                ...
                if lock:
                    panel = LockedPanel(self._content, label,
                                        lock[0], lock[1], self,
                                        unlocked_panel=AEVOptionPanel(self._content, self))
                else:
    ...
    def _quick_unlock(self, exe_path):
        """Unlock locked panels by reading EXE directly — no full scan needed."""
        applied = quick_detect_codes(exe_path)
        for tool in TOOLS_DEF:
            lock = tool.get("lock")
            if not lock:
                continue
            code_id = lock[0]
            panel   = self._panels.get(tool["id"])
            if isinstance(panel, LockedPanel):
                panel._unlocked_by_quick = code_id in applied
                panel._check_lock()

    def _check_locked_panels(self):
        """Unlock AEV/OSD panels if codes are already applied (after full scan)."""
        cm = self._cm_app
        if not cm:
            return
        for tool in TOOLS_DEF:
            ...
            if not lock:
                continue
            code_id = lock[0]
            if cm.applied.get(code_id) or cm.detected.get(code_id):
                panel = self._panels.get(tool["id"])
                if isinstance(panel, LockedPanel):
                    panel._check_lock()
    ...
    def _on_exe_change(self, *_):
        ...
        if path and os.path.isfile(path):
            ...
            # quick detect — unlock panels without needing full scan
            self.after(150, lambda: self._quick_unlock(path))
    ...
    def _open_settings(self):
        ...
        def apply_lang():
            global CURRENT_LANG
            MASTER_SETTINGS["lang"] = lang_var.get()
            APP_SETTINGS["lang"]    = lang_var.get()
            save_master_settings(MASTER_SETTINGS)
            save_settings(APP_SETTINGS)
            ...
            messagebox.showinfo(
                ...
                "Restart the app to apply language fully."
                if CURRENT_LANG == "en" else
                "أعد تشغيل البرنامج لتطبيق اللغة كاملاً."
            )
