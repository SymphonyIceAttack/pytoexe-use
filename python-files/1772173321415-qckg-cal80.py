...
def start_timer1(delay):
    ...
    # البدء بالعد التنازلي من 60 ثانية
...
def run_countdown1(sec, delay):
    ...
    if sec < 0:
        stop_timer1()
        # إذا انتهى التايمر الأساسي وكانت الإضافية مفعلة، يمكن بدء التتابع هنا
        return
    
    m, s = divmod(sec, 60)
    timer1_lbl.config(text=f"Timer 1: {m:02d}:{s:02d}")
    wait = int(delay * 1000) if (sec == 60 and delay != 0) else 1000
    t1_after_id = root.after(wait, lambda: run_countdown1(sec-1, 0))
...
def start_extra_sequence():
    ...
    run_extra_countdown(60, 1) # ابدأ بـ Extra 1

def run_extra_countdown(sec, sequence_num):
    global t_extra_after_id
    if sec < 0:
        if sequence_num == 1:
            # فور انتهاء الأول، ابدأ الثاني تلقائياً
    ...
    m, s = divmod(sec, 60)
    color = "darkgreen" if sequence_num == 1 else "purple"
    lbl_text = f"Extra {sequence_num}: {m:02d}:{s:02d}"
    
    timer_extra_lbl.config(text=lbl_text, fg=color)
...
def toggle_extra():
    ...
    st = "ON" if extra_timers_enabled else "OFF"
    messagebox.showinfo("Extra", f"Extra Timers: {st}")
...
def toggle_mode():
    ...
    for lbl in [left_lbl, middle_lbl, right_lbl]: lbl.config(fg="black")
