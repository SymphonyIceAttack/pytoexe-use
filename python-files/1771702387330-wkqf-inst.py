...
class TrapModsInstaller:
    ...
    def setup_ui(self):
        # Логотип из онлайн-источника
        url = "https://drive.google.com/uc?export=view&id=1ixB5TNbPbMW1n5CFCxKOOapB9mivtRre"
        image_bytes = urlopen(url).read()
        self.logo_img_orig = Image.open(io.BytesIO(image_bytes)).resize((300,150))
        ...
        self.btn_install = tk.Button(self.root, text="УСТАНОВИТЬ", width=25, command=self.install_mods)
    ...
    def animate_logo(self):
        ...
        for scale in range(50, 101, 5):
            ...
            img_resized = self.logo_img_orig.resize(size)
    ...
    def install_mods(self):
        ...
        if self.check_minimap.get(): self.copy_file("Миникарта", 15)
        if self.check_bigmap.get(): self.copy_file("Большая карта", 30)
        if self.check_crosshair.get(): self.copy_file("Прицел", 45)
        if self.check_timecycle.get(): self.copy_file("Timecycle", 60)
        if self.check_frontend.get(): self.copy_file("Интерфейс", 80)
        if self.check_effects.get(): self.copy_file("Эффекты", 95)

        self.progress["value"] = 100
        self.label_status["text"] = "Установка завершена!"
        messagebox.showinfo("TRAP MODS", "Установка завершена!\n\nTRAP MODS")

    def copy_file(self, mod_name, progress_value):
