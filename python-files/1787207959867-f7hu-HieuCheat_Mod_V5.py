# -*- coding: utf-8 -*-
# File: snow_mod_tool.py
# Chức năng: Giao diện style SNOWCHEATS - Mod đồ họa Free Fire + Hiển thị cấu hình máy & giả lập
# Đóng gói exe: pyinstaller --onefile --windowed --add-data "bg.jpg;." snow_mod_tool.py
...
KEY = "hieumod1"
VERSION = "5.1"
TIEU_DE = "SNOWCHEATS - FF MOD TOOL"
...
def detect_emulator():
    """
    Phát hiện giả lập đang chạy dựa trên tiến trình.
    Trả về tên giả lập hoặc "Không xác định".
    """
    emulators = {
    ...
    try:
        for name, procs in emulators.items():
            for p in procs:
                if os.system(f"tasklist /FI \"IMAGENAME eq {p}\" 2>NUL | find /I \"{p}\" >NUL") == 0:
                    return name
    except:
        pass
...
class SnowCheatGUI:
    ...
    def create_title_bar(self):
        ...
        tk.Label(self.title_bar, text="❄ SNOWCHEATS", font=("Arial", 11, "bold"), fg="#00d4ff", bg="#1a1a2e").pack(side="left", padx=15)
    ...
    def create_login(self):
        ...
        tk.Label(self.login_frame, text="❄ SNOWCHEATS", font=("Arial", 18, "bold"), fg="#00d4ff", bg="#0f0f1a").pack(pady=(20,0))
    ...
    # ==================== MAIN (Style SNOWCHEATS + Thông tin máy) ====================
    def create_main(self):
        ...
        tk.Label(h, text="❄ SNOWCHEATS", font=("Arial", 14, "bold"), fg="#00d4ff", bg="#1a1a2e").pack(side="left", padx=15)
        tk.Label(h, text="v5.1", font=("Arial", 9), fg="#ffd93d", bg="#1a1a2e").pack(side="left", padx=5)
        
        # ===== THÔNG TIN MÁY VÀ GIẢ LẬP =====
        info_frame = tk.LabelFrame(self.main_frame, text="🖥️ SYSTEM INFO", 
                                  font=("Arial", 9, "bold"), fg="#00d4ff",
                                  bg="#12122a", padx=8, pady=5, bd=2, relief="ridge")
...
📱 Giả lập: {self.emulator}"""
        ...
        info_label = tk.Label(info_frame, text=info_text, 
                             font=("Courier", 8), fg="#aaa", bg="#12122a", 
        ...
        # Trạng thái
        self.status_label = tk.Label(self.main_frame, text="💡 Chọn chế độ rồi bấm ÁP DỤNG", 
                                    font=("Arial", 9), fg="#aaa", bg="#0a0a12")
    ...
    def verify(self):
        if self.key_entry.get().strip() == KEY:
    ...
    def apply_graphics(self):
        ...
        try:
            with open(self.config_path, 'r', encoding='utf-8') as f:
                c = f.read()
            vals = {"resolution_scale": "0.08", "lod_distance": "0.02", "texture_quality": "0", "shadow_quality": "0", "graphics_quality": "0"}
            ...
            for k, v in vals.items():
                c = re.sub(f'<.*? name="{k}".*?>.*?</.*?>', f'<string name="{k}">{v}</string>', c)
...
if __name__ == "__main__":
    try:
        ...
        print("Cài: pip install psutil pillow")
        sys.exit()
    root = tk.Tk()
    app = SnowCheatGUI(root)
    root.mainloop()