...
class RE4PatcherApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # search results frame (hidden by default)
    ...
    def handle_toggle(self, code_id):
        ...
        if self.applied.get(code_id, False):
            ...
            needs_orig = any(p["type"] in ("offset_paste", "offset_replace")
    ...
    # ── mutual exclusion: dll codes ───────────────────────────────────────────
    # apply_dll_qingsheng and apply_dll_raz0r share offset 156 — enabling one
    # disables the other.
    DLL_MUTEX = {
    ...
    def _handle_dll_mutex(self, code_id):
        """If code_id is a dll mutex pair, revert the other one silently."""
        other = self.DLL_MUTEX.get(code_id)
        if not other:
            return
