...
class TypingTest:
    ...
    def update_errors_display(self):
        ...
        self.errors_label.config(text=f"Ошибки: {total_errors}")
    ...
    def check_text(self, event=None):
        ...
        # Обновляем счетчик ошибок с учетом исправлений
        previous_error_counts = self.error_counts.copy()
        self.error_counts = defaultdict(int)
        correct_letters = 0
        ...
        for i in range(min_length):
            ...
            if typed_text[i] == original_text[i]:
                ...
                correct_letters += 1
                # Если раньше была ошибка, уменьшаем счетчик
                if previous_error_counts.get(original_text[i], 0) > 0:
                    self.error_counts[original_text[i]] = previous_error_counts[original_text[i]] - 1
            else:
        ...
        if self.correct_letters == self.total_letters:
            ...
            messagebox.showinfo("Тест завершен", 
                                ...
                                f"Общее число ошибок: {total_errors}")
            self.test_started = False
