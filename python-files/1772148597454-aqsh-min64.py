...
def update_timer_logic():
    ...
    if 0 < diff <= 30:
        timer_label.config(text=f"PREPARE: {diff}s", fg=THEMES[curr_theme]["acc"])
    elif -10 <= diff <= 0:
...
def open_theme_picker():
    ...
    pop.title("Themes")
    pop.grab_set()
    tk.Label(pop, text="Change Design", pady=5).pack()
    for tn in THEMES.keys():
        tk.Button(pop, text=tn, width=15, command=lambda n=tn: [apply_theme(n), pop.destroy()]).pack(pady=3)
...
def open_scale():
    ...
    def change(v):
        ...
        # جعل خط البطاقات أصغر قليلاً للحفاظ على التنسيق الأفقي
        for c in [card1, card2, card3]: c.config(font=("Segoe UI", max(10, s-2), "bold"))
        timer_label.config(font=("Segoe UI", int(s*0.9), "bold"))
        root.geometry(f"{int(480 * (s/14))}x{int(350 * (s/14))}") # تصغير طول النافذة
    tk.Scale(win, from_=10, to=28, orient="horizontal", command=change).pack(padx=20, pady=20)

# ------------------- Main UI -------------------
root = tk.Tk()
root.title("Trade Companion")
root.geometry("480x350") # طول ملموم ومناسب
root.attributes("-topmost", True)

top_f = tk.Frame(root, pady=10)
top_f.pack()
entry = tk.Entry(top_f, justify="center", font=("Segoe UI", 20), width=10, relief="flat", bd=2)
entry.pack()
entry.insert(0, "0")
entry.bind("<Return>", on_generate) # مانيوال بـ Enter

ctrl_f = tk.Frame(root, pady=5)
ctrl_f.pack()
btn_gen = tk.Button(ctrl_f, text="Generate", command=on_generate, relief="flat", padx=10)
btn_gen.pack(side="left", padx=3)
btn_clr = tk.Button(ctrl_f, text="Clear", command=lambda: [c.config(text="") for c in [card1, card2, card3]], relief="flat")
btn_clr.pack(side="left", padx=3)
btn_pre = tk.Button(ctrl_f, text="◀", command=lambda: display_group(current_group_idx-1), relief="flat")
btn_pre.pack(side="left", padx=3)
btn_nxt = tk.Button(ctrl_f, text="▶", command=lambda: display_group(current_group_idx+1), relief="flat")
btn_nxt.pack(side="left", padx=3)

ai_canvas = tk.Canvas(ctrl_f, width=16, height=16, bd=0, highlightthickness=0)
ai_canvas.pack(side="left", padx=5)
def draw_ai_lamp():
    ai_canvas.delete("all")
    ai_canvas.config(bg=THEMES[curr_theme]["bg"])
    ai_canvas.create_oval(2, 2, 14, 14, fill="green" if ai_enabled else "red", outline=THEMES[curr_theme]["fg"])

timer_label = tk.Label(root, text="", font=("Segoe UI", 13, "bold"))
timer_label.pack(pady=2)

# --- منطقة العرض الجديدة (جنباً إلى جنب) ---
display_f = tk.Frame(root, pady=10)
display_f.pack(expand=True, fill="x", padx=15)

# استخدام grid لترتيب البطاقات أفقياً
card1 = tk.Label(display_f, text="", font=("Segoe UI", 12, "bold"), pady=8, width=15, relief="groove", bd=1)
card1.grid(row=0, column=0, padx=2, sticky="ew")
card2 = tk.Label(display_f, text="", font=("Segoe UI", 12, "bold"), pady=8, width=15, relief="groove", bd=1)
card2.grid(row=0, column=1, padx=2, sticky="ew")
card3 = tk.Label(display_f, text="", font=("Segoe UI", 12, "bold"), pady=8, width=15, relief="groove", bd=1)
card3.grid(row=0, column=2, padx=2, sticky="ew")

# جعل الأعمدة متساوية في التوزيع
...
view_m.add_command(label="Change UI Style", command=open_theme_picker)
view_m.add_command(label="Scale Adjust", command=open_scale)
menubar.add_cascade(label="View", menu=view_m)

ai_m = tk.Menu(menubar, tearoff=0)
ai_m.add_command(label="AI Toggle", command=toggle_ai)
ai_m.add_command(label="Timer View", command=lambda: [globals().update(timer_visible=not timer_visible)])
