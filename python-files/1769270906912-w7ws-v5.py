...
class StockApp:
    def __init__(self, root):
        ...
        self.create_ui()
        self.load_existing_data()  # Load stock.csv if exists
...
"""
    ...
    def import_csv(self):
        ...
        self.save_csv()  # save imported data
    ...
    def scan_barcode(self, event):
        ...
        if barcode in self.data:
            ...
            self.refresh_table()
            self.save_csv()  # save immediately
        else:
    ...
    # ---------------- SAVE/LOAD CSV ----------------
    def save_csv(self):
        path = resource_path(DATA_FILE)
        with open(path, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["SN","BARCODE","ITEM","ORIGINAL","SCANNED"])
            for b, v in self.data.items():
                writer.writerow([v["SN"], b, v["ITEM"], v["ORIGINAL"], self.scanned.get(b,0)])
        ...
        if os.path.exists(path):
            with open(path, newline="", encoding="utf-8") as f:
                ...
                for row in reader:
                    ...
                    self.scanned[barcode] = int(row.get("SCANNED", 0))
            self.refresh_table()
