...
def toggle_mode():
    ...
    for lbl in [left_lbl,middle_lbl,right_lbl]:
        lbl.config(bg=lbl.cget("bg"),fg=fg)
...
def apply_scale(val):
    ...
    for w in [entry,left_lbl,middle_lbl,right_lbl,calc_btn,clear_btn,prev_btn,next_btn]:
...
middle_lbl = tk.Label(res,width=4,bg="white")
right_lbl = tk.Label(res,width=6,bg="green",fg="white")
# لصقهم جنب بعض بدون مسافات
