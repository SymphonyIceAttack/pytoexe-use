...
loss_prep_lbl = tk.Label(root, font=(current_font,int(BASE_FONT*0.75)), fg="darkorange")
loss_main_lbl = tk.Label(root, font=(current_font,int(BASE_FONT*0.75)), fg="purple")
...
def start_loss_prep(skip_target):
    ...
    loss_prep_lbl.config(text="Prep: 01:00")
...
def _loss_prep_countdown(sec, skip_target):
    ...
    if sec <=0:
        ...
        return
    m,s = divmod(sec,60)
    loss_prep_lbl.config(text=f"Prep: {m:02d}:{s:02d}")
...
def start_loss_main(skip_target):
    ...
    # تحديث الثلاث مربعات أثناء العد
    left = skip_target - 3
    mid = skip_target
    right = skip_target + 3
...
def calculate(event=None):
    ...
    if ai_mode and v in AI_NEXT_VALUES:
        ...
        middle_lbl.config(text=f"{m} {sign}", bg=night_bg if not dark_mode else night_bg)
...
def open_alpha_popup():
    ...
    s=tk.Scale(p,from_=0.3,to=1.0,resolution=0.05,orient="horizontal",command=lambda v:root.attributes("-alpha",float(v)))
    s.set(root.attributes("-alpha"))
    s.pack()

def open_font_popup():
    fonts = ["Segoe UI","Arial","Consolas","Times New Roman","Courier New","Verdana","Tahoma","Impact","Georgia","Comic Sans MS"]
    p=tk.Toplevel(root)
...
file_menu.add_command(label="Edit Limits",command=lambda: simpledialog.askstring("Edit Limits","Comma separated:",initialvalue=",".join(map(str,limits))))
