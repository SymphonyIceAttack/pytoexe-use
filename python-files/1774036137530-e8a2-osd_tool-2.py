...
def parse_osd_file(raw):
    ...
    all_blocks = []
    first_block = True   # علم للبلوك الأول فقط

    for i, (t_start, t_end, tb) in enumerate(true_blocks):
        # البلوك الأول: يظهر حتى لو فاضي (AEV=0, items=0) كـ FALSE
        # الباقي: الفاضية تسكب
...
def do_extract(osd_path):
    ...
    # لو ما لقى MAGIC (ملف أصفار خام) → بلوك FALSE فاضي واحد
    if not blocks:
        blocks = [make_block(False, 0, [], [], 0, [], [])]
        print("  [!] ما لقيت بلوكات - الملف أصفار، سيتم إنشاء بلوك فاضي واحد")
...
def do_repack_inplace(osd_path):
    ...
    # لو الملف كله أصفار (ما فيه MAGIC) → اكتب البلوكات من الأوفست 0
    if not orig_blocks and raw_blocks:
        pos = 0
        for i in range(count):
            new_bytes = block_to_bytes(txt_to_block(raw_blocks[i]))
            new_sz    = len(new_bytes)
            if pos + new_sz <= len(raw):
                raw[pos : pos + new_sz] = new_bytes
                pos += new_sz
            else:
                # الملف ما يكفي → زِد
                raw[pos:] = bytearray(new_bytes[:len(raw)-pos])
                raw += bytearray(new_bytes[len(raw)-pos:])
                pos = len(raw)
        if has_footer: raw += bytearray(FOOTER)
        with open(osd_path, "wb") as f: f.write(raw)
        print(f"\n  [OK] تم الكتابة في الملف الأصلي (كان أصفار)")
        print(f"  الملف    : {osd_path}")
        print(f"  البلوكات : {count} مكتوبة")
        return True
    ...
    # ── بلوكات جديدة زيادة عن الأصل ──
    added = 0
    if count > len(orig_blocks):
        # نهاية آخر بلوك أصلي = موضع الكتابة
        last  = orig_blocks[-1]
        pos   = last["file_offset"] + last["file_size"] + size_added

        for i in range(len(orig_blocks), count):
            new_bytes = block_to_bytes(txt_to_block(raw_blocks[i]))
            new_sz    = len(new_bytes)
            if pos + new_sz <= len(raw):
                raw[pos : pos + new_sz] = new_bytes
                ...
                # الأصفار ما كفت → زِد الملف
                fill = max(0, len(raw) - pos)
                raw[pos:] = bytearray(new_bytes[:fill])
                raw += bytearray(new_bytes[fill:])
                pos = len(raw)
            added += 1
    ...
    print(f"  الملف    : {osd_path}")
    print(f"  محدثة    : {written} بلوك")
    if added:
        print(f"  مضافة    : {added} بلوك جديد في النهاية")
    if size_added:
