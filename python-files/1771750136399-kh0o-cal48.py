...
def calculate(event=None):
    ...
    if ai_mode and v in AI_SPECIALS:
        scaled_size = int(BASE_FONT * current_scale * 0.8)
        middle_lbl.config(text=f"{v} <", font=(current_font, scaled_size), bg="#ADD8E6")
        middle_next_lbl.config(text="next", font=(current_font, int(scaled_size*0.7)), bg="#ADD8E6")
    else:
...
middle_lbl = tk.Label(res, bg="white", width=6, justify="center")
middle_next_lbl = tk.Label(res, bg="white", justify="center")
right_lbl = tk.Label(res,width=6,bg="green",fg="white")

left_lbl.pack(side="left")
middle_lbl.pack()
middle_next_lbl.pack(pady=1)
