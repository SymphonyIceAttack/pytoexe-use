...
def parse_time_str(s):
    """حول 'HH:MM:SS' أو 'HH:MM:SS text' لـ datetime.time"""
    try:
...
def find_group_by_minute(minute=None):
    """مقارنة الوقت الحالي بالدقيقة والثانية فقط مع كل مجموعة في trade_table"""
    now = datetime.now()
    current_min_sec = (now.minute, now.second)
    for i, g in enumerate(trade_table):
        t_start = parse_time_str(g[0])
        t_end   = parse_time_str(g[2])
        if t_start and t_end:
            start_min_sec = (t_start.minute, t_start.second)
            end_min_sec   = (t_end.minute, t_end.second)
            if start_min_sec <= current_min_sec <= end_min_sec:
                return i
    return None
