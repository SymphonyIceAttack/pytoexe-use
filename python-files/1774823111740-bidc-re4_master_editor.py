...
class SpeechColorPanel(tk.Frame):
    ...
    def _on_char_mode_change(self):
        pass  # mode changes hover behavior dynamically
...
class AEVOptionPanel(tk.Frame):
    ...
    def _build(self):
        ...
        self._text.tag_configure("aev_boxes", foreground="#ffffff",
                                  font=("Courier New", 7))
        self._text.tag_configure("newpage",   foreground="#c8a035", font=("Courier New", 8))
        self._text.tag_configure("hover",     background="#2a2000")
    ...
    def _render(self, raw):
        ...
        last_was_0700   = False
        aev_tag_buffer  = []  # stores tags for current word's AEV options
            ...
            if end_pos > pos:
                ...
                if chunk:
                    if len(aev_tag_buffer) > 1:
                        # multiple AEVs → colored boxes + white text
                        box_str = "".join("■" if t == "aev_hide" else "□" for t in aev_tag_buffer)
                        self._text.insert("end", box_str + " ", ("aev_boxes",))
