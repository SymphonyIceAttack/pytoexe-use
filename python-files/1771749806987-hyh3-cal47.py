...
def calculate(event=None):
    ...
    middle_next_lbl.config(text="", bg=middle_bg)

    # AI-specific display
    if ai_mode and v in AI_SPECIALS:
        scaled_size = int(BASE_FONT * current_scale * 1.5 * 0.8)
        middle_lbl.config(text=f"{v} <", font=(current_font, scaled_size), bg="#ADD8E6")
        middle_next_lbl.config(text="next", font=(current_font, int(scaled_size*0.7)), bg="#ADD8E6", pady=1)
    else:
        middle_lbl.config(text=f"{m} {'=' if v==m else '<' if v<m else '>'}", font=(current_font,int(BASE_FONT*current_scale*1.05)), bg=middle_bg)

    # Manual special middles
    if not (ai_mode and v in AI_SPECIALS) and v == m and v in SPECIAL_MIDDLES:
...
def toggle_mode():
    ...
    middle_next_lbl.config(bg="white")
    draw_ai_lamp()
...
def apply_scale(val):
    ...
    for w in [left_lbl,middle_lbl,middle_next_lbl,right_lbl]:
...
middle_frame = tk.Frame(res, width=60, height=60, bg="white")  # Frame للرقم و next
middle_lbl = tk.Label(middle_frame, bg="white", justify="center")  # الرقم وعلامة
middle_next_lbl = tk.Label(middle_frame, bg="white", justify="center")  # next
right_lbl = tk.Label(res,width=6,bg="green",fg="white")

left_lbl.pack(side="left")
middle_frame.pack(side="left")
middle_lbl.pack()
middle_next_lbl.pack(pady=1)  # ضيق المسافة الرأسية
