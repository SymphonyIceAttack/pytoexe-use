...
import sys
import time
import numpy as np  # для еще большей скорости (можно установить: pip install numpy)

def clear_screen():
    os.system('cls' if os.name == 'nt' else 'clear')

def find_solutions_with_numpy(a, k, tolerance=1.0):
    """
    Супер-быстрый поиск с использованием NumPy
    (если установлен numpy)
    """
    try:
        import numpy as np
        ...
        print(f"\n🔍 NUMPY-ускоренный поиск для a={a}, k={k}...")
        start_time = time.time()
        
        # Создаем все возможные комбинации f, z, y
        f_values = np.arange(256)
        ...
        for f in f_values:
            for z in z_values:
                ...
                # Векторизованная операция для всех y сразу
                base_array = base_fz + y_values
                
                # Вычисляем x для всех y
                x_array = (target_value - base_array) * 100
                x_ints = np.round(x_array).astype(int)
                
                # Фильтруем допустимые x
                mask = (x_ints >= 0) & (x_ints <= 255)
                
                for idx in np.where(mask)[0]:
                    y = y_values[idx]
                    x = x_ints[idx]
                    
                    expr = base_array[idx] + x * 0.01
                    result = expr * k_float
        ...
        print(f"✅ Поиск завершен за {elapsed_time:.3f} сек.")
        
        solutions.sort(key=lambda s: s[5])
        return solutions
        
    except ImportError:
        print("⚠️  NumPy не установлен. Используйте Optimized V2 метод.")
        return find_solutions_optimized_v2(a, k, tolerance)
...
def main():
    while True:
        ...
        try:
            ...
            print("   1. NumPy-ускоренный (если есть numpy) - 0.5-1 секунда")
            print("   2. Оптимизированный V2 - 5-10 секунд")
            print("   3. Проверить конкретные значения")
            
            method = input("Ваш выбор (1/2/3) [по умолчанию 1]: ").strip() or "1"
            ...
            if method == "3":
                ...
                if method == "1":
                    search_func = find_solutions_with_numpy
                else:  # method == "2"
                    search_func = find_solutions_optimized_v2
                
                # Параллельный поиск для обоих тарифов
                print(f"\n🔄 Выполняется параллельный расчет для обоих тарифов...")
                
                # Поиск для дневного тарифа
                solutions_day = search_func(a, k, tolerance)
                
                # Поиск для ночного тарифа
                solutions_night = search_func(a1, k, tolerance)
