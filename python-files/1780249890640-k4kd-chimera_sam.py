import hashlib
import secrets
import base64
import lzma
import struct
import os
import sys


class QShadowV7:

    # =========================
    # KEY STREAM
    # =========================

    def _keystream_byte(self, key: int, pos: int) -> int:
        h = hashlib.sha256(f"{key}:{pos}".encode()).digest()
        return h[pos % len(h)]

    # =========================
    # COMPRESSION
    # =========================

    def _compress(self, data: bytes) -> bytes:
        return lzma.compress(
            data,
            format=lzma.FORMAT_XZ,
            preset=9 | lzma.PRESET_EXTREME
        )

    def _decompress(self, data: bytes) -> bytes:
        return lzma.decompress(data)

    # =========================
    # ENCODING
    # =========================

    def _encode(self, data: bytes) -> str:
        return base64.b64encode(data).decode()

    def _decode(self, data: str) -> bytes:
        return base64.b64decode(data.encode())

    # =========================
    # TEXT (MULTILINE SUPPORT)
    # =========================

    def encrypt_text(self, text: str) -> str:
        key = secrets.randbits(64)
        compressed = self._compress(text.encode("utf-8"))

        enc = bytearray()
        for i, b in enumerate(compressed):
            enc.append(b ^ self._keystream_byte(key, i))

        return f"{key}:{self._encode(bytes(enc))}"

    def decrypt_text(self, container: str) -> str:
        key_str, payload = container.split(":", 1)
        key = int(key_str)

        data = self._decode(payload)

        dec = bytearray()
        for i, b in enumerate(data):
            dec.append(b ^ self._keystream_byte(key, i))

        return self._decompress(bytes(dec)).decode("utf-8")

    # =========================
    # FILES (HIDDEN METADATA)
    # =========================

    def encrypt_file(self, src: str, dst: str):
        with open(src, "rb") as f:
            raw = f.read()

        key = secrets.randbits(64)

        filename = os.path.basename(src)

        if "." in filename:
            name, ext = filename.rsplit(".", 1)
        else:
            name, ext = filename, "bin"

        name_b = name.encode()
        ext_b = ext.encode()

        header = (
            struct.pack("B", len(name_b)) +
            name_b +
            struct.pack("B", len(ext_b)) +
            ext_b
        )

        combined = header + raw
        compressed = self._compress(combined)

        enc = bytearray()
        for i, b in enumerate(compressed):
            enc.append(b ^ self._keystream_byte(key, i))

        payload = self._encode(bytes(enc))

        container = f"{key}:{payload}"

        with open(dst, "w", encoding="utf-8") as f:
            f.write(container)

    def decrypt_file(self, src: str, dst_folder: str):
        with open(src, "r", encoding="utf-8") as f:
            key_str, payload = f.read().split(":", 1)

        key = int(key_str)

        data = self._decode(payload)

        dec = bytearray()
        for i, b in enumerate(data):
            dec.append(b ^ self._keystream_byte(key, i))

        decompressed = self._decompress(bytes(dec))

        name_len = decompressed[0]
        name = decompressed[1:1 + name_len].decode()

        ext_pos = 1 + name_len
        ext_len = decompressed[ext_pos]

        ext = decompressed[ext_pos + 1:ext_pos + 1 + ext_len].decode()

        file_start = ext_pos + 1 + ext_len
        file_data = decompressed[file_start:]

        out_path = os.path.join(dst_folder, f"{name}.{ext}")

        with open(out_path, "wb") as f:
            f.write(file_data)

        print(f"[✓] Файл восстановлен: {out_path}")

    # =========================
    # MULTILINE TEXT INPUT
    # =========================

    def _get_multiline_input(self, prompt: str) -> str:
        """Сбор многострочного текста от пользователя"""
        print(prompt)
        print("(Для завершения ввода оставьте пустую строку или нажмите Ctrl+D/Ctrl+Z)")
        lines = []
        
        try:
            while True:
                line = input()
                if line == "":
                    break
                lines.append(line)
        except EOFError:
            # Ctrl+D (Linux/Mac) или Ctrl+Z (Windows)
            pass
        
        return "\n".join(lines)

    # =========================
    # TERMINAL UI
    # =========================

    def run(self):
        while True:
            print("=" * 40)
            print("1. Зашифровать текст")
            print("2. Расшифровать текст")
            print("3. Зашифровать файл")
            print("4. Расшифровать файл")
            print("5. Зашифровать текст (однострочный)")
            print("6. Расшифровать текст из файла")
            print("0. Выход")
            print("=" * 40)

            choice = input("Выберите опцию: ").strip()

            # TEXT ENC (MULTILINE)
            if choice == "1":
                text = self._get_multiline_input("\n📝 Введите текст (поддерживается многострочный):")
                
                if not text:
                    print("⚠️ Текст не может быть пустым!")
                    continue
                
                result = self.encrypt_text(text)
                print("\n🔐 Результат:")
                print(result)
                print("\n💡 Сохраните этот контейнер для расшифровки")

            # TEXT DEC
            elif choice == "2":
                data = input("\n🔑 Введите контейнер:\n> ")
                try:
                    decrypted = self.decrypt_text(data)
                    print("\n🔓 Расшифрованный текст:")
                    print("-" * 40)
                    print(decrypted)
                    print("-" * 40)
                except Exception as e:
                    print(f"❌ Ошибка: {e}")

            # FILE ENC
            elif choice == "3":
                src = input("📁 Путь к файлу: ")
                dst = input("💾 Куда сохранить (.txt контейнер): ")

                try:
                    self.encrypt_file(src, dst)
                    print("[✓] Файл зашифрован")
                except Exception as e:
                    print(f"❌ Ошибка: {e}")

            # FILE DEC
            elif choice == "4":
                src = input("📁 Путь к контейнеру: ")
                dst = input("📂 Папка для восстановления: ")

                try:
                    self.decrypt_file(src, dst)
                except Exception as e:
                    print(f"❌ Ошибка: {e}")

            # TEXT ENC (SINGLE LINE)
            elif choice == "5":
                text = input("\n📝 Введите текст (однострочный):\n> ")
                result = self.encrypt_text(text)
                print("\n🔐 Результат:")
                print(result)

            # DECRYPT FROM FILE
            elif choice == "6":
                filepath = input("📁 Путь к файлу с контейнером: ")
                try:
                    with open(filepath, "r", encoding="utf-8") as f:
                        data = f.read().strip()
                    
                    decrypted = self.decrypt_text(data)
                    print("\n🔓 Расшифрованный текст:")
                    print("-" * 40)
                    print(decrypted)
                    print("-" * 40)
                    
                    # Предложение сохранить результат
                    save = input("\n💾 Сохранить расшифрованный текст в файл? (y/n): ").lower()
                    if save == 'y':
                        out_file = input("Путь для сохранения: ")
                        with open(out_file, "w", encoding="utf-8") as f:
                            f.write(decrypted)
                        print(f"[✓] Сохранено в {out_file}")
                        
                except Exception as e:
                    print(f"❌ Ошибка: {e}")

            # EXIT
            elif choice == "0":
                print("👋 Выход...")
                break

            else:
                print("❌ Неверный выбор")


# =========================
# RUN
# =========================

if __name__ == "__main__":
    QShadowV7().run()

    