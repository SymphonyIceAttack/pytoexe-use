...
def collect_overview():
    """Основная информация для титульного листа."""
    data = {
...
def collect_system_info():
    """Детальная информация о системе (производитель, модель, ОЗУ, серийный номер, дата установки)."""
    info = {}
    try:
...
def collect_users_and_groups():
    """Локальные пользователи и все локальные группы с членами."""
    users = []
    admins = []
    try:
...
def collect_network():
    """Сетевые настройки: RDP, netstat, hosts, DNS кэш."""
    rdp = {}
    try:
        import winreg
        with winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE,
...
def collect_installed_software():
    """Установленные программы из реестра (Uninstall)."""
    import winreg
    software = []
    paths = [
...
def collect_portable_apps():
    """Портативные программы в Program Files и LocalAppData."""
    known_names = set()
...
def collect_autoruns():
    """Расширенные точки автозагрузки (Run, RunOnce, Winlogon)."""
    import winreg
    entries = []
    hives = {
...
def collect_scheduled_tasks():
    """Все задачи планировщика (детальный CSV)."""
    tasks = []
    try:
...
def collect_security_events():
    """Ключевые события безопасности за последние 24 часа (wevtutil)."""
    events = []
    try:
...
def collect_usb_history():
    """Ранее подключавшиеся USB-устройства (реестр)."""
    import winreg
    devices = []
...
def collect_applocker():
    """Правила AppLocker (если доступны)."""
    rules = []
    try:
        out = subprocess.check_output(
...
def style_sheet(ws, headers=None):
    """Применяет стили: жирный заголовок, автоширина, замораживание первой строки."""
    if headers:
...
def create_workbook(data_blocks):
    """Создаёт Excel с титульным листом и всеми блоками."""
    wb = Workbook()
    ...
    overview_data = data_blocks.get("Overview", {})
    ...
    for block_name, (headers, rows) in data_blocks.items():
        if block_name == "Overview":
            continue
...
def main():
    ...
    data_blocks = {
        ...
        "Netstat": (["Line"], netstat) if netstat else (["Line"], []),
        "Hosts": (["Entry"], hosts) if hosts else (["Entry"], []),
        "DNS Cache": (["Record"], [{"Record": str(r)} for r in dns_cache]) if dns_cache else (["Record"], []),
        ...
        "BitLocker": (["Volume", "ConversionStatus", "ProtectionMethod"], bitlocker) if bitlocker else ([], []),
    ...
    wb = create_workbook(data_blocks)
    xlsx_buffer = BytesIO()
    wb.save(xlsx_buffer)
    ...
    with open(enc_path, "wb") as f:
        f.write(encrypted)
    print(f"Отчёт сохранён в каталоге C:\Temp: {enc_path}")
    input("Нажмите Enter для выхода, прикрепите созданный отчет к задаче RM...")
