...
# Производители материалов (0-20)
MANUFACTURERS = {
    0: {"de": "Unbekannt", "en": "Unknown"},
    1: {"de": "QIDI", "en": "QIDI"},
    2: {"de": "eSUN", "en": "eSUN"},
    3: {"de": "Sunlu", "en": "Sunlu"},
    4: {"de": "Polymaker", "en": "Polymaker"},
    5: {"de": "Prusament", "en": "Prusament"},
    6: {"de": "Overture", "en": "Overture"},
    7: {"de": "Hatchbox", "en": "Hatchbox"},
    8: {"de": "Anycubic", "en": "Anycubic"},
    9: {"de": "Creality", "en": "Creality"},
    10: {"de": "Bambu Lab", "en": "Bambu Lab"},
    11: {"de": "Extrudr", "en": "Extrudr"},
    12: {"de": "Das Filament", "en": "Das Filament"},
    13: {"de": "3DJAKE", "en": "3DJAKE"},
    14: {"de": "ColorFabb", "en": "ColorFabb"},
    15: {"de": "Fillamentum", "en": "Fillamentum"},
    16: {"de": "Formfutura", "en": "Formfutura"},
    17: {"de": "MatterHackers", "en": "MatterHackers"},
    18: {"de": "Proto-Pasta", "en": "Proto-Pasta"},
    19: {"de": "Verbatim", "en": "Verbatim"},
    20: {"de": "Recycling", "en": "Recycling"}
}
...
def read_tag():
    ...
    try:
        ...
        # Backward compatibility check for old format (3 bytes)
        is_old_format = False
        if len(data) > 3:
            # If only material and color are set, and manufacturer/value are zero
    ...
    while True:
        ...
        try:
            ...
            if tag_now_present and not tag_present:
                ...
                try:
                    # Quick check if tag is still there
...
def switch_lang():
    ...
    # Save current manufacturer selection
    current_man = manufacturer_var.get()
    current_man_num = None
    
    # Try to find the manufacturer number
    if current_lang == "de":
    ...
    # Restore manufacturer selection if possible
    if current_man_num is not None:
        if current_lang == "de":
            manufacturer_var.set(MANUFACTURERS[current_man_num]["de"])
        else:
            manufacturer_var.set(MANUFACTURERS[current_man_num]["en"])
...
# Create manufacturer list based on current language
def get_manufacturer_list():
    if current_lang == "de":
        return [MANUFACTURERS[i]["de"] for i in range(21)]
    else:
        return [MANUFACTURERS[i]["en"] for i in range(21)]
...
read_btn = ctk.CTkButton(main_frame, text=LANG[current_lang]["read"], command=lambda: read_tag(),
...
def show_tag_info(material, hex_code, name, manufacturer="---"):
    ...
    if name and name != "---" and name != "":  # If we have a color name
        text_color = "black" if hex_code in ["#FAFAFA", "#D9E3ED", "#99DEFF", "#CEC0FE", 
                                           "#CADE4B", "#E2DFCD", "#CAC59F"] else "white"
...
def update_labels():
    ...
    # Update manufacturer dropdown values
    current_man = manufacturer_var.get()
    manufacturer_combo.configure(values=get_manufacturer_list())
    
    # Try to restore manufacturer selection
    if current_lang == "de":
        for i in range(21):
            if MANUFACTURERS[i]["en"] == current_man:
                manufacturer_var.set(MANUFACTURERS[i]["de"])
                break
    else:
        for i in range(21):
            if MANUFACTURERS[i]["de"] == current_man:
                manufacturer_var.set(MANUFACTURERS[i]["en"])
                break
    
    # If not found, set default
    if not manufacturer_var.get():
