...
FADE_SPEED = 15           # 淡入速度（越小越快）
ROUND_RADIUS = 30         # 圆角大小
...
# 柔和随机颜色
bg_colors = [
    "#FAC4ED", "#9FD7F5", "#B0FBB6", "#FBD7B5",
    "#FAC0D0", "#FFBCF0", "#ADC9F4", "#F6D8B4"
]
...
class PerfectPopupApp:
    ...
    def create_rounded_win(self, width, height, bg_color, init_alpha=0):
        ...
        win.attributes("-alpha", init_alpha)  # 初始透明度
    ...
    def create_main_popup(self):
        win, canvas = self.create_rounded_win(MAIN_W, MAIN_H, "#ffffff", WINDOW_ALPHA)
        sw, sh = self.root.winfo_screenwidth(), self.root.winfo_screenheight()
        win.geometry(f"+{(sw-MAIN_W)//2}+{(sh-MAIN_H)//2}")

        tk.Label(canvas, text="赠予你满心温柔", bg="white", font=("楷体", 22, "bold"), fg="#444"
                ).place(x=0,y=30,width=MAIN_W,height=50)
        tk.Button(canvas, text="取消", bg="#D9B0B4", fg="white", font=("楷体",14),
                 command=lambda: self.close_win(win)).place(x=60,y=110,width=100,height=40)
        tk.Button(canvas, text="接收", bg="#9EB88A", fg="white", font=("楷体",14),
    ...
    def create_small_popup(self):
        ...
        tk.Label(canvas, text=random.choice(messages), bg=color, font=("楷体", 18, "bold"), fg="white"
    ...
    def create_tip_win(self):
        ...
        tk.Label(canvas, text="💖 点击汇聚爱心", bg=TIP_COLOR, font=("楷体", 19, "bold"), fg="white"
