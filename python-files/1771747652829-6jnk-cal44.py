...
def calculate(event=None):
    ...
    if ai_mode and v in AI_SPECIALS:
        # الرقم + العلامة + next كلها أصغر 40%
        base_size = int(BASE_FONT * current_scale * 1.05 * 0.6)
        middle_text = f"{v} <\nnext"
        middle_font = (current_font, base_size)
...
def auto_update():
    ...
    # تحديد المجموعة مباشرة من limits دون انتظار
    group_value = None
    for i in range(len(limits)-1):
        if limits[i] <= total_min % 60 <= limits[i+1]:
            group_value = limits[i]
            break

    if group_value is not None:
        entry.delete(0,"end")
        entry.insert(0, group_value)
        calculate()
