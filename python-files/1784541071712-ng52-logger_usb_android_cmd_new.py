...
def get_usb_devices_wmic():
    # Используем надежный запрос для получения DeviceID
...
def save_to_log(info, usb_devices, android_results, log_file):
    ...
    # Если устройств нет, ставим заглушки
    if usb_devices and len(usb_devices) > 0:
        first_usb = usb_devices[0]
        # Формируем строку вида: bus_class: USBSTOR || dev_type: DISK || vendor: ... и т.д.
        usb_details = (
    ...
    # Все найденные папки через запятую
    if android_results:
        android_str = ", ".join(android_results)
    else:
        android_str = "[Папки не найдены]"

    # ГЛАВНАЯ СТРОКА: собираем всё по твоему шаблону
    log_line = (
