...
class RE4MasterEditor(tk.Tk):
    ...
    def _build_ui(self):
        ...
        for tool in TOOLS_DEF:
            tid      = tool["id"]
            lock     = tool.get("lock")
            init_fg  = "#cc2222" if lock else _MUTED  # red if locked
    ...
    def _switch_tool(self, tool_id):
        ...
        if lock:
            ...
            code_name = lock[1]
            # check quick detect
            is_open = getattr(self, "_unlocked_codes", set())
            if code_id not in is_open:
                messagebox.showwarning(
                    "Module Locked" if CURRENT_LANG == "en" else "القسم مقفل",
                    f"This module requires [{code_name}] to be enabled in RE4 CODE MANAGER."
                    if CURRENT_LANG == "en" else
                    f"هذا القسم يتطلب تفعيل [{code_name}] في RE4 CODE MANAGER."
                )
                return
    ...
    def _quick_unlock(self, exe_path):
        """Read EXE directly to detect which codes are applied."""
        applied = quick_detect_codes(exe_path)
        self._unlocked_codes = applied  # store for nav button check
        for tool in TOOLS_DEF:
            lock = tool.get("lock")
            if not lock:
                continue
            ...
            if btn:
                if code_id in applied:
                    btn.configure(fg=_GOLD)   # normal — unlocked
                else:
                    btn.configure(fg="#cc2222")  # red — locked

    def _check_locked_panels(self):
        """Unlock AEV/OSD panels if codes are applied (after full scan)."""
        cm = self._cm_app
        if not cm:
            return
        for tool in TOOLS_DEF:
            ...
            btn = self._nav_btns.get(tid)
            if btn:
                if code_id in self._unlocked_codes:
                    btn.configure(fg=_GOLD)
                else:
                    btn.configure(fg="#cc2222")
