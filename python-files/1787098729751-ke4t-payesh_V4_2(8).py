...
class App:
    ...
    def monitor_symbol(self, index, item):
        ...
        while self.running and not self.monitor_stop_event.is_set():
            try:
                code, title = get_status(inscode)
                self.update_monitor_indicator(True)
                ...
                if code != previous:
                    ...
                    if previous == TARGET_FROM and code == TARGET_TO:
                        ...
                        # Once IS → IR is detected, monitoring is no longer
                        # needed for this run. Stop all monitor workers first,
                        # then let the click sequence continue.
                        self.monitor_stop_event.set()
                        self.root.after(
                            0,
                            lambda n=name: self.write_log(
                                f"{n}: تغییر IS → IR تشخیص داده شد؛ فقط پایش متوقف شد و کلیک‌ها شروع می‌شوند."
                            )
                        )
                        ...
                        self.root.after(
                            0,
                            lambda n=name: self.write_log(
                                f"{n}: Trigger کلیک ثبت شد؛ مسیر کلیک مستقل از پایش اجرا می‌شود."
                            )
                        )
                        threading.Thread(
            ...
            self.monitor_stop_event.wait(max(0, next_time - time.perf_counter()))
    ...
    def delayed_click(self, symbol, detected_perf):
        ...
        with self.click_lock:
            ...
            # After a confirmed IS → IR trigger, click execution is independent
            # of monitor_stop_event. Only an explicit global stop may cancel it.
            if self.stop_event.wait(delay_s):
                self.root.after(0, lambda: self.write_log(
                    f"{symbol}: عملیات کلیک قبل از شروع با Stop متوقف شد."
                ))
                return
            ...
            for i in range(self.click_count):
                if self.stop_event.is_set():
                    self.root.after(0, lambda n=i + 1: self.write_log(
                        f"{symbol}: کلیک در مرحله {n} با Stop متوقف شد."
                    ))
                    return
                ...
                try:
                    ...
                    self.root.after(
                        0,
                        lambda n=i + 1, e=msg: self.write_log(
                            f"{symbol}: خطا در اجرای کلیک {n}: {e}"
                        )
                    )
                    return
