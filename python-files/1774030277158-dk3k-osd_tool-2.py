...
def parse_osd_file(raw):
    ...
    all_blocks = []
    block_num  = 0   # عداد شامل (TRUE + FALSE)

    for i, (t_start, t_end, tb) in enumerate(true_blocks):
        block_num += 1
        # البلوكات 1-6: تظهر حتى لو فاضية وتكون FALSE
        # البلوك 7+: الفاضية تسكب
...
def do_extract(osd_path):
    ...
    # لو ما لقى MAGIC (ملف أصفار خام) → 6 بلوكات FALSE فاضية
    if not blocks:
        blocks = [make_block(False, 0, [], [], 0, [], []) for _ in range(6)]
        print("  [!] ما لقيت بلوكات - الملف أصفار، سيتم إنشاء 6 بلوكات فاضية")
