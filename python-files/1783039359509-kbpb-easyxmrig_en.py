...
class EasyXMRig(tk.Tk):
    ...
    def _build_widgets(self):
        ...
        ttk.Label(settings_frame, text="Pool (fixed):").grid(row=0, column=0, sticky="w", padx=8, pady=4)
        ...
        ttk.Checkbutton(
            settings_frame, text="Use TLS", variable=self.tls_var, state="disabled"
    ...
    def _validate_wallet(self, wallet):
        ...
        if wallet[0] not in "456789":
            return False, f"Wallet address should start with 4, 5, 8, or 9, but got '{wallet[0]}'. Is this a valid Monero address?"
        
        if len(wallet) < 80 or len(wallet) > 120:
            return False, f"Wallet address seems too short/long ({len(wallet)} chars). Monero addresses are typically 80-120 chars."
        
        # Check if alphanumeric only
        if not all(c.isalnum() for c in wallet):
            return False, "Wallet address should contain only alphanumeric characters."
    ...
    def _start_mining(self):
        ...
        if not is_valid:
            messagebox.showwarning(APP_TITLE, f"Wallet address validation failed:\n\n{error}")
            return
        ...
        self._log(f"Starting user miner with config {config_path} ({user_threads} thread(s)) ...")
