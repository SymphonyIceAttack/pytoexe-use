...
# 错误日志记录
def log_error(msg):
    log_file = Path.home() / ".video_wallpaper_error.log"
...
try:
    ...
    try:
        ...
        USE_QT_MEDIA = False
        log_error(f"QtMultimedia 导入失败: {e}")
        
except Exception as e:
    log_error(f"PyQt5 导入失败: {e}")
...
class WallpaperWindow(QWidget):
    ...
    def setup_ui(self):
        ...
        if USE_QT_MEDIA:
            ...
            # 使用 QLabel 作为占位符
            self.label = QLabel("视频加载中...\n请确保已安装 PyQt5 和 PyQt5-multimedia")
...
class SettingsWindow(QMainWindow):
    ...
    def play_video(self):
        ...
        try:
            if self.current_video and os.path.exists(self.current_video):
                ...
                self.status_label.setStyleSheet("color: green;")
                self.status_info.setText("状态: 播放中")
                self.save_config()
            else:
    ...
    def stop_video(self):
        ...
        try:
            ...
            self.status_info.setText("状态: 已停止")
        except Exception as e:
            log_error(f"停止失败: {e}")
