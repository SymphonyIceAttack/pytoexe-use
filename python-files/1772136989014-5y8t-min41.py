...
history_win = None
history_text = None
adjust_value = 0  # slider adjust in seconds
...
def draw_ai_lamp():
    ...
    ai_canvas.config(width=size+8, height=size+8)
...
def ai_loop():
    ...
    if not ai_enabled:
        return
    now = datetime.now()
    # put minute into entry (keeps old behaviour)
...
def toggle_ai():
    ...
    if ai_enabled:
        ...
        # stop any running timer when AI turned off
        stop_timer()

# ------------------- Timer UI & Logic -------------------
timer_frame = tk.Frame(root)
timer_frame.pack(pady=4)
timer_label = tk.Label(timer_frame, text="", font=(current_font, 14), fg="blue")
timer_label.pack()

def start_timer(t_type, seconds):
    global timer_active, timer_type, timer_seconds, timer_after_id
    if timer_active:
        return
    timer_active = True
    timer_type = t_type
    timer_seconds = max(0, seconds)
    update_timer_label()
    timer_after_id = root.after(1000, tick_timer)

def tick_timer():
    global timer_seconds, timer_active, timer_type, timer_after_id
    timer_seconds -= 1
    if timer_seconds > 0:
        update_timer_label()
        timer_after_id = root.after(1000, tick_timer)
        ...
        # timer finished
        if timer_type == "next":
            # next finished -> automatically start go trade 25s
            hide_timer_label()  # remove 'next' display before starting go
            timer_active = False
            timer_type = None
            start_timer("go", 25 + adjust_value)
        elif timer_type == "go":
            # go finished -> advance to next group and clear
            hide_timer_label()
            timer_active = False
            timer_type = None
            # display next group (mimic pressing next)
            display_group(current_group_idx+1)

def update_timer_label():
    if timer_active and timer_type:
        text = f"{timer_type} {timer_seconds}s" if timer_type!="go" else f"go trade {timer_seconds}s"
        timer_label.config(text=text)
        timer_label.pack()
    else:
        hide_timer_label()

def hide_timer_label():
    timer_label.config(text="")
    # keep the widget but hide visually by empty text

def stop_timer():
    global timer_after_id, timer_active, timer_type
    if timer_after_id:
        try:
...
# ------------------- Timer Checker -------------------
def check_timers(now_dt):
    """
    now_dt: datetime object from ai_loop (passed for precision)
    """
    global timer_active
    if not ai_enabled:
        return
    ...
    # If timer already active, ignore new triggers (we chain inside start/tick)
    if timer_active:
        return

    # Priority: if now_str in NEXT_TIMES -> start next; elif in GO_TIMES -> start go
    if now_str in NEXT_TIMES:
        start_timer("next", 30 + adjust_value)
        return
    if now_str in GO_TIMES:
        start_timer("go", 25 + adjust_value)
        return
...
def toggle_mode():
    ...
    timer_label.config(fg="yellow" if dark_mode else "blue")
    draw_ai_lamp()

def open_scale_popup():
    scale_win = tk.Toplevel(root)
    scale_win.title("Scale / Font Size")
    scale_win.geometry("300x120")
    def on_zoom(val):
        val = int(float(val))
        ...
        timer_label.config(font=(current_font, int(val*0.9)))
        draw_ai_lamp()
...
def open_edit_timer():
    ...
    tk.Button(btn_frame, text="-10", command=minus10).pack(side="left")
    tk.Button(btn_frame, text="+10", command=plus10).pack(side="left", padx=8)
    ...
    tk.Button(edit_win, text="OK", command=save_adjust).pack(pady=8)
