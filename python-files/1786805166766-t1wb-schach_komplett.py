...
class ChessApp(tk.Tk):
    ...
    def _show_board_screen(self):
        ...
        # Materialbilanz-Panel (aus schach.py uebernommen): zeigt laufend an,
        # wer wieviel Material gewonnen hat und welche Figuren geschlagen wurden.
        material_frame = tk.Frame(left, bg=APP_BG)
        material_frame.pack(pady=(8, 0), fill="x")
        tk.Label(material_frame, text="Materialbilanz:", bg=APP_BG, fg="#333333",
                 font=("Tahoma", 10, "bold")).grid(row=0, column=0, sticky="w")
        self.material_label = tk.Label(material_frame, text="+0", bg=APP_BG, fg="#2f6f2f",
                                        font=("Tahoma", 12, "bold"))
        self.material_label.grid(row=0, column=1, sticky="w", padx=(6, 0))
        tk.Label(material_frame, text="Von Weiß geschlagen:", bg=APP_BG, fg="#555555",
                 font=("Tahoma", 9)).grid(row=1, column=0, columnspan=2, sticky="w", pady=(4, 0))
        self.captured_by_white_label = tk.Label(
            material_frame, text="-", bg=APP_BG, fg="#333333",
            font=("Segoe UI Symbol", 14), wraplength=BOARD_PIXELS + 20, justify="left")
        self.captured_by_white_label.grid(row=2, column=0, columnspan=2, sticky="w")
        tk.Label(material_frame, text="Von Schwarz geschlagen:", bg=APP_BG, fg="#555555",
                 font=("Tahoma", 9)).grid(row=3, column=0, columnspan=2, sticky="w", pady=(4, 0))
        self.captured_by_black_label = tk.Label(
            material_frame, text="-", bg=APP_BG, fg="#333333",
            font=("Segoe UI Symbol", 14), wraplength=BOARD_PIXELS + 20, justify="left")
        self.captured_by_black_label.grid(row=4, column=0, columnspan=2, sticky="w")
    ...
    def _update_material_panel(self):
        """Aktualisiert die Materialbilanz-Anzeige anhand der bisher geschlagenen
        Figuren in der aktuell angezeigten Stellung (self.state_.history) -
        funktioniert dadurch automatisch auch beim Zurueck-/Vorblaettern."""
        ...
        def symbole(figuren):
            return " ".join(UNICODE_PIECES[p] for p in figuren) if figuren else "-"

        self.captured_by_white_label.config(text=symbole(von_weiss_geschlagen))
        self.captured_by_black_label.config(text=symbole(von_schwarz_geschlagen))
