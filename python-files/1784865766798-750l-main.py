...
class JSRigGUI(customtkinter.CTk):
    def __init__(self):
        ...
        # Pool & Port Row (MoneroOcean Fixed)
        ...
        self.pool_entry.configure(state="disabled")   # 고정
        ...
        self.port_entry.configure(state="disabled")   # 고정
        ...
        self.start_button = customtkinter.CTkButton(
            ...
            height=36, width=140, corner_radius=4, border_width=1, border_color=GOOGLE_GRAY
        )
    ...
    def start_mining(self):
        ...
        if not wallet:
            messagebox.showwarning("jsrig", "Please enter a Monero wallet address.")
            return
        ...
        try:
            ...
            self.status_label.configure(text="jsrig is mining on MoneroOcean...", text_color=GOOGLE_BLUE)
    ...
    def monitor_miner(self):
        for line in iter(self.miner_process.stdout.readline, ''):
            if "Submitting share" in line or "share found" in line.lower():
                self.after(0, lambda l=line: self.status_label.configure(
                    text=f"Share: {l.strip()[:60]}..."
                ))
