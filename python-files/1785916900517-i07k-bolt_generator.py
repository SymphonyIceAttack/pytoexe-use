# -*- coding: utf-8 -*-
"""
Fastener Assembly TXT Generator v6
Generates Bocad-format fastener assembly input files based on
bolt size, connection thickness, and bolt length.

v6 changes:
  - Connection thickness input semantics: T_input = actual connector thickness T_conn
    Center-to-center distance between flat washers = T_input + t_w
  - Bolt head NREVOLUTION chamfer radius fixed (was exceeding blank diameter)
  - GUI label simplified to "连接件厚度" (removed "中心距")
  - T_conn display field removed (T_input IS T_conn now)
  - Filename format: M{size}x{length}-{thickness}.txt
...
# v6 fix: chamfer outer radius changed from 18 to 17.35 (e/2 = blank radius)
# to prevent NREVOLUTION from exceeding the cylinder blank
...
def fmt_thickness(val):
    """Format thickness value for filename - integer without decimal, decimal kept."""
    if val == int(val):
        return f"{int(val)}"
    else:
        s = f"{val:.2f}".rstrip('0').rstrip('.')
...
# ============================================================
# Z Position Calculations (v6 algorithm)
# ============================================================
...
def calc_z_positions(bolt_size, bolt_length, t_input):
    ...
    v6 algorithm:
      t_input = actual connector thickness T_conn (被夹连接件实际厚度)
      Center-to-center distance between flat washers = t_input + t_w
      (tool internally adds one washer thickness)

    Stacking from -Z to +Z:
      bolt head -> flat washer 1 -> [T_conn connection] -> flat washer 2
      -> spring washer -> nut

    Z coordinate rules:
      Flat washer 1 center = -L + t_w/2  (relative to bolt tip at +Z)
      Flat washer 2 center = FW1 center + (t_input + t_w)
    ...
    # v6: Flat washer 1: against head (head side)
    # FW1 center = interface_z + fw_h / 2.0
    # Equivalent to: -L + t_w/2 from bolt tip (REF_BOLT_TIP_Z - L + fw_h/2)
    fw1_center_z = interface_z + fw_h / 2.0

    # v6: Flat washer 2: nut side
    # Center-to-center distance = t_input + fw_h
...
def generate_fastener_txt(bolt_size, bolt_length, t_input):
    ...
    Syntax matches reference pattern with v6 optimizations.

    v6: t_input = actual connector thickness T_conn
         Center-to-center = t_input + t_w
         NREVOLUTION chamfer radius fixed to e/2 (blank radius)
    """
    bolt = BOLT_DATA[bolt_size]
...
class BoltGeneratorApp:
    ...
    def create_widgets(self):
        ...
        # v6: Connection thickness (T_input = actual connector thickness)
    ...
    def update_info(self):
        ...
        v6: Shows bolt, flat washer, spring washer, nut dimensions.
        T_conn field removed (T_input IS the connector thickness).
        """
    ...
    def generate_fastener(self):
        ...
        v6: Filename format: M{size}x{length}-{thickness}.txt
        """
        # Validate inputs
        bolt_size = self.bolt_size_var.get()
        if not bolt_size:
        ...
        # v6: Default filename = M{size}x{length}-{thickness}.txt
        # Integer thickness: no decimal point; decimal: keep decimal
