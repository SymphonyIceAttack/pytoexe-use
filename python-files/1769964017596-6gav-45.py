...
class TextEditor:
    def __init__(self, root):
        self.root = root
        self.root.title("ConvenientRusEditingOD")
        ...
        buttons = [
            ...
            ("Изменить шрифт", self.change_font), ("Цвет текста", self.change_text_color),
            ("Размер шрифта", self.change_font_size), ("Отмена", self.undo)  # Добавленные кнопки
        ]
        ...
        self.text_area = tk.Text(self.root, wrap=tk.WORD, font=("Consolas", 12), bg=self.current_theme["text_bg"], fg=self.current_theme["text_fg"], insertbackground=self.current_theme["insert_bg"], selectbackground=self.current_theme["select_bg"], bd=0, undo=True)  #undo
        self.text_area.pack(side=tk.LEFT, expand=True, fill=tk.BOTH, padx=(0, 10), pady=10)

        self.current_file = None
        self.default_font = ("Consolas", 12)  # Сохраняем шрифт по умолчанию здесь
        self.text_area.config(font=self.default_font)
        self.root.createcommand('tk::mac::ReopenApplication', self.open_file)  #macOS
    ...
    def apply_theme(self):
        ...
        for text, command in [
            ...
            ("Eng/Рус", self.translate_text), ("Сменить тему", self.toggle_theme),
            ("Изменить шрифт", self.change_font), ("Цвет текста", self.change_text_color),
            ("Размер шрифта", self.change_font_size), ("Отмена", self.undo)
        ]:
    ...
    def change_font(self):
        font_choices = ["Times New Roman", "Arial", "Consolas"]
        font_name = simpledialog.askstring("Изменить шрифт", "Выберите шрифт (Times New Roman, Arial, Consolas):", initialvalue=self.text_area.cget("font"))
        if font_name in font_choices:
            current_size = font.Font(font=self.text_area.cget("font")).actual("size")
            self.default_font = (font_name, current_size)
            self.text_area.configure(font=self.default_font)
        elif font_name:
            messagebox.showinfo("Шрифт", "Шрифт не найден")
    ...
    def change_text_color(self):
        try:
            ...
            if color:
                ...
                self.text_area.tag_add("colored", start, end)
        except tk.TclError:
            messagebox.showinfo("Цвет", "Выделите текст для изменения цвета.")

    def change_font_size(self):
        font_size = simpledialog.askinteger("Размер шрифта", "Введите размер шрифта:", initialvalue=font.Font(font=self.text_area.cget("font")).actual("size"))
        if font_size:
            current_font_name = font.Font(font=self.text_area.cget("font")).actual("family")
            self.default_font = (current_font_name, font_size)
            self.text_area.configure(font=self.default_font)
