...
def open_history():
    ...
    history_text = tk.Text(history_win,state="disabled",
                           font=(current_font,int(BASE_FONT*current_scale)))
...
def open_scale_popup():
    p = tk.Toplevel(root)
    p.title("Scale")
    scale = tk.Scale(p, from_=0.7, to=1.5, resolution=0.05,
...
def edit_limits():
    global limits
    s = simpledialog.askstring("Edit Limits","Comma separated:",
...
def draw_ai_lamp():
    ...
    lamp_size = int(BASE_FONT * current_scale * 0.3 * 1.5)
...
def auto_update_time():
    ...
    entry.insert(0, str((now.hour*60 + now.minute) % 60))
    calculate()
    root.after(1000, auto_update_time)
