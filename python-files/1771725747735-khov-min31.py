...
def display_group_by_index(idx):
    ...
    if idx is None:
        return
    idx %= len(trade_table)  # loop around
...
def prev_group(event=None):
    display_group_by_index((current_group_idx - 1) % len(trade_table))
    return "break"
...
entry_minutes = tk.Entry(top_frame, justify="center", font=(current_font, int(BASE_FONT*1.3)), width=12)
entry_minutes.pack()
...
btn_generate = tk.Button(control_frame, text="Generate", font=(current_font,int(BASE_FONT*0.75)), width=10, command=on_generate)
btn_generate.pack(side="left", padx=6)

btn_clear = tk.Button(control_frame, text="Clear", font=(current_font,int(BASE_FONT*0.75)), width=10, command=clear_output)
btn_clear.pack(side="left", padx=6)

btn_prev = tk.Button(control_frame, text="◀", font=(current_font,int(BASE_FONT*0.75)), width=5, command=prev_group)
btn_prev.pack(side="left", padx=12)

btn_next = tk.Button(control_frame, text="▶", font=(current_font,int(BASE_FONT*0.75)), width=5, command=next_group)
