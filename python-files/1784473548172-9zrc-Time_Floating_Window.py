...
class FloatingClockApp():
    ...
    def load_settings(self):
        ...
        if os.path.exists(self.config_file):
            ...
            self.settings = {
                ...
                "text_color": "#FFFFFF",
                "bg_opacity": 0.8,
                "text_opacity": 1.0,  # New: text opacity
                "width": 180,
                "height": 60,
                ...
                "show_date": True,  # New: show date
                "show_weekday": True,  # New: show weekday
                "date_format": "%Y-%m-%d",  # New: date format
                "weekday_format": "%A",  # New: weekday format
                "date_font_size": 10,  # New: date font size
            }
            return True
    ...
    def create_window(self, first_run):
        ...
        # Create main container for time and date
        ...
        # Create date and weekday label
        self.date_label = tk.Label(
            self.main_container,
            text="",
            font=(self.settings["font_family"], self.settings["date_font_size"]),
            bg=self.settings["bg_color"],
            fg=self.settings["text_color"]
        )
        self.date_label.pack()

        # Apply text opacity if supported
        try:
            self.time_label.config(alpha=self.settings["text_opacity"])
            self.date_label.config(alpha=self.settings["text_opacity"])
        except:
            pass
    ...
    def setup_tray(self):
        ...
        menu = Menu(
            MenuItem(self.texts["show"], on_show),
            MenuItem(self.texts["hide"], on_hide),
            MenuItem(self.texts["settings"], on_settings),
            MenuItem(self.texts["close"], on_quit)
        )

        self.tray_icon = Icon("FloatingClock", create_icon_image(), "Floating Clock", menu)
    ...
    def open_settings(self):
        ...
        ttk.Label(frm, text=self.texts["text_opacity"]).grid(row=r, column=0, padx=10, pady=5, sticky="w")
        text_alpha_frame = ttk.Frame(frm)
        text_alpha_frame.grid(row=r, column=1, padx=10, pady=5, sticky="w")
        self.text_alpha_scale = ttk.Scale(
            text_alpha_frame,
            from_=0.1,
            to=1.0,
            value=self.settings.get("text_opacity", 1.0),
            orient="horizontal",
            length=120,
            command=self.text_alpha_scale_changed
        )
        self.text_alpha_scale.pack(side="left")

        self.text_alpha_entry = ttk.Entry(text_alpha_frame, width=5)
        self.text_alpha_entry.insert(0, str(self.settings.get("text_opacity", 1.0)))
        self.text_alpha_entry.pack(side="left", padx=5)
        self.text_alpha_entry.bind("<FocusOut>", lambda e: self.text_alpha_entry_apply())
        self.text_alpha_entry.bind("<Return>", lambda e: self.text_alpha_entry_apply())
        r += 1
        ...
        # Show date checkbox
        show_date = tk.BooleanVar(value=self.settings.get("show_date", True))
        ttk.Label(frm, text=self.texts["show_date"]).grid(row=r, column=0, padx=10, pady=5, sticky="w")
        ttk.Checkbutton(frm, variable=show_date, command=lambda: self.set_show_date(show_date.get())).grid(row=r, column=1, padx=10, pady=5, sticky="w")
        r += 1

        # Show weekday checkbox
        show_weekday = tk.BooleanVar(value=self.settings.get("show_weekday", True))
        ttk.Label(frm, text=self.texts["show_weekday"]).grid(row=r, column=0, padx=10, pady=5, sticky="w")
        ttk.Checkbutton(frm, variable=show_weekday, command=lambda: self.set_show_weekday(show_weekday.get())).grid(row=r, column=1, padx=10, pady=5, sticky="w")
        r += 1

        # Date font size
        ttk.Label(frm, text=self.texts["date_font_size"]).grid(row=r, column=0, padx=10, pady=5, sticky="w")
        dfs_frame = ttk.Frame(frm)
        dfs_frame.grid(row=r, column=1, padx=10, pady=5, sticky="w")
        ttk.Button(dfs_frame, text="-1", command=lambda: self.edit_date_font(-1)).pack(side="left")
        self.lbl_date_font = ttk.Label(dfs_frame, text=str(self.settings.get("date_font_size", 10)))
        self.lbl_date_font.pack(side="left")
        ttk.Button(dfs_frame, text="+1", command=lambda: self.edit_date_font(1)).pack(side="left")
        r += 1
    ...
    def text_alpha_scale_changed(self, v):
        # When the value of the text opacity scale widget changes
        try:
            f = max(0.1, min(float(v), 1.0))
            f = round(f, 2)
            self.settings["text_opacity"] = f
            self.update_text_opacity(f)
            self.text_alpha_entry.delete(0, tk.END)
            self.text_alpha_entry.insert(0, str(f))
            self.save_settings()
        except ValueError:
            pass

    def text_alpha_entry_apply(self):
        # Apply the text opacity value from the entry box
        try:
            f = float(self.text_alpha_entry.get())
            f = max(0.1, min(f, 1.0))
            f = round(f, 2)
            self.settings["text_opacity"] = f
            self.update_text_opacity(f)
            self.text_alpha_scale.set(f)
            self.text_alpha_entry.delete(0, tk.END)
            self.text_alpha_entry.insert(0, str(f))
            self.save_settings()
        except ValueError:
    ...
    def update_text_opacity(self, opacity):
        # Update text opacity for all text elements
        try:
            # Try to set alpha attribute if supported
            self.time_label.config(alpha=opacity)
            self.date_label.config(alpha=opacity)
            self.pin_button.config(alpha=opacity)
            self.close_button.config(alpha=opacity)
        except:
            # Fallback: use fg color with opacity by adjusting color
            pass
    ...
    def set_show_date(self, value):
        self.settings["show_date"] = value
        self.save_settings()
        self.update_time()

    def set_show_weekday(self, value):
        self.settings["show_weekday"] = value
        self.save_settings()
        self.update_time()

    def edit_date_font(self, n):
        # Edit the date font size
        self.settings["date_font_size"] = max(6, min(72, self.settings.get("date_font_size", 10) + n))
        self.lbl_date_font.config(text=str(self.settings["date_font_size"]))
        self.date_label.config(font=(self.settings["font_family"], self.settings["date_font_size"]))
        self.save_settings()
    ...
    def pick_bg_color(self):
        ...
        if c:
            ...
            for widget in [self.floating_window, self.main_container, self.time_label, self.date_label, self.pin_button, self.close_button]:
                widget.config(bg=c)
            self.save_settings()
    ...
    def pick_text_color(self):
        ...
        if c:
            ...
            for widget in [self.time_label, self.date_label, self.pin_button, self.close_button]:
                widget.config(fg=c)
            self.save_settings()
    ...
    def change_font(self, f):
        ...
        self.date_label.config(font=(f, self.settings.get("date_font_size", 10)))
    ...
    def min_label_w(self):
        ...
        # Add date and weekday to the test
        if self.settings.get("show_date", True):
            test_text += "\n2024-01-01"
        if self.settings.get("show_weekday", True):
            test_text += " Monday"
    ...
    def update_time(self, online=False):
        ...
        date_text = ""
        if self.settings.get("show_date", True):
            date_text += now.strftime(self.settings.get("date_format", "%Y-%m-%d"))
        if self.settings.get("show_weekday", True):
            if date_text:
                date_text += " "
            date_text += now.strftime(self.settings.get("weekday_format", "%A"))
