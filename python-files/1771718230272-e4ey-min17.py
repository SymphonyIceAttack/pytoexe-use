...
def search_trade(event=None):
    ...
    if not text or ":" not in text:
        messagebox.showerror("Error","Enter MM:SS (minutes:seconds)")
        return
    try:
    ...
    if not found:
        output.config(state="normal")
        output.delete("1.0", tk.END)
        output.insert(tk.END, f"{target}\nNo matching trade group\n")
        output.config(state="disabled")
...
def clear_output():
    output.config(state="normal")
    output.delete("1.0", tk.END)
    output.insert(tk.END, entry_minutes.get() + "\n")  # ':' يظل ثابت
    output.config(state="disabled")
...
def toggle_live():
    ...
    if live_mode:
        live_mode = False
        btn_live.config(text="⏱ Live")
    else:
        live_mode = True
        now = datetime.now()
        m = now.minute
        s = 0 if now.second<30 else 30
        entry_minutes.delete(0,tk.END)
        entry_minutes.insert(0,f"{m}:{s}")
        search_trade()
        live_mode = False
        btn_live.config(text="⏱ Live")
...
entry_minutes.insert(0,"0:00")

output = tk.Text(root,font=main_font, height=8)
output.pack(expand=True, fill="both", padx=10, pady=10)
output.config(state="normal")
output.insert(tk.END,"0:00\n")
output.config(state="disabled")

# أزرار تحت مربع الإدخال، أصغر 50%
button_font = tkfont.Font(family=current_font, size=int(BASE_FONT*0.7))

btn_generate = tk.Button(root,text="Generate", font=button_font, width=10, command=search_trade)
btn_generate.pack(pady=2)

btn_clear = tk.Button(root,text="Clear", font=button_font, width=10, command=clear_output)
btn_clear.pack(pady=2)

btn_prev = tk.Button(root,text="◀", font=button_font, width=5, command=prev_group)
btn_prev.pack(pady=2)

btn_next = tk.Button(root,text="▶", font=button_font, width=5, command=next_group)
btn_next.pack(pady=2)

btn_live = tk.Button(root,text="⏱ Live", font=button_font, width=10, command=toggle_live)
btn_live.pack(pady=2)
