...
LOSS_TRIGGERS = {6,9,12}  # القيم اللي تقدر تشتغل عليها Loss كما طلبت (مثال)
...
def _loss_prep_countdown(sec, skip_target):
    ...
    if sec <= 0:
        stop_loss_prep()
        # بعد الدقيقة نحسب الوقت للوصول للـ skip target من توقيت الكمبيوتر
...
def start_loss_main(skip_target):
    """يحسب الوقت الفعلي المتبقي للوصول إلى skip_target بناءً على ساعة الكمبيوتر،
       ثم يبدأ العد التنازلي لذلك الوقت ويعرضه في loss_main_lbl.
       كما يحدث الثلاث مربعات لتظهر الحالة 'بعدها'."""
    global loss_main_running
    # تحديث الثلاث مربعات إلى القيم المطلوبة أثناء العد
    left = skip_target - 3
    mid = skip_target
    right = skip_target + 3
    left_lbl.config(text=left)
    middle_lbl.config(text=f"{mid} >", bg=ai_bg)  # إبراز أن هذا هو الهدف المؤقت
    right_lbl.config(text=right)

    # حساب الفرق بالنسبة لمقياس الـ minute-mod (مطابقة auto_update لو تستخدم نفس الخريطة)
    now = datetime.now()
    current_mod = (now.hour * 60 + now.minute) % 60
    target_mod = skip_target % 60
    delta_minutes = (target_mod - current_mod) % 60
    # المدة بالثواني حتى نصل للدقيقة التي تحمل target_mod
    # نخصم الثواني الحالية لنحصل على الدقيقة + ثواني صحيحة متبقية
...
def _loss_main_countdown(sec, skip_target):
    ...
    if not loss_main_running:
        return
    if sec <= 0:
        # عند انتهاء تايمر Loss الرئيسي — قم بتعيين الحقل للـ skip_target واستدعاء calculate
        stop_loss_main()
        entry.delete(0, "end")
...
def calculate(event=None):
    ...
    # تأكد من بقاء الخط كما هو (لا نغيّر حجم الخط هنا)
    middle_lbl.config(
    ...
    # ===== AI mode behavior for Next-values (AI-only) =====
    if ai_mode and v in AI_NEXT_VALUES:
        # عرض الرقم ثم علامة أكبر (>) ثم Next تحتها — مع خلفية لبني
        middle_lbl.config(text=f"{v} >\nNext", bg=ai_bg)
        # شغّل التايمر العادي (01:00) إذا لم يكن شغّالاً
        start_timer()

    # ===== Manual special middles (or AI when middle reached) =====
    elif v == m and v in SPECIAL_MIDDLES:
        # هذا القاعدة تعمل سواء في manual او في AI (لو وصّل الـ AI للـ middle)
        stop_timer()
...
# ================= LOSS action (menu) =================
def loss_action():
    """تنفيذ عملية Loss كما وصفت: تعمل prep minute ثم تحسب الوقت للوصول للـ skip_target وتبدأ عدًّا مستقلاً."""
    txt = entry.get().strip()
    if not txt.isdigit():
    ...
    # حسبت skip_target على أنه يفوت واحدة ويذهب إلى الرقم الذي بعد التالية (مثال: 6 -> 12)
    skip_target = v + 6
    # أوقف أي تايمرات Loss سابقة ثم ابدأ جديدة
    stop_loss_prep()
    stop_loss_main()
    start_loss_prep(skip_target)
    messagebox.showinfo("Loss", f"Loss started: prep 01:00 — then target {skip_target:02d} (will set entry then).")
...
# ===== timers labels (hidden by default) =====
