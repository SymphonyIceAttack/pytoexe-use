...
def save_to_log(info, usb_output, disk_scan_results, log_file):
    ...
    # Формируем строку с результатами сканирования дисков.
    # Формат: Drive=E:;Status=found;Paths=^...^...;Drive=F:;Status=not_found...
    # Используем ^ как разделитель внутри путей, чтобы не сломать структуру ;
    
    disk_report_parts = []
    for res in disk_scan_results:
        # Собираем пути через ^
        paths_str = "^".join(res["paths"])
        # Экранируем возможные ^ в самих путях (на всякий случай, хотя join уже использует их)
        # Но лучше заменить внутренние ; на ^, если они есть в путях
    ...
    if not usb_devices:
        full_line = f"{sys_info_str};usb_devices=none;disk_report={full_disk_report}\n"
        with open(log_file, "a", encoding="utf-8") as f:
            f.write(full_line)
        return

    # ГЛАВНОЕ ИЗМЕНЕНИЕ:
    # Для КАЖДОГО USB-устройства пишем ОТДЕЛЬНУЮ строку.
    # В каждой строке дублируем отчёт по дискам (так как мы не можем точно сопоставить диск E: с конкретным USB ID без сложных запросов к Win32_DiskDrive).
    # Это дает гарантию: 2 USB -> 2 строки в логе, в каждой полный отчет по дискам.
    
    for parsed in usb_devices:
        usb_part = format_parsed_usb(parsed)
        
        # Строка: SysInfo; USB_Device; DiskReport
...
if __name__ == "__main__":
    try:
        ...
        # Теперь эта функция возвращает структурированные данные по каждому диску
