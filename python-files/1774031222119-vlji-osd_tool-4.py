...
def do_repack_inplace(osd_path):
    ...
    # لو الملف كله أصفار (ما فيه MAGIC) → اكتب من الأوفست 0
    if not orig_blocks and raw_blocks:
        new_bytes = block_to_bytes(txt_to_block(raw_blocks[0]))
        new_sz    = len(new_bytes)
        if new_sz <= len(raw):
            raw[:new_sz] = new_bytes
        else:
            raw = bytearray(new_bytes) + bytearray(max(0, len(raw) - new_sz))
