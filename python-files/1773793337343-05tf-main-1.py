...
class RE4PatcherApp(tk.Tk):
            ...
            if ok:
                ...
                if dependents:
                    ...
                    if messagebox.askyesno(title, msg_cascade):
                        ...
                        # User said No: mark dependents as not-applied so they
                        # show as locked (requires not met) but keep their state
                        # honest — they need re-applying after the base code is re-enabled
                        for dep in dependents:
                            self.applied[dep] = False
                            self.detected[dep] = False
    ...
    OFFSET_MUTEX = {
        ...
        # verdugo versions share same find pattern
