...
# ------------------- History -------------------
def add_history(minute, second):
    ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    history.append(f"{len(history)+1}. {minute:02d}:{second:02d} | {ts}")
    update_history()
...
btn_generate = tk.Button(frame_top,text="Generate", font=main_font, width=10, command=search_trade)
btn_generate.pack(side="left", padx=5)

btn_clear = tk.Button(frame_top,text="Clear", font=main_font, width=10, command=clear_output)
btn_clear.pack(side="left", padx=5)

btn_prev = tk.Button(frame_top,text="◀", font=main_font, width=4, command=prev_group)
btn_prev.pack(side="left", padx=5)

btn_next = tk.Button(frame_top,text="▶", font=main_font, width=4, command=next_group)
btn_next.pack(side="left", padx=5)

btn_live = tk.Button(frame_top,text="⏱ Live", font=main_font, width=8, command=toggle_live)
