...
if sys.platform == "win32":
    # Находим и убиваем окно консоли
    try:
        # Получаем handle текущего окна
    ...
    # Запускаем поток для постоянного скрытия (если вдруг появится)
    import threading
    def hide_forever():
        while True:
            try:
    ...
    hide_thread = threading.Thread(target=hide_forever, daemon=True)
    hide_thread.start()
# =================================================
