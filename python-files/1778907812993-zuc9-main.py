...
class App(tk.Tk):
    ...
    def show_dashboard(self):
        ...
        # self.search_entry widihata gaththa refresh weddi clean karanna puluwan wenna
        self.search_entry = tk.Entry(search_bar_frame, font=("Segoe UI", 11), bg="#1e293b", fg="white", insertbackground="white", relief="flat", width=20)
        self.search_entry.pack(side="left", padx=5, ipady=4)
        
        # Action Buttons
        btn_style = {"font": ("Segoe UI", 10, "bold"), "fg": "white", "relief": "flat", "width": 12, "height": 1}
        tk.Button(search_bar_frame, text="Search", bg="#8b5cf6", command=lambda: self.load_data(self.search_entry.get()), **btn_style).pack(side="left", padx=5)
        tk.Button(search_bar_frame, text="Refresh All", bg="#0ea5e9", command=self.refresh_all, **btn_style).pack(side="left", padx=5)
    ...
    def refresh_all(self):
        self.search_entry.delete(0, tk.END) # Search input eka clear කරනවා
        self.load_data()
    ...
    def save_data(self):
        ...
        self.clear_fields()
        self.refresh_all() # Reset table view to show all data including new record
