# -*- coding: utf-8 -*-
"""
Fastener Assembly TXT Generator v5
Generates Bocad-format fastener assembly input files based on
bolt size, connection thickness, and bolt length.

v5 changes:
  - Connection thickness input semantics: T_input = T_conn + t_w (center-to-center)
  - GUI localized to Chinese (TXT output remains English)
...
def calc_z_positions(bolt_size, bolt_length, t_input):
    ...
    t_input = center-to-center distance between the two flat washers
            = T_conn + t_w  (actual connection thickness + one washer thickness)

    Stacking from -Z to +Z:
      bolt head -> flat washer 1 -> [T_conn connection] -> flat washer 2
      -> spring washer -> nut

    Z coordinate rules (strictly per m20x130-95.5.txt reference):
      Flat washer 1 center = -L + t_w/2  (relative to bolt tip at +Z)
      Flat washer 2 center = FW1 center + t_input
    ...
    # Flat washer 2: nut side, t_input distance from FW1 (center to center)
    # FW2 center = FW1 center + t_input
    fw2_center_z = fw1_center_z + t_input
    ...
    # Actual connection thickness T_conn = t_input - fw_h
    t_conn = t_input - fw_h
...
def generate_fastener_txt(bolt_size, bolt_length, t_input):
    ...
    t_input: center-to-center distance between flat washers
             = T_conn + t_w (actual connection thickness + one washer thickness)
    """
    bolt = BOLT_DATA[bolt_size]
...
class BoltGeneratorApp:
    ...
    def create_widgets(self):
        ...
        # Connection thickness (T_input = center-to-center between washers)
        ttk.Label(main_frame, text="连接件厚度（中心距）：").grid(
    ...
    def update_info(self):
        ...
        # Add T_conn if thickness is entered
        ...
        if t_str:
            try:
                ...
                if t_input > 0:
                    t_conn = t_input - fw['h']
                    info += f"\n实际连接件厚度 T_conn = {t_input} - {fw['h']} = {t_conn}mm"
            except ValueError:
                pass
    ...
    def generate_fastener(self):
        ...
        t_str = self.t_thickness_var.get().strip()
        if not t_str:
            messagebox.showwarning("警告", "请输入连接件厚度（中心距）")
            return
        ...
        # Default filename
        default_name = f"{bolt_size}_{t_input}.txt"
        ...
        try:
            ...
            messagebox.showinfo("成功",
                ...
                f"连接件厚度（中心距）：{t_input} mm\n"
                f"实际连接件厚度 T_conn：{t_conn} mm")
