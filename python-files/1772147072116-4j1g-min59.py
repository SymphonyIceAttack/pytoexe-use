...
current_group_idx = 0
ai_enabled = False
timer_visible = True  # زر التحكم في ظهور التايمر
ai_after_id = None
timer_after_id = None
...
def update_timer_logic():
    ...
    elif -10 <= diff <= 0: # يظهر GO لمدة 10 ثواني
...
def toggle_timer_visibility():
    ...
    if not timer_visible: timer_label.config(text="")
...
def toggle_mode():
    ...
    root.config(bg=bg)
    for w in [top_frame, control_frame, entry, output, timer_label, btn_gen, btn_clr, btn_pre, btn_nxt]:
        try: w.config(bg=bg, fg=fg)
        except: pass
    draw_ai_lamp()

def open_scale():
    win = tk.Toplevel(root); win.title("Scale Setting")
    def change(v):
        s = int(float(v))
        f = (current_font, s)
        entry.config(font=(current_font, s+6))
        for b in [btn_gen, btn_clr, btn_pre, btn_nxt]: b.config(font=(current_font, int(s*0.75)))
        output.config(font=f)
        timer_label.config(font=(current_font, int(s*0.9), "bold"))
        # تعديل حجم البرنامج تلقائياً مع السكيل
...
# التايمر الأحمر (أصغر قليلاً وتحت الأزرار مباشرة)
...
view_m.add_command(label="Day/Night", command=toggle_mode)
...
ai_m.add_separator()
ai_m.add_command(label="Timer/GO ON-OFF", command=toggle_timer_visibility) # زر التحكم في التايمر
