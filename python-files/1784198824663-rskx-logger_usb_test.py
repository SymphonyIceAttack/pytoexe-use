...
BASE_LOG_PATH = r"C:\Logs"
HOSTNAME = socket.gethostname()
# Имя файла теперь зависит от имени ПК: C:\Logs\DESKTOP-G0C6PK7.log
LOG_FILE = os.path.join(BASE_LOG_PATH, f"{HOSTNAME}.log")
...
def log_error(message):
    try:
        ...
        # Для ошибок тоже используем файл под имя ПК
        error_file = os.path.join(BASE_LOG_PATH, f"{HOSTNAME}_errors.log")
...
def save_to_log(info, usb_output, android_results, log_file):
    ...
    with open(log_file, "a", encoding="utf-8") as f:
        f.write("=" * 60 + "\n")
        f.write(f"LOG ENTRY: {info['timestamp']}\n")
        f.write("=" * 60 + "\n\n")
        
        f.write("--- SYSTEM INFO (SHORT) ---\n")
