...
def display_group(idx):
    ...
    # تحديث البطاقات بدلاً من صندوق النص
...
def open_theme_picker():
    ...
    pop.title("Themes")
    pop.geometry("220x250")
    pop.grab_set()
    for tn in THEMES.keys():
        tk.Button(pop, text=tn, width=15, command=lambda n=tn: [apply_theme(n), pop.destroy()]).pack(pady=5)
...
def open_scale():
    ...
    def change(v):
        ...
        for c in [card1, card2, card3]: c.config(font=("Segoe UI", s, "bold"))
        timer_label.config(font=("Segoe UI", int(s*0.9), "bold"))
        root.geometry(f"{int(420 * (s/14))}x{int(480 * (s/14))}")
    tk.Scale(win, from_=10, to=28, orient="horizontal", command=change).pack(padx=20, pady=20)

# ------------------- Main UI -------------------
root = tk.Tk()
root.title("Premium Trade Bot")
root.geometry("420x480")
root.attributes("-topmost", True)

top_f = tk.Frame(root, pady=10)
top_f.pack()
entry = tk.Entry(top_f, justify="center", font=("Segoe UI", 22), width=10, relief="flat", bd=2)
entry.pack()
entry.insert(0, "0")
entry.bind("<Return>", on_generate) # تصليح المانيوال
...
# --- منطقة العرض الجديدة (Cards) ---
display_f = tk.Frame(root, pady=10)
display_f.pack(expand=True, fill="both", padx=20)

card1 = tk.Label(display_f, text="", font=("Segoe UI", 15, "bold"), pady=10, relief="groove", bd=1)
card1.pack(fill="x", pady=3)
card2 = tk.Label(display_f, text="", font=("Segoe UI", 15, "bold"), pady=10, relief="groove", bd=1)
card2.pack(fill="x", pady=3)
card3 = tk.Label(display_f, text="", font=("Segoe UI", 15, "bold"), pady=10, relief="groove", bd=1)
card3.pack(fill="x", pady=3)
...
view_m.add_command(label="Change Design UI", command=open_theme_picker)
view_m.add_command(label="Scale View", command=open_scale)
menubar.add_cascade(label="View", menu=view_m)

ai_m = tk.Menu(menubar, tearoff=0)
ai_m.add_command(label="AI Switch", command=toggle_ai)
ai_m.add_command(label="Timer Visibility", command=lambda: [globals().update(timer_visible=not timer_visible)])
