...
class CoinTossGame:
    def __init__(self, root):
        ...
        self.save_file = "coin_toss_save.json"
        
        # Настройка интерфейса
        self.setup_ui()
        
        # Загружаем сохраненный прогресс при запуске
        self.load_progress()
    ...
    def update_buttons_for_mode(self):
        ...
        if self.mode == "minute":
            # В режиме минуты: пауза и сброс не нужны
    ...
    def change_mode(self):
        ...
        if self.mode == "minute":
            ...
            # Загружаем сохраненный прогресс при смене на бесконечный режим
            self.load_progress()
    ...
    def animate_toss(self):
        ...
        # Сохраняем прогресс каждые 100 итераций (только для бесконечного режима)
        if self.mode == "unlimited" and (self.heads_count + self.tails_count) % 100 == 0:
            self.save_progress()
    ...
    def start_or_stop_toss(self):
        """Универсальная кнопка: старт или завершение"""
        if self.is_running:
    ...
    def start_toss(self):
        ...
        if self.mode == "minute":
            # В режиме минуты: пауза и сброс неактивны
            self.pause_button.config(state="disabled")
            self.reset_button.config(state="disabled")
        else:
            # В бесконечном режиме активируем паузу, сброс оставляем активным
    ...
    def pause_toss(self):
        ...
        # Меняем текст кнопки старт обратно на СТАРТ (для возобновления)
        self.start_button.config(text="СТАРТ")
    ...
    def stop_toss(self):
        ...
        # Обновляем таймер паузы
        if self.pause_start_time > 0:
            self.total_paused_time += time.time() - self.pause_start_time
        
        # Сохраняем финальный результат только для бесконечного режима
        ...
        # В режиме минуты кнопка сброса остается неактивной
        if self.mode == "minute":
            self.reset_button.config(state="disabled")
        else:
            self.reset_button.config(state="normal")
    ...
    def save_progress(self):
        """Сохраняет прогресс в файл (только для бесконечного режима)"""
        if self.mode == "unlimited" and self.is_running and not self.is_paused:
            elapsed = time.time() - self.start_time - self.total_paused_time
            data = {
    ...
    def load_progress(self):
        """Загружает сохраненный прогресс (только для бесконечного режима)"""
        if self.mode == "unlimited" and os.path.exists(self.save_file):
            try:
                ...
                answer = messagebox.askyesno(
                    "Загрузить прогресс",
                    f"Найден сохраненный прогресс:\n"
                    f"Орлов: {data['heads']}\n"
                    f"Решек: {data['tails']}\n"
                    f"Время: {str(timedelta(seconds=int(data['elapsed_time'])))}\n"
                    f"Сохранено: {data['last_save']}\n\n"
                    f"Загрузить?"
                )
