...
import time
import matplotlib
matplotlib.use("TkAgg")  # NECESARIO PARA .EXE
    ...
    try:
        ...
        while i < PasosTotal:
            ...
            if linea == "BACK":
                ...
                dfE.to_csv(f"{ruta}\\{nombre_csv}_Background.csv", index=True)
                continue
        ...
        label_progreso.config(text=f"Error puerto: {e}")
        return
    finally:
        btn.config(bg="red")
...
def Plots(polarizador,grados,mediciones,errores,Progreso,d1, d2, d3, d4, nombre_grafica, nombre_csv, ruta, label_progreso,):
    ...
    # Gráficas
    plt.plot(ang1, mediciones1, label='Pol: 0',color = colores[0])
    plt.plot(ang2, mediciones2, label='Pol: 45',color = colores[1])
    plt.plot(ang3, mediciones3, label='Pol: 90',color = colores[2])
    plt.plot(ang4, mediciones4, label='Pol: 135',color = colores[3])
    plt.legend()
    plt.grid(True)
    ...
    for ax, data, ang, col in zip(axs.flat, datos, angulos, colores):
        ax.set_yscale('log')
        ax.scatter(ang1, data, marker = '+', color=col)
    ...
    # Proceso paso a paso
    plt.scatter(Progreso, mediciones, marker = '+')
    plt.grid(True)
    plt.title('Detección')
    plt.xlabel('Pasos')
    plt.ylabel('Intensidad')
    ...
    df1 = pd.DataFrame({
        'Ángulo': ang1,
        'Detector': mediciones1,
        'Error': errores1
    })
    
    df1.to_csv(f"{ruta}\\{nombre_csv}_Vertical.csv", index=True)
    
    # Guardar csv
    df2 = pd.DataFrame({
        'Ángulo': ang2,
        'Detector': mediciones2,
        'Error': errores2
    })
    
    df2.to_csv(f"{ruta}\\{nombre_csv}_CircularDerecha.csv", index=True)
    
    # Guardar csv
    df3 = pd.DataFrame({
        'Ángulo': ang3,
        'Detector': mediciones3,
        'Error': errores3
    })
    
    df3.to_csv(f"{ruta}\\{nombre_csv}_Horizontal.csv", index=True)
    
    # Guardar csv
    df4 = pd.DataFrame({
        'Ángulo': ang4,
        'Detector': mediciones4,
        'Error': errores4
    })
    
    df4.to_csv(f"{ruta}\\{nombre_csv}_CitcularIzquierda.csv", index=True)
