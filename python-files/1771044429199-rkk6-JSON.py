...
# -*- coding: utf-8 -*-
"""
JSON转SRT/TXT字幕转换器 - 独立GUI版本
支持输出带时间轴的SRT字幕或不带时间轴的纯文本TXT文件
"""
...
class JSONToSRTConverter:
    ...
    def batch_convert(self, folder_path, target_format='srt'):
        """
        批量转换文件夹中的所有JSON文件
        target_format: 'srt' 或 'txt'
        """
...
class JSONToSRTGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("JSON 转 SRT/TXT 字幕转换器 v1.1")
    ...
    def setup_ui(self):
        ...
        tk.Label(title_frame, text="JSON 转 SRT/TXT 字幕转换器", 
                font=("微软雅黑", 18, "bold"), bg=bg_color).pack()
        tk.Label(title_frame, text="支持剪映等视频编辑软件的字幕JSON格式 | 可输出SRT或纯文本TXT", 
        ...
        tk.Button(frame1, text="浏览...", command=self.browse_json,
                 ...
                 relief=tk.RAISED).grid(row=0, column=3, padx=5, pady=5)
        
        # 输出文件选择
        tk.Label(frame1, text="输出文件:", bg=bg_color,
                font=("微软雅黑", 10)).grid(row=1, column=0, sticky=tk.W, pady=5)
        
        self.output_path_var = tk.StringVar()
        output_entry = tk.Entry(frame1, textvariable=self.output_path_var,
                               width=50, font=("微软雅黑", 9))
        output_entry.grid(row=1, column=1, padx=5, pady=5, columnspan=2)
        
        tk.Button(frame1, text="浏览...", command=self.browse_output,
                 font=("微软雅黑", 9), bg="#2196F3", fg="white",
                 relief=tk.RAISED).grid(row=1, column=3, padx=5, pady=5)
        
        # 两个转换按钮：转SRT 和 转TXT
        btn_frame = tk.Frame(frame1, bg=bg_color)
        btn_frame.grid(row=2, column=0, columnspan=4, pady=15)
        
        tk.Button(btn_frame, text="转 SRT", command=self.convert_single_to_srt,
                 font=("微软雅黑", 11, "bold"), bg=button_color, fg="white",
                 padx=20, pady=5, relief=tk.RAISED).pack(side=tk.LEFT, padx=10)
        
        tk.Button(btn_frame, text="转 TXT", command=self.convert_single_to_txt,
        ...
        folder_entry = tk.Entry(frame2, textvariable=self.folder_path_var,
                               width=50, font=("微软雅黑", 9))
        folder_entry.grid(row=0, column=1, padx=5, pady=5, columnspan=2)
        ...
        # 批量转换按钮
        batch_btn_frame = tk.Frame(frame2, bg=bg_color)
        batch_btn_frame.grid(row=1, column=0, columnspan=4, pady=15)
        
        tk.Button(batch_btn_frame, text="批量转 SRT", command=self.batch_convert_to_srt,
                 font=("微软雅黑", 11, "bold"), bg=button_color, fg="white",
                 padx=20, pady=5, relief=tk.RAISED).pack(side=tk.LEFT, padx=10)
        
        tk.Button(batch_btn_frame, text="批量转 TXT", command=self.batch_convert_to_txt,
                 font=("微软雅黑", 11, "bold"), bg="#FF9800", fg="white",
                 padx=20, pady=5, relief=tk.RAISED).pack(side=tk.LEFT, padx=10)
        ...
        tk.Label(bottom_frame, text="© 2025 JSON转SRT/TXT转换器 | 支持剪映、PR等软件字幕导出",
    ...
    def browse_json(self):
        ...
        if filename:
            ...
            self.output_path_var.set(base_name + '.srt')  # 默认SRT，用户可改
    ...
    def convert_single_to_srt(self):
        """单文件转SRT"""
    ...
    def convert_single_to_txt(self):
        """单文件转TXT"""
        json_path = self.json_path_var.get().strip()
        output_path = self.output_path_var.get().strip()
    ...
    def batch_convert_to_srt(self):
        ...
        self.clear_log()
        self.log_message("=" * 50)
        self.log_message(f"开始批量转换SRT: {folder_path}")
        self.log_message("=" * 50)
        
        try:
            success, message = self.converter.batch_convert(folder_path, target_format='srt')
            if success:
                self.log_message("\n✅ " + message)
    ...
    def batch_convert_to_txt(self):
        ...
        self.clear_log()
        self.log_message("=" * 50)
        self.log_message(f"开始批量转换TXT: {folder_path}")
        self.log_message("=" * 50)
        
        try:
            success, message = self.converter.batch_convert(folder_path, target_format='txt')
            if success:
                self.log_message("\n✅ " + message)
