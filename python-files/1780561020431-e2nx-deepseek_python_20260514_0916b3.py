...
# ----------------------------------------------------------------------
# Класс для управления пользовательскими шаблонами с сохранением в папке рядом с программой
# ----------------------------------------------------------------------
class TemplateManager:
    def __init__(self):
        self.arshin_func = None
        self.accreditation_func = None
        self.arshin_code = None
        self.accred_code = None
        self.template_dir = self.get_template_dir()
        self.template_file = os.path.join(self.template_dir, "templates.json")
        self.load_defaults()
    ...
    def load_defaults(self):
        ...
        try:
            ...
            self.arshin_code = DEFAULT_ARSHIN_XML_TEMPLATE
        ...
        try:
            ...
            self.accred_code = DEFAULT_ACCREDITATION_XML_TEMPLATE
        except Exception as e:
            print(f"Ошибка загрузки шаблона аккредитации: {e}")
            self.accreditation_func = None
    
    def save_templates(self):
        """Сохраняет текущие шаблоны в JSON"""
        data = {
            'arshin_code': self.arshin_code if self.arshin_code else DEFAULT_ARSHIN_XML_TEMPLATE,
            'accreditation_code': self.accred_code if self.accred_code else DEFAULT_ACCREDITATION_XML_TEMPLATE
        }
    ...
    def load_saved_templates(self):
        ...
        try:
            with open(self.template_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
            if 'arshin_code' in data:
                self.update_arshin_template(data['arshin_code'])
            if 'accreditation_code' in data:
                self.update_accreditation_template(data['accreditation_code'])
        except Exception as e:
    ...
    def restore_arshin(self):
        return self.update_arshin_template(DEFAULT_ARSHIN_XML_TEMPLATE)
    
    def restore_accreditation(self):
        return self.update_accreditation_template(DEFAULT_ACCREDITATION_XML_TEMPLATE)

# ----------------------------------------------------------------------
# GUI-класс с улучшенным стилем и всеми доработками
# ----------------------------------------------------------------------
class App:
    TEMPLATE_PASSWORD = "520593"
    ...
    def __init__(self, root):
        ...
        # Очистка буфера обмена при старте для избежания проблем
        try:
    ...
    def create_header(self):
        ...
        dev_label = tk.Label(header, text="Разработчик: Омельченко Алексей | Версия 4.0",
                             font=("Segoe UI", 9), bg="#2c3e50", fg="#bdc3c7")
        dev_label.place(relx=0.95, y=25, anchor="ne")
    
    def ask_password(self) -> bool:
        pwd = simpledialog.askstring("Пароль", "Введите пароль для редактирования шаблона:", show='*', parent=self.root)
        return pwd == self.TEMPLATE_PASSWORD

    # ---------- Вставка из буфера обмена для любой таблицы (улучшенная версия) ----------
    def paste_from_clipboard(self, tree, columns):
        """Вставляет данные из буфера обмена с тройным механизмом получения"""
        clipboard_text = None
        
        # Способ 1: прямая попытка
        try:
            clipboard_text = self.root.clipboard_get()
        except:
            pass
        
        # Способ 2: через обновление окна (решает проблему "залипания")
        ...
        # Способ 3: через временное окно (самый надёжный)
        if not clipboard_text:
            try:
                temp = tk.Tk()
                temp.withdraw()
                temp.update()
                clipboard_text = temp.clipboard_get()
                temp.destroy()
            except:
                pass
        
        if not clipboard_text:
            self.status_var.set("Буфер обмена пуст. Попробуйте скопировать данные заново (Ctrl+C в Excel).")
            messagebox.showwarning("Вставка", 
                ...
                "4. Нажмите Ctrl+V\n\n"
                "Совет: Если не работает, попробуйте скопировать в Блокнот, а из Блокнота — сюда.")
            return
        ...
        if not rows_data:
            self.status_var.set("Не удалось разобрать данные из буфера обмена")
            return
        
        # Определяем позицию вставки
        selected = tree.selection()
        if not selected:
            current_row_index = len(tree.get_children())
            current_col = 0
        else:
            first_selected = selected[0]
            current_row_index = tree.index(first_selected)
            current_col = 0
        ...
        for i, row_cells in enumerate(rows_data):
            target_row_idx = current_row_index + i
            if target_row_idx >= total_rows:
                new_values = [''] * len(columns)
                tree.insert("", tk.END, values=new_values)
                total_rows += 1
                target_item = tree.get_children()[target_row_idx]
            else:
                target_item = all_items[target_row_idx]
            
            current_vals = list(tree.item(target_item, 'values'))
            for j, cell in enumerate(row_cells):
                col_idx = current_col + j
                if col_idx < len(current_vals):
                    current_vals[col_idx] = cell
            tree.item(target_item, values=current_vals)
        
        # Обновление цветов
        if tree == self.arshin_tree:
            self.recolor_arshin_rows()
        else:
            self.recolor_accred_rows()
        
        self.status_var.set(f"Вставлено {len(rows_data)} строк из буфера обмена")
    ...
    def init_arshin_tab(self):
        ...
        self.gen_button_arshin = ttk.Button(btn_frame, text="⚡ Сгенерировать XML", command=self.arshin_generate_xml,
                                            style="Primary.TButton")
        self.gen_button_arshin.pack(side=tk.LEFT, padx=2)
        ...
        ttk.Button(btn_frame, text="🔄 Восстановить шаблон", command=self.arshin_restore_template,
                   style="Warning.TButton").pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame, text="📤 Экспорт шаблона", command=self.arshin_export_template,
                   style="Primary.TButton").pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame, text="📥 Импорт шаблона", command=self.arshin_import_template,
                   style="Primary.TButton").pack(side=tk.LEFT, padx=2)
        ...
        self.arshin_tree.bind('<Control-v>', lambda e: self.paste_from_clipboard(self.arshin_tree, self.arshin_columns))
        self.arshin_tree.bind('<Command-v>', lambda e: self.paste_from_clipboard(self.arshin_tree, self.arshin_columns))
    ...
    def validate_arshin_row(self, values, row_num):
        errors = []
        required_fields = [0,1,2,4,5,6,8,10,11,12]
        for idx in required_fields:
            if not values[idx]:
                errors.append(f"Строка {row_num}: {self.arshin_columns[idx]} обязательно для заполнения")
        if values[2] and (not values[2].isdigit() or len(values[2]) != 4):
            errors.append(f"Строка {row_num}: Год должен быть 4 цифрами")
        for date_idx, name in [(6, "Дата поверки"), (7, "Дата действия")]:
            if values[date_idx]:
                try:
                    datetime.strptime(values[date_idx], "%d.%m.%Y")
                except:
                    errors.append(f"Строка {row_num}: {name} в неверном формате (дд.мм.гггг)")
        if values[8] not in ('пригодно', 'непригодно'):
            errors.append(f"Строка {row_num}: Пригодность должна быть 'пригодно' или 'непригодно'")
        return errors
    ...
    def arshin_generate_xml(self):
        ...
        for i, row in enumerate(data_rows):
            errors = self.validate_arshin_row(row, i+1)
            all_errors.extend(errors)
        if all_errors:
            answer = messagebox.askyesno("Ошибки в данных", f"Найдено {len(all_errors)} ошибок.\nПоказать первые 5?\n\nНажмите 'Да' для просмотра, 'Нет' для продолжения генерации (проблемные строки будут пропущены).")
            if answer:
                messagebox.showinfo("Ошибки", "\n".join(all_errors[:10]))
            # Продолжаем генерацию, но с предупреждением
        
        self.gen_button_arshin.config(state=tk.DISABLED, text="⏳ Генерация...")
        self.status_var.set("Генерация XML для Аршин... (может занять время)")
        ...
        def generate():
            try:
                ...
                self.result_queue.put(('success', xml_str))
            except Exception as e:
                self.result_queue.put(('error', str(e) + "\n" + traceback.format_exc()))
        
        thread = threading.Thread(target=generate, daemon=True)
        thread.start()
        self.check_arshin_generation_result()
    
    def check_arshin_generation_result(self):
        try:
            status, data = self.result_queue.get_nowait()
            if status == 'success':
                self.arshin_xml_text.delete(1.0, tk.END)
                self.arshin_xml_text.insert(tk.END, data)
                self.status_var.set("XML для Аршин успешно сгенерирован")
            else:
                messagebox.showerror("Ошибка генерации", data)
                self.status_var.set("Ошибка генерации XML")
        except:
            self.root.after(100, self.check_arshin_generation_result)
            return
        self.gen_button_arshin.config(state=tk.NORMAL, text="⚡ Сгенерировать XML")
    ...
    def arshin_edit_template(self):
        if self.ask_password():
            self.open_template_editor("Аршин", self.tm.arshin_code or DEFAULT_ARSHIN_XML_TEMPLATE, self.tm.update_arshin_template)
    ...
    def arshin_export_template(self):
        filepath = filedialog.asksaveasfilename(defaultextension=".py", filetypes=[("Python files", "*.py")])
        if filepath:
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(self.tm.arshin_code if self.tm.arshin_code else DEFAULT_ARSHIN_XML_TEMPLATE)
            self.status_var.set(f"Шаблон Аршин экспортирован в {os.path.basename(filepath)}")
    
    def arshin_import_template(self):
        filepath = filedialog.askopenfilename(filetypes=[("Python files", "*.py")])
        if filepath:
            with open(filepath, 'r', encoding='utf-8') as f:
                code = f.read()
            success, msg = self.tm.update_arshin_template(code)
            if success:
                messagebox.showinfo("Успех", "Шаблон успешно импортирован и применён.")
                self.status_var.set("Шаблон Аршин импортирован")
            else:
                messagebox.showerror("Ошибка", msg)
    ...
    def init_accreditation_tab(self):
        ...
        ttk.Button(btn_frame, text="📂 Загрузить CSV", command=self.accred_load_csv,
                   style="Success.TButton").pack(side=tk.LEFT, padx=2)
        self.gen_button_accred = ttk.Button(btn_frame, text="⚡ Сгенерировать XML", command=self.accred_generate_xml,
                                            style="Primary.TButton")
        self.gen_button_accred.pack(side=tk.LEFT, padx=2)
        ...
        ttk.Button(btn_frame, text="🔄 Восстановить шаблон", command=self.accred_restore_template,
                   style="Warning.TButton").pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame, text="📤 Экспорт шаблона", command=self.accred_export_template,
                   style="Primary.TButton").pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame, text="📥 Импорт шаблона", command=self.accred_import_template,
                   style="Primary.TButton").pack(side=tk.LEFT, padx=2)
        ...
        self.accred_tree.bind('<Control-v>', lambda e: self.paste_from_clipboard(self.accred_tree, self.accred_columns))
        self.accred_tree.bind('<Command-v>', lambda e: self.paste_from_clipboard(self.accred_tree, self.accred_columns))
    ...
    def accred_generate_xml(self):
        ...
        self.gen_button_accred.config(state=tk.DISABLED, text="⏳ Генерация...")
        self.status_var.set("Генерация XML для аккредитации... (может занять время)")
        self.root.config(cursor="watch")
        self.result_queue = Queue()
        ...
        thread = threading.Thread(target=generate, daemon=True)
        thread.start()
        self.check_accred_generation_result()
    
    def check_accred_generation_result(self):
        try:
            status, data = self.result_queue.get_nowait()
            if status == 'success':
                self.accred_xml_text.delete(1.0, tk.END)
                self.accred_xml_text.insert(tk.END, data)
                self.status_var.set("XML для аккредитации успешно сгенерирован")
            else:
                messagebox.showerror("Ошибка генерации", data)
                self.status_var.set("Ошибка генерации XML")
        except:
            self.root.after(100, self.check_accred_generation_result)
            return
        self.gen_button_accred.config(state=tk.NORMAL, text="⚡ Сгенерировать XML")
        self.root.config(cursor="")
    ...
    def accred_edit_template(self):
        if self.ask_password():
            self.open_template_editor("аккредитации", self.tm.accred_code or DEFAULT_ACCREDITATION_XML_TEMPLATE, self.tm.update_accreditation_template)
    ...
    def accred_export_template(self):
        filepath = filedialog.asksaveasfilename(defaultextension=".py", filetypes=[("Python files", "*.py")])
        if filepath:
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(self.tm.accred_code if self.tm.accred_code else DEFAULT_ACCREDITATION_XML_TEMPLATE)
            self.status_var.set(f"Шаблон аккредитации экспортирован в {os.path.basename(filepath)}")
    
    def accred_import_template(self):
        filepath = filedialog.askopenfilename(filetypes=[("Python files", "*.py")])
        if filepath:
            with open(filepath, 'r', encoding='utf-8') as f:
                code = f.read()
            success, msg = self.tm.update_accreditation_template(code)
            if success:
                messagebox.showinfo("Успех", "Шаблон успешно импортирован и применён.")
                self.status_var.set("Шаблон аккредитации импортирован")
            else:
                messagebox.showerror("Ошибка", msg)
