...
class CodeRow(tk.Frame):
    ...
    def _build(self):
        ...
        is_dropdown = self.code.get("dialog") in ("dropdown", "punisher_pierce",
                                                   "r11c_cabin", "luis_cabin",
                                                   "drawn_enemies_cam", "custom_ces",
                                                   "bgm_files", "link_tweaks",
                                                   "mod_expansion", "memory_alloc")
        ...
        if not is_numeric and not is_dropdown:
...
class RE4PatcherApp(tk.Tk):
    ...
    def _dialog_memory_alloc(self, code_id):
        ...
        make_label(dlg,
                   ...
                   fg=MUTED, bg="#111", font=FONT_TINY).pack(pady=(0, 8))
    ...
    def _dialog_punisher_pierce(self, code_id):
        """Two separate numeric inputs side by side for punisher pierce count."""
        exe = self.exe_path.get().strip()
        if not exe or not os.path.isfile(exe):
            messagebox.showerror("Error", "Please select a valid bio4.exe first.")
            return
        ...
        row_frame = tk.Frame(dlg, bg="#111")
        row_frame.pack(padx=20)

        for field in fields:
            lbl    = field.get("label_en" if CURRENT_LANG == "en" else "label_ar", "")
            default = field.get("default_dec", 0)

            # read current value from EXE
            try:
                off = int(field["offset"], 16)
                with open(exe, "rb") as f:
                    f.seek(off + 1)  # skip B8 prefix byte
                    raw = f.read(field["byte_count"] - 1)
                current = int.from_bytes(raw, byteorder="little")
            except Exception:
                current = default

            col = tk.Frame(row_frame, bg="#111")
            col.pack(side="left", padx=16)

            make_label(col, lbl, fg=TEXT_DIM, bg="#111",
                       font=FONT_TINY, anchor="w").pack(anchor="w", pady=(0, 4))

            input_row = tk.Frame(col, bg="#111")
            input_row.pack()

            var = tk.StringVar(value=str(current))
            e = tk.Entry(input_row, textvariable=var, font=FONT_NORMAL,
                         fg=ACCENT2, bg="#1a1a1a", insertbackground=ACCENT2,
                         relief="flat", bd=0,
                         highlightthickness=1, highlightbackground=BORDER,
            ...
            def make_apply(v=var, f=field):
                def do():
                    try:
                        ...
                        return
                    hex_bytes = bytes([0xB8]) + val.to_bytes(f["byte_count"] - 1, "little")
                    try:
                        off = int(f["offset"], 16)
                        with open(exe, "r+b") as fh:
                            fh.seek(off)
                            orig = fh.read(f["byte_count"])
                            backup = load_patch_backup()
                            backup.setdefault(code_id, {})[f["offset"].upper().lstrip("0") or "0"] = orig.hex().upper()
                            save_patch_backup(backup)
                            fh.seek(off)
                            fh.write(hex_bytes)
                    except Exception as ex:
                        ...
                        return
                    self.applied[code_id] = True
                    write_log("APPLIED", name + " [" + f.get("label_en","") + "] = " + str(val), exe)
                    if not APP_SETTINGS.get("silent_apply", False):
                        messagebox.showinfo("[+] Applied",
                            f.get("label_en" if CURRENT_LANG == "en" else "label_ar","") + " = " + str(val))
                    self._after_state_change()
                return do

            tk.Button(input_row, text="Apply" if CURRENT_LANG == "en" else "تطبيق",
                      font=FONT_SMALL, fg="#ffffff", bg="#2255cc",
                      activeforeground="#ffffff", activebackground="#1a44bb",
                      relief="flat", bd=0, cursor="hand2",
                      highlightthickness=0,
                      command=make_apply(), padx=14, pady=5
                      ).pack(side="left")

        make_button(dlg, "Close" if CURRENT_LANG == "en" else "إغلاق",
                    dlg.destroy, fg=MUTED, bg="#1a1a1a",
                    active_bg="#2a2a2a", width=8).pack(pady=16)
