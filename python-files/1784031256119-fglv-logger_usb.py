...
def get_usb_devices():
    try:
        # Убрали "where Name like '%USB%'" — теперь берём все PnP-устройства
        cmd = [
            "wmic",
        ...
        if not output:
            return "Нет данных об устройствах или команда вернула пустой результат."
        return output
    except FileNotFoundError:
        return "Ошибка: утилита wmic не найдена (возможно, не Windows)."
    except subprocess.TimeoutExpired:
        return "Ошибка: превышение времени ожидания при сборе данных."
...
def save_to_log(system_info, usb_info, log_file):
    ...
    with open(log_file, "a", encoding="utf-8") as f:
        ...
        f.write("\n" + "=" * 60 + "\n")
        f.write("ИНФОРМАЦИЯ О ПОДКЛЮЧЁННЫХ УСТРОЙСТВАХ (включая USB)\n")
        f.write("=" * 60 + "\n")
        f.write(usb_info)
        f.write("\n\n")
