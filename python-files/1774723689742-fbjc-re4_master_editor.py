...
_QUICK_DETECT = {
    "aev_osd": {
        "replace": bytes.fromhex("E9892E00009090909090E84850D4".replace(" ","")),
    },
    "aev_option": {
        "replace": bytes.fromhex("E931FFFFFFE9BF5BCFFFFF".replace(" ","")),
    },
}
    ...
    def _save(self):
        ...
        for n, (row_vars, entry) in enumerate(
            ...
            if n < len(entry_positions):
                ...
                # append new entry — build from template
                try:
                    tmpl_hex = AVL_ENTRY_BYTES.replace("XX", "00")
                    new_bytes = bytearray(bytes.fromhex(tmpl_hex))
                    for i, off in enumerate(offsets):
                        if off < len(new_bytes):
                            new_bytes[off] = vals.get(i, 0)
                    data.extend(new_bytes)
                except Exception as ex:
                    messagebox.showerror("Error", f"Failed to append entry: {ex}")
                    return

        # update count byte in all known entry positions
        count = len(self._entries)
        entry_positions2 = []
        pos2 = 0
        while True:
            i2 = data.find(header, pos2)
            if i2 < 0: break
            entry_positions2.append(i2)
            pos2 = i2 + 16
        for ep in entry_positions2:
            if ep + 15 < len(data):
                data[ep + 15] = count
...
class RE4MasterEditor(tk.Tk):
    ...
    def _build_ui(self):
        ...
        for tool in TOOLS_DEF:
            ...
            if tid == "code_manager":
                ...
                # AEV OPTION: create both locked and real panel
                real_aev = AEVOptionPanel(self._content, self)
                if lock:
                    panel = LockedPanel(self._content, label,
                                        lock[0], lock[1], self,
                                        unlocked_panel=real_aev)
                else:
                    panel = real_aev
                self._panels_real = getattr(self, "_panels_real", {})
                self._panels_real[tid] = real_aev
            elif tid == "avl_editor":
    ...
    def _open_settings(self):
        ...
        def apply_lang():
            ...
            dlg.destroy()
            # reload Code Manager UI if open
            cm = getattr(self, "_cm_app", None)
            if cm:
                try:
                    cm._reload_ui()
                except Exception:
            ...
            messagebox.showinfo(
                ...
                "Language updated. Some labels may require reopening the module."
                if CURRENT_LANG == "en" else
                "تم تحديث اللغة. بعض الأقسام قد تحتاج إعادة فتح."
            )
