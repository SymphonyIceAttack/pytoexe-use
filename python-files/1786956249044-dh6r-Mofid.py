...
# ===== MOFID MULTI-POINT CLICK CONTROLLER =====
class MofidMultiPoint:
    """Sequential 1..4 point mouse click controller."""
    MAX_POINTS = 4
    def __init__(self, delay_ms=100):
        self.points = [None] * self.MAX_POINTS
        self.delay_ms = max(0, int(delay_ms))
    def set_delay(self, delay_ms):
        self.delay_ms = max(0, int(delay_ms))
    def register_point(self, index, x=None, y=None):
        import pyautogui
        i = int(index) - 1
        if not 0 <= i < self.MAX_POINTS:
            raise ValueError("Point must be 1..4")
    ...
    def get_point(self, index):
        i = int(index)-1
        if not 0 <= i < self.MAX_POINTS: raise ValueError("Point must be 1..4")
        return self.points[i]
    def click_sequence(self, count=None):
        import time, pyautogui
        count = self.available_count() if count is None else int(count)
        if not 1 <= count <= 4: raise ValueError("Count must be 1..4")
        if any(self.points[i] is None for i in range(count)):
            raise ValueError("Register all selected points first")
        for i in range(count):
            pyautogui.click(*self.points[i])
            if i < count-1:
    ...
    def available_count(self):
        ...
        for p in self.points:
            ...
            n += 1
        return n
    def test_all(self, count=None):
        self.click_sequence(count)
