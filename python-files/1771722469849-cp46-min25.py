...
def find_closest_group(hh, mm, ss):
    target_sec = hh*3600 + mm*60 + ss
    closest_group = trade_table[0]
    min_diff = float('inf')
    for group in trade_table:
        for t in group:
            total_sec = time_to_seconds(t)
...
def search_trade(event=None):
    ...
    try:
        ...
        return
    group = find_closest_group(hh, mm, ss)
    show_group_lines(group)
    # نقل المؤشر للثواني لو كان المستخدم كتب الدقائق فقط
...
btn_prev = tk.Button(btn_frame,text="◀", font=main_font, width=5, command=lambda: show_group_lines(trade_table[max(0, trade_table.index(find_closest_group(*map(int, entry_minutes.get().split(":"))))-1)]))
btn_prev.pack(side="left", padx=2)

btn_next = tk.Button(btn_frame,text="▶", font=main_font, width=5, command=lambda: show_group_lines(trade_table[min(len(trade_table)-1, trade_table.index(find_closest_group(*map(int, entry_minutes.get().split(":"))))+1)]))
