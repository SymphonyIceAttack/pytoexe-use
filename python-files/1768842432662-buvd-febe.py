...
def atualizar_altura(*args):
    ...
    perfil = perfil_var.get().split(" - ")[0]
    suporte = suporte_var.get().split(" - ")[0]
...
def tratar_diametro(event):
    valor = ''.join(c for c in diametro_var.get() if c.isdigit() or c == '.')
    if valor:
...
def tratar_comprimento(event):
    valor = ''.join(c for c in comprimento_var.get() if c.isdigit() or c == '.')
    if valor:
        tol = calcular_tolerancia_geral(valor)
...
def tratar_micragem(event):
    valor = ''.join(c for c in micragem_var.get() if c.isdigit() or c == '.')
    if valor:
        m = float(valor)
...
root.resizable(False, False)          # Remove maximizar (redimensionar)
root.attributes('-toolwindow', True)  # Remove minimizar (Windows)

def label(text):
    tk.Label(frame, text=text, font=("Arial", 12, "bold")).pack(anchor="w", pady=(10, 0))
...
label("Suporte")
suporte = ttk.Combobox(
    frame,
    textvariable=suporte_var,
    state="readonly",
    values=[
        "Q15 - 1,50 x 2,50",
        "Q20 - 2,00 x 3,20",
