...
class TypingTestApp:
    def __init__(self, root):
        ...
        self.text_display = tk.Text(
            root,
            height=7, # Увеличена высота для отображения длинных текстов
            width=100,
            font=self.base_font,
            wrap='word',
            bg='#f0f0f0',
        ...
        self.text_display.tag_configure('error', background='#FFB3BA')   # Светло-красный фон для ошибки
        self.text_display.tag_configure('correct', foreground='#2E8B57', background='#E6FFF2') # Зеленый цвет для верного символа
    ...
    def load_from_file(self):
        """Открывает диалог выбора файла и загружает текст."""
        ...
        if file_path:
            try:
                ...
                if text_from_file:
                    ...
                    messagebox.showinfo("Успех", f"Текст успешно загружен!\nНажмите 'Старт'.")
                else:
    ...
    def set_custom_text(self):
        ...
        btn_ok = tk.Button(btn_frame, text="ОК",
                          command=lambda: self.save_and_close(custom_text_area, dialog_window),
                          font=self.base_font)
    ...
    def start_test(self):
        ...
        if not self.current_phrase.strip():
            messagebox.showwarning("Предупреждение", "Текст для набора отсутствует.")
            return
    ...
    def highlight_errors(self):
        ...
        Безопасно обрабатывает ввод текста любой длины.
        """
        user_text = self.entry_input.get()
