...
# Время блокировки по умолчанию (с 16:00 до 18:00)
...
class TelegramBot:
    ...
    def send_message(self, text, parse_mode='Markdown'):
        ...
        try:
            ...
            Logger.log(f"Ошибка отправки сообщения: {e}")
            return None
    ...
    def get_updates(self):
        ...
        try:
            ...
            Logger.log(f"Ошибка получения обновлений: {e}")
    ...
    def process_commands(self, state, game_blocker):
        ...
        for update in self.get_updates():
            ...
            Logger.log(f"Получена команда: {text}")
            ...
            # /on - включить блокировку
                ...
                response = "✅ *Блокировка включена*\nТеперь игры будут закрываться по расписанию"
                
            # /off - выключить блокировку
            elif command == '/off':
                state.is_enabled = False
                state.block_until = 0
                response = "❌ *Блокировка выключена*\nИгры больше не будут закрываться"
            ...
            # /time HH MM - установить свое время
                ...
                try:
                    ...
                    if 0 <= hour < 24 and 0 <= minute < 60:
                        ...
                        response = f"✅ *Установлено свое время*\n"
                        response += f"Блокировка будет работать до {hour:02d}:{minute:02d}"
                    else:
                        response = "❌ *Неверный формат*\nЧасы 0-23, минуты 0-59"
                except ValueError:
                    response = "❌ *Неверный формат*\nИспользуйте: `/time ЧЧ ММ`"
                    
            # /reset - сбросить на стандартное время
            elif command == '/reset':
                state.use_custom_time = False
                response = "✅ *Сброшено на стандартное время*\nБлокировка: 16:00 - 18:00"
            ...
            # /block N - заблокировать на N минут
                ...
                try:
                    ...
                    if minutes > 0:
                        ...
                        response = f"⏳ *Временная блокировка*\n"
                        response += f"Игры заблокированы на *{minutes} минут*\n"
                        
                        # Показываем время окончания
                        end_time = datetime.fromtimestamp(state.block_until).strftime("%H:%M")
                        response += f"До: {end_time}"
                    else:
                        response = "❌ Количество минут должно быть больше 0"
                except ValueError:
                    response = "❌ *Неверный формат*\nИспользуйте: `/block N` (N - минуты)"
                    
            # /unblock - снять временную блокировку
            elif command == '/unblock':
                state.block_until = 0
                response = "✅ *Временная блокировка снята*\nВозврат к обычному расписанию"
            ...
            # /killnow - принудительно закрыть все игры сейчас
                ...
                response = f"🔫 *Принудительное закрытие*\nЗавершено игр: *{killed}*"
    ...
    def _get_help_message(self):
        ...
        help_text = "🤖 *Game Blocker Bot*\n\n"
        help_text += "Управление блокировкой игр на компьютере\n\n"
        help_text += "*Доступные команды:*\n"
        help_text += "├ /status - текущий статус\n"
        help_text += "├ /on - включить блокировку\n"
        help_text += "├ /off - выключить блокировку\n"
        help_text += "├ /time `ЧЧ ММ` - установить свое время\n"
        help_text += "│  пример: `/time 20 00` (до 20:00)\n"
        help_text += "├ /reset - сбросить на 16:00-18:00\n"
        help_text += "├ /block `N` - заблокировать на N минут\n"
        help_text += "│  пример: `/block 30` (на полчаса)\n"
        help_text += "├ /unblock - снять временную блокировку\n"
        help_text += "├ /list - список отслеживаемых игр\n"
        help_text += "├ /stats - статистика работы\n"
        help_text += "├ /mute - отключить уведомления\n"
    ...
    def _get_status_message(self, state):
        ...
        status_text += f"Блокировка: {'✅ *ВКЛЮЧЕНА*' if state.is_enabled else '❌ *ВЫКЛЮЧЕНА*'}\n"
        
        # Если есть временная блокировка
        if state.block_until > 0:
            remaining = int((state.block_until - time.time()) / 60)
            if remaining > 0:
                status_text += f"⏳ Временная блокировка: *{remaining} мин*\n"
        
        # Расписание
        if state.use_custom_time:
            status_text += f"⏰ Свое расписание: *{state.custom_start_hour:02d}:00 - {state.custom_end_hour:02d}:00*\n"
        else:
            status_text += "⏰ Стандартное расписание: *16:00 - 18:00*\n"
    ...
    def _get_stats_message(self, game_blocker):
        ...
        stats = "📈 *Статистика:*\n\n"
        stats += f"Всего игр в списке: *{len(GAME_PROCESSES)}*\n"
        stats += f"Сейчас запущено игр: *{len(running_games)}*\n"
        ...
        if running_games:
            ...
            if len(running_games) > 5:
                stats += f"└ ...и еще {len(running_games) - 5}\n"
        
        stats += f"\n⏱ Время работы ПК: *{uptime_hours:.1f}* часов"
...
class GameBlocker:
    ...
    def load_state(self):
        ...
        if state_file.exists():
            try:
                ...
                Logger.log(f"Ошибка загрузки состояния: {e}")
    ...
    def save_state(self):
        ...
        try:
            ...
            Logger.log(f"Ошибка сохранения состояния: {e}")
    ...
    def get_running_games(self):
        ...
        try:
            ...
            Logger.log(f"Ошибка при получении списка процессов: {e}")
    ...
    def kill_game(self, proc_info):
        ...
        try:
            ...
            Logger.log(f"Игра закрыта: {proc_info['name']} (PID: {proc_info['pid']})")
            
            return True
            
        except Exception as e:
            Logger.log(f"Ошибка при завершении {proc_info['name']}: {e}")
            return False
            ...
            if self.kill_game(game):
                ...
                if not self.mute_notifications and self.bot:
                    ...
                    if now - self.last_notification_time > 60:
                        msg = f"⚠️ Игра *{game['name']}* была закрыта.\n"
    ...
    def monitor_loop(self):
        ...
        while self.running:
            try:
                ...
                Logger.log(f"Ошибка в цикле мониторинга: {e}")
                time.sleep(CHECK_INTERVAL)
    ...
    def add_to_startup(self):
        ...
        try:
            ...
            Logger.log(f"Ошибка добавления в автозагрузку: {e}")
    ...
    def remove_from_startup(self):
        ...
        try:
            ...
            Logger.log(f"Ошибка удаления из автозагрузки: {e}")
    ...
    def is_in_startup(self):
        ...
        try:
            ...
            Logger.log(f"Ошибка проверки автозагрузки: {e}")
            return False
        ...
        try:
            ...
            def on_click(icon, item):
                if str(item) == "Выход":
                    ...
                    icon.notify(f"Статус: {status}", APP_NAME)
                elif str(item) == "Вкл/Выкл":
                    self.state.is_enabled = not self.state.is_enabled
                    self.save_state()
                    icon.notify(f"Блокировка {'включена' if self.state.is_enabled else 'выключена'}", APP_NAME)
            
            # Создаем меню
            menu = pystray.Menu(
                pystray.MenuItem("Статус", on_click),
                pystray.MenuItem("Вкл/Выкл", on_click),
            ...
            Logger.log(f"Ошибка настройки трея: {e}")
...
def check_config():
    ...
    if BOT_TOKEN == "YOUR_BOT_TOKEN_HERE" or ALLOWED_USER_ID == "YOUR_TELEGRAM_ID":
        ...
        print("3. Скопировать токен (пример: 123456:ABCdef)")
        ...
        ctypes.windll.user32.MessageBoxW(
            0,
            "Не заполнены настройки Telegram!\n\n"
            "1. Напишите @BotFather в Telegram\n"
            "2. Создайте бота командой /newbot\n"
            "3. Скопируйте токен\n"
            "4. Напишите @userinfobot и узнайте свой ID\n"
...
def main():
    ...
    try:
        ...
        mutex = win32event.CreateMutex(None, False, "Global\\GameBlockerMutex")
    ...
    bot.send_message("🟢 *Game Blocker запущен*\nКомпьютер под контролем")
    ...
    def telegram_worker():
        while blocker.running:
            try:
                ...
                Logger.log(f"Ошибка в Telegram потоке: {e}")
                time.sleep(5)
