...
class SectorDataConverter:
    ...
    def setup_ui(self):
        ...
        # 左边：数值输入框
        ttk.Label(main_frame, text="数值").grid(row=0, column=0, sticky=tk.W, pady=(0, 5))
        ...
        # 等号标签
        ttk.Label(center_frame, text="=", font=("Arial", 24)).pack()
        ...
        self.hex_entry = ttk.Entry(main_frame, textvariable=self.hex_var, width=30)
        self.hex_entry.grid(row=1, column=2, sticky=(tk.W, tk.E), pady=(0, 10))
        self.hex_entry.bind('<Return>', lambda e: self.convert_hex_to_value())
        
        # 状态栏
        self.status_var = tk.StringVar()
        self.status_var.set("输入数值或字节码，点击转换或按回车")
