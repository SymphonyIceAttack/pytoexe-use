...
class BinEditor:
    ...
    def build_serial_editor(self):
        ...
        # 创建显示框架，同时显示ASCII和HEX
        display_frame = tk.Frame(frame)
        display_frame.pack(fill="x", pady=5)
        
        tk.Label(display_frame, text="ASCII:").grid(row=0, column=0, sticky="w", padx=5)
        self.serial_ascii_display = tk.Label(display_frame, text="", font=("Consolas", 12), 
                                             bg="white", relief="sunken", width=30, anchor="w")
        self.serial_ascii_display.grid(row=0, column=1, sticky="w", padx=5)
        
        tk.Label(display_frame, text="HEX:").grid(row=1, column=0, sticky="w", padx=5)
        self.serial_hex_display = tk.Label(display_frame, text="", font=("Consolas", 12), 
                                           bg="white", relief="sunken", width=40, anchor="w")
        self.serial_hex_display.grid(row=1, column=1, sticky="w", padx=5)
        
        tk.Label(display_frame, text="字节数:").grid(row=2, column=0, sticky="w", padx=5)
        self.serial_length_display = tk.Label(display_frame, text="", font=("Consolas", 12))
        self.serial_length_display.grid(row=2, column=1, sticky="w", padx=5)

        # 新序列号输入
        tk.Label(frame, text="新序列号 (ASCII):").pack(anchor="w", pady=(10, 0))
        ascii_frame = tk.Frame(frame)
        ascii_frame.pack(fill="x", pady=5)
        
        self.new_serial = tk.Entry(ascii_frame, font=("Consolas", 12), width=30)
        self.new_serial.pack(side="left", padx=5)
        tk.Button(ascii_frame, text="更新(ASCII)", command=self.update_serial_ascii,
                  bg="lightgreen", width=12).pack(side="left", padx=5)
        tk.Label(ascii_frame, text="(只支持ASCII字符)", fg="gray").pack(side="left", padx=5)

        tk.Label(frame, text="新序列号 (十六进制):").pack(anchor="w", pady=(10, 0))
        hex_frame = tk.Frame(frame)
        hex_frame.pack(fill="x", pady=5)
        
        self.new_serial_hex = tk.Entry(hex_frame, font=("Consolas", 12), width=30)
        self.new_serial_hex.pack(side="left", padx=5)
        tk.Button(hex_frame, text="更新(HEX)", command=self.update_serial_hex,
                  width=12).pack(side="left", padx=5)
        tk.Label(hex_frame, text="(例如: 41 42 43)", fg="gray").pack(side="left", padx=5)
    ...
    def read_serial(self):
        ...
        try:
            ...
            # 同时更新十六进制编辑器的输入框，方便查看
            self.new_serial.delete(0, tk.END)
            self.new_serial.insert(0, ascii_str.replace('.', ''))  # 只显示可打印字符
    ...
    def update_serial_ascii(self):
        ...
        if len(new_serial) > SERIAL_LENGTH:
            if not messagebox.askyesno("确认",
                                       f"序列号长度({len(new_serial)})超过预设长度({SERIAL_LENGTH})，\n"
                                       f"是否继续？超过的部分会被截断。"):
                return
            new_serial = new_serial[:SERIAL_LENGTH]
        ...
        try:
            ...
            if len(serial_bytes) < SERIAL_LENGTH:
                for i in range(len(serial_bytes), SERIAL_LENGTH):
            ...
            if self.offset_var.get() == f"{TARGET_OFFSET:X}":
                self.read_hex_data()
            self.refresh_info()
    ...
    def update_serial_hex(self):
        ...
        try:
            ...
            if len(serial_bytes) > SERIAL_LENGTH:
                if not messagebox.askyesno("确认",
                                           f"数据长度({len(serial_bytes)})超过预设长度({SERIAL_LENGTH})，\n"
                                           f"是否继续？超过的部分会被截断。"):
                    return
                serial_bytes = serial_bytes[:SERIAL_LENGTH]
