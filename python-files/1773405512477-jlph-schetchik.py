...
def print_header():
    print("=" * 110)
    print("   ПРОГРАММА ДЛЯ ПОДБОРА ЗНАЧЕНИЙ ".center(110))
    print("   (С РАСЧЕТОМ ДНЕВНОГО И НОЧНОГО ТАРИФОВ)".center(110))
    print("=" * 110)

def print_dual_solution_table(solutions_day, solutions_night, a, a1, k):
    """Вывод таблицы с результатами для двух тарифов"""
    
    print("\n" + "═" * 110)
    print("   РЕЗУЛЬТАТЫ РАСЧЕТА".center(110))
    print("═" * 110)
...
def main():
    while True:
        ...
        try:
            ...
            a_input = input("\n🌞 Введите значение дневного тарифа: ").strip()
            ...
            a1_input = input("🌙 Введите значение ночного тарифа: ").strip()
            a1_input = a1_input.replace(',', '.')
            a1 = float(a1_input)
            
            # Ввод коэффициента k
            k_input = input("🔄 Введите коэффициент трансформации: ").strip()
            k_input = k_input.replace(',', '.')
            k = float(k_input)
            
            if k <= 0:
                print("\n⚠️  Ошибка: коэффициент трансформации должен быть положительным!")
                input("\n⏎ Нажмите Enter...")
                continue
            ...
            if method == "2":
                ...
                tolerance_input = input("📏 Допустимое отклонение от значения тарифа [по умолчанию 1]: ").strip()
                tolerance = float(tolerance_input) if tolerance_input else 1.0
                
                # Параллельный поиск для обоих тарифов
                print(f"\n🔄 Выполняется расчет для обоих тарифов...")
