...
def apply_patch(exe_path, code_id, codes_data, mod_expansion=None):
    ...
    # shared_patches: applied but never reverted (e.g. 002B7AC8 shared between ESE/AUTO_DOOR/CAM)
    all_patches = list(patches) + data.get("shared_patches", [])
...
def revert_patch(exe_path, orig_path, code_id, codes_data, mod_expansion=None):
    """
    Revert a code's patches.
    - find_replace : swap replace->find. If replace bytes not found, skip (already reverted or never applied).
    - offset_paste / offset_replace : read from bio4_original.exe.
    Returns (success, message, skipped_count)
    """
    data = codes_data.get(code_id)
    if not data:
    ...
    for patch in all_patches:
        # shared patches are applied-only, never reverted
        if patch.get("shared"):
            skipped += 1
            continue
        try:
            ...
            if ptype == "find_replace":
                ...
                if orig_bytes is None:
                    return False, (
                        "bio4_original.exe not found.\n"
                        "Place the original bio4.exe in:\n"
                        "the_codes/bio4_original.exe"
                    ), skipped
                orig_chunk = orig_bytes[off:off + length]
                if len(orig_chunk) != length:
                    return False, "Original EXE too short at offset " + patch["offset"], skipped
...
class RE4PatcherApp(tk.Tk):
    # قائمة الأوفستات المتعارضة (سيتم بناؤها تلقائياً)
    OFFSET_CONFLICTS = {}
    EXCLUDED_OFFSETS = ["002B7AC8"]  # أوفست مستثنى

    def __init__(self):
        super().__init__()

        self.title("RE4 Code Patcher Tool")
        ...
        # بناء خريطة الأوفستات المتعارضة
        self._build_offset_conflicts()
        
        self._build_ui()
        self.select_section(self.sections_list[0]["id"])

    def _build_offset_conflicts(self):
        """يبني خريطة للأوفستات المتعارضة مباشرة من codes_data.json"""
        self.OFFSET_CONFLICTS.clear()
        
        # تجميع كل الأوفستات مع الأكواد التي تستخدمها
        offset_usage = {}
        
        for code_id, data in self.codes_data.items():
            all_patches = []
            
            if "variants" in data:
                for variant in data["variants"].values():
                    all_patches.extend(variant.get("patches", []))
            else:
                all_patches.extend(data.get("patches", []))
            
            all_patches.extend(data.get("shared_patches", []))
            
            for patch in all_patches:
                if patch.get("type") in ("offset_paste", "offset_replace"):
                    offset = patch.get("offset", "")
                    # استثناء الأوفستات المشتركة
                    if offset and offset.upper() not in self.EXCLUDED_OFFSETS:
                        if offset not in offset_usage:
                            offset_usage[offset] = []
                        offset_usage[offset].append(code_id)
        
        # بناء علاقات التعارض
        for offset, codes in offset_usage.items():
            if len(codes) > 1:
                for code in codes:
                    if code not in self.OFFSET_CONFLICTS:
                        self.OFFSET_CONFLICTS[code] = []
                    self.OFFSET_CONFLICTS[code].extend([c for c in codes if c != code])

    def _handle_offset_conflicts(self, code_id):
        """عند تطبيق كود، نلغي أي كود آخر يستخدم نفس الأوفست"""
        if code_id not in self.OFFSET_CONFLICTS:
            return
        ...
        for conflicting_code in self.OFFSET_CONFLICTS[code_id]:
            if self.applied.get(conflicting_code, False):
                ok, _, _ = revert_patch(exe, ORIG_FILE, conflicting_code, self.codes_data)
                if ok:
                    self.applied[conflicting_code] = False
                    self.detected[conflicting_code] = False
                    write_log("AUTO-REVERTED (conflict)", 
                             self.code_by_id.get(conflicting_code, {}).get("name", conflicting_code), 
                             exe)

    def _restore_original(self):
        """إعادة جميع الأكواد إلى حالتها الأصلية باستخدام bio4_original.exe"""
        if not messagebox.askyesno(
            "Restore Original",
            "⚠️ WARNING: This will revert ALL applied codes to their original state.\n\n"
        ...
        if not os.path.isfile(ORIG_FILE):
            messagebox.showerror(
                "Error",
                "Original EXE file not found.\n\n"
                "Please place the original bio4.exe in:\n"
                f"{ORIG_FILE}"
            )
            return
        ...
        if is_game_running(exe):
            messagebox.showerror(
                "Game is Running",
                "Please close the game before restoring the original EXE."
            )
            return
        
        # قائمة الأكواد المطبقة حالياً
        applied_codes = [cid for cid, applied in self.applied.items() if applied]
        
        if not applied_codes:
            messagebox.showinfo("Info", "No applied codes to restore.")
            return
        
        success = []
        failed = []
        
        for code_id in applied_codes:
            ...
            if ok:
                ...
                write_log("RESTORED (original)", 
                         self.code_by_id.get(code_id, {}).get("name", code_id), 
                         exe)
            else:
                failed.append((code_id, msg))
        
        summary = f"Restored {len(success)} code(s) successfully."
        if failed:
            summary += "\n\nFailed to restore:\n"
            for cid, err in failed:
                summary += f"- {self.code_by_id.get(cid, {}).get('name', cid)}: {err}\n"
            messagebox.showwarning("Restore Completed with Errors", summary)
        else:
            messagebox.showinfo("Restore Completed", summary)
    ...
    def _build_ui(self):
        ...
        self.search_results_frame = tk.Frame(right, bg="#0d0d1a",
                                             highlightthickness=1,
    ...
    def _apply_best_settings(self):
        ...
        success, failed = [], []
        for cid in queue:
            self._handle_offset_conflicts(cid)
    ...
    def _open_settings(self):
        ...
        make_label(dlg, "Restore Original",
                   fg=ACCENT2, bg="#111", font=FONT_SMALL
                   ).pack(pady=(4, 6))
        make_button(dlg, "⚠️ Restore All Codes", self._restore_original,
                    fg=RED_SOFT, bg="#2a0a0a", active_bg="#4a1a1a", width=16
                    ).pack(pady=(0, 10))
    ...
    def handle_toggle(self, code_id):
        ...
        if self.applied.get(code_id, False):
            ...
            if ok:
                ...
                if dependents:
                    dep_names = "\n".join(
                        "  - " + self.code_by_id.get(d, {}).get("name", d)
                        for d in dependents
                    )
                    if messagebox.askyesno(
                        "Cascade Revert",
                        "The following codes depend on this one and will also be turned off:\n\n"
                        + dep_names + "\n\nContinue?"
                    ):
                        for dep in dependents:
                            ...
                            if ok2:
                                ...
                                write_log("REVERTED (cascade)", self.code_by_id.get(dep, {}).get("name", dep), exe)
    ...
    def _do_apply(self, code_id, mod_expansion=None):
        ...
        self._handle_offset_conflicts(code_id)
    ...
    def _on_search(self):
        ...
        self.search_results_frame.pack(fill="x", padx=10, pady=(0, 4))
    ...
    def _add_new_code(self):
        ...
        def save_new_code():
            ...
            # إعادة بناء خريطة التعارضات بعد إضافة كود جديد
            self._build_offset_conflicts()
