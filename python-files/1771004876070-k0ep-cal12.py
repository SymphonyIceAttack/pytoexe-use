...
view_menu.add_command(label="Move Window", command=enable_move_mode)

# Transparency
transp_menu = tk.Menu(view_menu, tearoff=0)
view_menu.add_cascade(label="Transparency (%)", menu=transp_menu)
for p in range(10, 101, 10):
    transp_menu.add_command(label=f"{p}%", command=lambda x=p: set_alpha(x))
