...
def run_wmic_command(command):
    try:
        result = subprocess.run(
            ...
            text=False,
            timeout=10,
            creationflags=subprocess.CREATE_NO_WINDOW  # Скрывает окно cmd при выполнении wmic
        )
...
if __name__ == "__main__":
    try:
        ...
        # ПРИНТЫ УДАЛЕНЫ, так как окно скрыто. 
        # Вся информация теперь только в логе C:\Logs\usb_logs.log
        
    except PermissionError:
        # Эту ошибку тоже нельзя увидеть в скрытом режиме без логов!
        pass 
    except Exception as e:
        # Ошибки тоже пишем в лог, если нужно, или игнорируем, если скрипт фоновый
        pass