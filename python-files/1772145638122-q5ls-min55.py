import tkinter as tk
import tkinter.font as tkfont
from datetime import datetime

# ------------------- Trade Table -------------------
trade_table = [
    ["01:00:00 prepare","01:00:30","01:01:00 go trade"],
    ["01:01:30 prepare","01:02:00","01:02:30 go trade"],
    ["01:03:00 prepare","01:03:30","01:04:00 go trade"],
    ["01:04:30 prepare","01:05:00","01:05:30 go trade"],
    ["01:06:00 prepare","01:06:30","01:07:00 go trade"],
    ["01:07:30 prepare","01:08:00","01:08:30 go trade"],
    ["01:09:00 prepare","01:09:30","01:10:00 go trade"],
    ["01:10:30 prepare","01:11:00","01:11:30 go trade"],
    ["01:12:00 prepare","01:12:30","01:13:00 go trade"],
    ["01:13:30 prepare","01:14:00","01:14:30 go trade"],
    ["01:15:00 prepare","01:15:30","01:16:00 go trade"],
    ["01:16:30 prepare","01:17:00","01:17:30 go trade"],
    ["01:18:00 prepare","01:18:30","01:19:00 go trade"],
    ["01:19:30 prepare","01:20:00","01:20:30 go trade"],
    ["01:21:00 prepare","01:21:30","01:22:00 go trade"],
    ["01:22:30 prepare","01:23:00","01:23:30 go trade"],
    ["01:24:00 prepare","01:24:30","01:25:00 go trade"],
    ["01:25:30 prepare","01:26:00","01:26:30 go trade"],
    ["01:27:00 prepare","01:27:30","01:28:00 go trade"],
    ["01:28:30 prepare","01:29:00","01:29:30 go trade"],
    ["01:30:00 prepare","01:30:30","01:31:00 go trade"],
    ["01:31:30 prepare","01:32:00","01:32:30 go trade"],
    ["01:33:00 prepare","01:33:30","01:34:00 go trade"],
    ["01:34:30 prepare","01:35:00","01:35:30 go trade"],
    ["01:36:00 prepare","01:36:30","01:37:00 go trade"],
    ["01:37:30 prepare","01:38:00","01:38:30 go trade"],
    ["01:39:00 prepare","01:39:30","01:40:00 go trade"],
    ["01:40:30 prepare","01:41:00","01:41:30 go trade"],
    ["01:42:00 prepare","01:42:30","01:43:00 go trade"],
    ["01:43:30 prepare","01:44:00","01:44:30 go trade"],
    ["01:45:00 prepare","01:45:30","01:46:00 go trade"],
    ["01:46:30 prepare","01:47:00","01:47:30 go trade"],
    ["01:48:00 prepare","01:48:30","01:49:00 go trade"],
    ["01:49:30 prepare","01:50:00","01:50:30 go trade"],
    ["01:51:00 prepare","01:51:30","01:52:00 go trade"],
    ["01:52:30 prepare","01:53:00","01:53:30 go trade"],
    ["01:54:00 prepare","01:54:30","01:55:00 go trade"],
    ["01:55:30 prepare","01:56:00","01:56:30 go trade"],
    ["01:57:00 prepare","01:57:30","01:58:00 go trade"],
    ["01:58:30 prepare","01:59:00","01:59:30 go trade"]
]

# ------------------- Globals -------------------
BASE_FONT = 14
current_font = "Segoe UI"
dark_mode = False
topmost = True
current_group_idx = 0
ai_enabled = False
ai_after_id = None
main_font = None
timer_after_id = None

# ------------------- Root -------------------
root = tk.Tk()
root.title("Trade Lookup + AI + View")
root.geometry("520x470")
try:
    root.attributes("-topmost", True)
    root.attributes("-alpha", 0.95)
except:
    pass
main_font = tkfont.Font(family=current_font, size=BASE_FONT)

# ------------------- Helper Functions -------------------
def parse_time_str(s):
    try:
        return datetime.strptime(s.split()[0], "%H:%M:%S").time()
    except:
        return None

def display_group(idx):
    global current_group_idx
    idx %= len(trade_table)
    current_group_idx = idx
    output.config(state="normal")
    output.delete("1.0", tk.END)
    for l in trade_table[idx]:
        output.insert(tk.END, l+"\n")
    output.config(state="disabled")
    start_timer_for_group(idx)

# ------------------- Timer -------------------
def start_timer_for_group(idx):
    global timer_after_id
    if timer_after_id:
        root.after_cancel(timer_after_id)
        timer_after_id = None
    t = trade_table[idx]
    prepare_time = 30  # ثواني للـ prepare
    go_time = 20       # ثواني للـ go
    timer_label.config(text="prepare", fg="#D2B48C")  # بني فاتح
    def update_prepare(sec):
        if sec <= 0:
            timer_label.config(text="go", fg="orange")
            root.after(go_time*1000, lambda: timer_label.config(text=""))
            return
        timer_label.config(text=f"{sec}s prepare", fg="#D2B48C")
        global timer_after_id
        timer_after_id = root.after(1000, lambda: update_prepare(sec-1))
    update_prepare(prepare_time)

# ------------------- Find Group -------------------
def find_group_by_time():
    now = datetime.now()
    current = (now.minute, now.second)
    for i in range(len(trade_table)):
        t_end = parse_time_str(trade_table[i][2])
        if current <= (t_end.minute, t_end.second):
            return i
    return len(trade_table)-1

def find_group_by_manual_minute(minute):
    for i, g in enumerate(trade_table):
        t_start = parse_time_str(g[0])
        t_end   = parse_time_str(g[2])
        if t_start.minute <= minute <= t_end.minute:
            return i
    return None

def on_generate(event=None):
    if ai_enabled:
        idx = find_group_by_time()
    else:
        try:
            minute = int(entry.get())
        except:
            return "break"
        idx = find_group_by_manual_minute(minute)
    if idx is not None:
        display_group(idx)
    return "break"

def clear_output():
    output.config(state="normal")
    output.delete("1.0", tk.END)
    output.config(state="disabled")
    timer_label.config(text="")

def prev_group(event=None):
    display_group(current_group_idx-1)
    return "break"

def next_group(event=None):
    display_group(current_group_idx+1)
    return "break"

# ------------------- AI -------------------
def draw_ai_lamp():
    ai_canvas.delete("all")
    color = "green" if ai_enabled else "red"
    ai_canvas.create_oval(2,2,14,14, fill=color, outline="black")

def ai_loop():
    global ai_after_id
    if not ai_enabled: return
    now = datetime.now()
    entry.config(state="normal")
    entry.delete(0, tk.END)
    entry.insert(0, f"{now.minute:02d}:{now.second:02d}")
    entry.config(state="disabled")
    on_generate()
    ai_after_id = root.after(1000, ai_loop)

def toggle_ai():
    global ai_enabled, ai_after_id
    ai_enabled = not ai_enabled
    draw_ai_lamp()
    if ai_enabled:
        entry.config(state="disabled")
        ai_loop()
    else:
        if ai_after_id:
            root.after_cancel(ai_after_id)
            ai_after_id = None
        entry.config(state="normal")
        entry.delete(0, tk.END)

# ------------------- View Functions -------------------
def toggle_mode():
    global dark_mode
    dark_mode = not dark_mode
    bg = "black" if dark_mode else "white"
    fg = "white" if dark_mode else "black"
    root.config(bg=bg)
    top_frame.config(bg=bg)
    control_frame.config(bg=bg)
    entry.config(bg=bg, fg=fg, insertbackground=fg)
    btn_generate.config(bg=bg, fg=fg)
    btn_clear.config(bg=bg, fg=fg)
    btn_prev.config(bg=bg, fg=fg)
    btn_next.config(bg=bg, fg=fg)
    output.config(bg=bg, fg=fg)
    timer_label.config(bg=bg, fg=fg)
    draw_ai_lamp()

def open_scale_popup():
    scale_win = tk.Toplevel(root)
    scale_win.title("Scale / Font Size")
    scale_win.geometry("300x100")
    def on_zoom(val):
        val = int(float(val))
        main_font.configure(size=val)
        entry.config(font=(current_font,int(val*1.2)))
        btn_generate.config(font=(current_font,int(val*0.7)))
        btn_clear.config(font=(current_font,int(val*0.7)))
        btn_prev.config(font=(current_font,int(val*0.7)))
        btn_next.config(font=(current_font,int(val*0.7)))
        output.config(font=main_font)
        timer_label.config(font=(current_font,int(val*0.9)))
        draw_ai_lamp()
    scale = tk.Scale(scale_win, from_=10, to=28, orient="horizontal",
                     label="Font Size", command=on_zoom)
    scale.set(main_font.cget("size"))
    scale.pack(fill="x", padx=10, pady=10)

# ------------------- UI -------------------
top_frame = tk.Frame(root)
top_frame.pack(pady=2)

entry = tk.Entry(top_frame, justify="center",
                 font=(current_font,int(BASE_FONT*1.2)), width=10)
entry.pack()
entry.insert(0,"0")
entry.bind("<Return>", on_generate)

control_frame = tk.Frame(root)
control_frame.pack(pady=2)

btn_generate = tk.Button(control_frame, text="Generate",
                         font=(current_font,int(BASE_FONT*0.7)),
                         command=on_generate)
btn_generate.pack(side="left", padx=2)
btn_clear = tk.Button(control_frame, text="Clear",
                      font=(current_font,int(BASE_FONT*0.7)),
                      command=clear_output)
btn_clear.pack(side="left", padx=2)
btn_prev = tk.Button(control_frame, text="◀",
                     font=(current_font,int(BASE_FONT*0.7)),
                     command=prev_group)
btn_prev.pack(side="left", padx=2)
btn_next = tk.Button(control_frame, text="▶",
                     font=(current_font,int(BASE_FONT*0.7)),
                     command=next_group)
btn_next.pack(side="left", padx=2)
ai_canvas = tk.Canvas(control_frame, bd=0, highlightthickness=0, width=16, height=16)
ai_canvas.pack(side="left", padx=4)
draw_ai_lamp()

output = tk.Text(root, font=main_font, height=8)
output.pack(expand=True, fill="both", padx=10, pady=6)
output.config(state="disabled")

# ------------------- Timer Label -------------------
timer_label = tk.Label(root, text="", font=(current_font, int(BASE_FONT*0.9)))
timer_label.pack(pady=5)

# ------------------- Menu -------------------
menubar = tk.Menu(root)
root.config(menu=menubar)

file_menu = tk.Menu(menubar, tearoff=0)
file_menu.add_command(label="Pin / Unpin", command=lambda: root.attributes("-topmost", not root.attributes("-topmost")))
file_menu.add_separator()
file_menu.add_command(label="Exit", command=root.quit)
menubar.add_cascade(label="File", menu=file_menu)

view_menu = tk.Menu(menubar, tearoff=0)
view_menu.add_command(label="Day / Night", command=toggle_mode)
view_menu.add_separator()
view_menu.add_command(label="Scale / Font", command=open_scale_popup)
menubar.add_cascade(label="View", menu=view_menu)

ai_menu = tk.Menu(menubar, tearoff=0)
ai_menu.add_command(label="AI ON / OFF", command=toggle_ai)
menubar.add_cascade(label="AI", menu=ai_menu)

# ------------------- Keys -------------------
root.bind("<Left>", prev_group)
root.bind("<Right>", next_group)

display_group(0)
root.mainloop()