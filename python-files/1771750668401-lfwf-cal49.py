...
TIMER_TARGETS = {3,9,15,21,27,33,39,45,51,57}
...
timer_running = False  # حالة التايمر
timer_seconds = 0      # العد التنازلي بالثواني
...
root.geometry("500x350")  # زيادة المساحة للتايمر
...
def calculate(event=None):
    ...
    if ai_mode and v in AI_SPECIALS:
        ...
        # إذا بدأ من رقم AI معين، شغّل التايمر لمدة دقيقة
        if not timer_running:
            timer_seconds = 60
            timer_lbl.config(text=f"Timer: {timer_seconds}s", fg="blue")
    ...
    middle_lbl.config(text=middle_text, bg=middle_bg, justify="center", font=middle_font, anchor="center")
...
def run_timer(start_value):
    ...
    # تحقق إذا الرقم الحالي وصل للهدف التالي
    next_target = min([x for x in TIMER_TARGETS if x > start_value], default=None)
    current_v = int(entry.get() or 0)
    if next_target and current_v >= next_target:
        timer_lbl.pack_forget()
        timer_running = False
        return

    timer_lbl.config(text=f"Timer: {timer_seconds}s")
    timer_seconds -= 1
    root.after(1000, lambda: run_timer(start_value))
...
# ================= TIMER LABEL =================
timer_lbl = tk.Label(root, text="", font=(current_font, int(BASE_FONT*0.8)), fg="blue")
