...
class RE4PatcherApp(tk.Tk):
    ...
    def select_section(self, section_id, preserve_scroll=None, preserve_selected=None):
        ...
        for code in codes:
            ...
            # restore selection
            if preserve_selected and code["id"] in preserve_selected:
                row.sel_var.set(1)
                row.selected = True
    ...
    def _after_state_change(self):
        ...
        Preserves scroll position and selected codes.
        """
        # save scroll position and selected codes
        scroll_pos = self.codes_canvas.yview()[0]
        selected_before = {cid for cid, row in self.code_rows.items() if row.selected}

        self._refresh_all()
        if self.active_section:
            self.select_section(self.active_section, preserve_scroll=scroll_pos,
                                preserve_selected=selected_before)
        self._update_statusbar()
        self._update_apply_bar()
