...
def show_group(idx):
    ...
    if idx < 0: idx = 0
    if idx >= len(trade_table): idx = len(trade_table)-1
...
def next_group(event=None):
    if current_group_idx < len(trade_table)-1:
        show_group(current_group_idx+1)
...
def add_history_group(idx):
    ...
    history.append(f"{len(history)+1}. Group {idx+1} | {ts}")
    update_history()
...
btn_generate = tk.Button(frame_top,text="Generate", font=main_font, width=10, command=lambda: show_group(int(entry_minutes.get())%len(trade_table)))
...
btn_prev = tk.Button(nav_frame,text="◀", font=main_font, width=5, command=prev_group)
btn_prev.pack(side="left", padx=20)

btn_next = tk.Button(nav_frame,text="▶", font=main_font, width=5, command=next_group)
