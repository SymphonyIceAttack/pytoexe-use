...
def init_db():
    ...
    for col in ('rate_prev_year', 'rate_prev2_year'):
        try:
    ...
    c.execute('''
        CREATE TABLE IF NOT EXISTS monthly_data (
            ...
            invoice INTEGER DEFAULT 0,   -- 0=нет, 1=Отправлен, 2=Напечатан
            act INTEGER DEFAULT 0,       -- 0=нет, 1=Отправлен, 2=Напечатан
            docs_returned INTEGER DEFAULT 0, -- 0=Нет, 1=Да
...
def format_month(month_str):
    """Преобразует 'YYYY-MM' в 'Месяц YYYY года' на русском"""
...
class ClientEditDialog(tk.Toplevel):
    ...
    def _enable_copy_paste(self, widget):
        """Добавляет горячие клавиши и контекстное меню для копирования/вставки"""
        def copy(event):
...
class MainApplication:
    ...
    def on_cell_edit(self, event):
        ...
        if region != "cell":
            return
        column = self.month_tree.identify_column(event.x)  # '#1', '#2', ...
        col_index = int(column[1:]) - 1  # 0-based
        item = self.month_tree.identify_row(event.y)
        if not item:
            return
        values = self.month_tree.item(item, 'values')
        month_display = values[0]
        # Найдём month_year по отображаемому названию
        month_year = None
        all_months = get_all_months()
        for m in all_months:
        ...
        # В зависимости от колонки показываем меню выбора
        if col_index == 1:  # Счёт
            self._show_option_menu(event, ["Отправлен", "Напечатан"],
                                   lambda val: self._save_cell(month_year, col_index, val, inv, act, docs, visits))
        elif col_index == 2:  # Акт
            self._show_option_menu(event, ["Отправлен", "Напечатан"],
                                   lambda val: self._save_cell(month_year, col_index, val, inv, act, docs, visits))
        elif col_index == 3:  # Вернул доки
            self._show_option_menu(event, ["Да", "Нет"],
                                   lambda val: self._save_cell(month_year, col_index, val, inv, act, docs, visits))
        elif col_index == 4:  # Выезды
            numbers = [str(i) for i in range(1, 11)]
            self._show_option_menu(event, numbers,
                                   lambda val: self._save_cell(month_year, col_index, int(val), inv, act, docs, visits))

    def _show_option_menu(self, event, options, callback):
        """Показывает всплывающее меню с вариантами выбора"""
    ...
    def _save_cell(self, month_year, col_index, value, inv, act, docs, visits):
        """Сохраняет выбранное значение в БД и обновляет таблицу"""
        # col_index: 1-Счёт, 2-Акт, 3-Вернул, 4-Выезды
        if col_index == 1:
            # value: "Отправлен" или "Напечатан"
            new_inv = 1 if value == "Отправлен" else 2 if value == "Напечатан" else 0
            inv = new_inv
        elif col_index == 2:
            new_act = 1 if value == "Отправлен" else 2 if value == "Напечатан" else 0
            act = new_act
        elif col_index == 3:
            new_docs = 1 if value == "Да" else 0
            docs = new_docs
        elif col_index == 4:
            visits = int(value)
