...
# ==================== КОНФИГУРАЦИЯ ====================
# Папка в той же директории, где находится программа
ROOT_DIR = Path(os.path.dirname(os.path.abspath(__file__))) / "MediaVault"
MEDIA_EXTENSIONS = {
...
class MediaManagerApp(tk.Tk):
    def __init__(self):
        ...
        # Создаём корневую папку в той же директории
        try:
            ROOT_DIR.mkdir(exist_ok=True)
            print(f"Папка создана: {ROOT_DIR}")
    ...
    def create_widgets(self):
        ...
        for text, cmd in buttons:
            btn = tk.Button(toolbar, text=text, font=('Segoe UI', 10),
                           bg="white", relief=tk.RAISED, bd=1, padx=12, pady=4,
    ...
    def go_up(self):
        ...
        if self.current_folder != ROOT_DIR:
            ...
            try:
                ...
                pass
            self.status_var.set(f"📂 Переход в: {self.current_folder.name}")
    ...
    def refresh_content(self):
        ...
        # Обновляем статус
        self.status_var.set(f"📁 {len([i for i in items if i[0] == 'folder'])} папок, " +
                           f"🖼️ {len([i for i in items if i[0] == 'media'])} файлов")
    ...
    def create_folder_widget(self, folder_path: Path, row, col):
        ...
        # Кнопка-иконка папки (большой символ)
