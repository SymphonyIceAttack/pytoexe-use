...
# Лже-функции (никогда не вызываются)
...
def block_start_menu():
    ...
    try:
        ...
        winreg.CloseKey(key)
        print("[DEBUG] Меню Пуск и панель задач заблокированы через реестр.")
    except Exception as e:
        print(f"[WARN] Не удалось заблокировать меню Пуск: {e}")
...
class UniversalLock(tk.Tk):
    def __init__(self):
        ...
        # ---------- БЛОКИРОВКИ ----------
        self.block_cad()
        self.block_start_menu()          # <--- НОВОЕ: скрыть кнопку Пуск
        self.add_to_startup()
    ...
    def block_start_menu(self):
        # Вызываем функцию из глобальной области (она выше)
        block_start_menu()
    ...
    # ---------- ХУК КЛАВИАТУРЫ (уже блокирует Win, Alt+Tab, Ctrl+Esc, Alt+F4, Ctrl+Shift+Esc) ----------
    def block_keys(self):
        WH_KEYBOARD_LL = 13
        VK_LWIN = 0x5B
        VK_RWIN = 0x5C
...
"""
    ...
    def safe_destroy(self):
        ...
        self.show_taskbar()
        self.restore_start_menu()          # <--- ВОССТАНОВЛЕНИЕ МЕНЮ ПУСК
