...
def run_countdown1(sec, delay):
    ...
    # تطبيق التاخير فقط في اول ثانية
    wait = 1000 if delay <= 0 else int(delay * 1000)
...
def run_extra_countdown(sec, sequence_num):
    ...
    m, s = divmod(sec, 60)
    if sequence_num == 1:
        timer_extra_lbl.config(text=f"Extra 1: {m:02d}:{s:02d}", fg="darkgreen")
    else:
        timer_extra_lbl.config(text=f"Extra 2: {m:02d}:{s:02d}", fg="purple")
