...
Версия 6.2 (улучшенная совместимость с EXE)
"""
...
# ------------------------------------------------------------
# Функция для корректного поиска файлов в EXE (PyInstaller)
...
# ------------------------------------------------------------
# Настройка matplotlib (должна быть до импорта pyplot)
try:
    import matplotlib
    # Явно указываем бэкенд, совместимый с Tkinter
...
# ------------------------------------------------------------
# Заставка (с поддержкой resource_path)
class SplashScreen:
    def __init__(self, root, callback, duration=2500):
        ...
        # Загрузка splash.gif (работает и в EXE)
...
class UltimateApp:
    ...
    def _draw_graph(self):
        ...
        try:
            # Попытка отрисовать интерактивный график (работает в .py, в EXE может упасть)
            ...
            # Fallback: сохраняем график во временный файл PNG и показываем через PhotoImage
            try:
                fig, ax = plt.subplots(figsize=(8, 3.5))
                ax.plot(dates, vbrs, marker='o', linestyle='-', linewidth=2, color='#2c3e50')
                ax.set_xlabel("Дата/время", fontsize=9)
                ...
                # Масштабирование картинки под размер фрейма (упрощённо)
                img_label = tk.Label(self.graph_frame, image=img, bg="white")
                img_label.image = img
                img_label.pack(expand=True)
                # Удаляем временный файл после загрузки в Label
                os.remove(temp_file)
            except Exception as e2:
                lbl = tk.Label(self.graph_frame, text=f"Ошибка построения графика: {e2}", font=("Segoe UI", 10), fg="red")
                lbl.pack(expand=True)
    ...
    def _about(self):
        msg = ("КСА 45Л6-1С – Оценка технического состояния\n"
               "Версия 6.2 (исправленная для EXE)\n"
               ...
               "Для работы графиков требуется matplotlib.\n"
               "При сборке в EXE используйте PyInstaller с ключами:\n"
               "--hidden-import=matplotlib.backends.backend_tkagg --collect-data matplotlib")
