...
class SnowflakeCalculator:
    ...
    def create_angle_bpm_converter_tab(self):
        ...
        self.converter_save_btn = ttk.Button(converter_action_frame, text="保存为JSON文件", 
                                           style='Win11.TButton',
                                           command=self.save_converter_to_file, 
                                           state='disabled')
        self.converter_save_btn.pack(side=tk.LEFT, padx=(0, 10))
    ...
    def clear_converter(self):
        ...
        self.converter_save_btn.configure(state='disabled')
    ...
    def convert_to_bpm_events(self):
        ...
        self.converter_stats_label.config(text=f"转换完成！共生成 {len(events)} 个事件")
        
        self.converter_copy_btn.configure(state='normal')
        self.converter_save_btn.configure(state='normal')
        
        self.status_bar.config(text=f"角度-BPM转换完成！生成 {len(events)} 个事件")
    ...
    def save_converter_to_file(self):
        """保存角度-BPM转换器结果到文件"""
        if not hasattr(self, 'current_events_json') or not self.current_events_json:
            messagebox.showwarning("保存失败", "没有可保存的事件数据")
            return
        
        file_path = filedialog.asksaveasfilename(
            defaultextension=".json",
            filetypes=[("JSON文件", "*.json"), ("文本文件", "*.txt"), ("所有文件", "*.*")],
            initialfile="bpm_events.json"
        )
        ...
        if file_path:
            try:
                ...
                self.status_bar.config(text=f"事件数据已保存到: {file_path}")
                messagebox.showinfo("保存成功", f"事件数据已保存为JSON文件:\n{file_path}")
            except Exception as e:
                messagebox.showerror("保存失败", f"保存文件时出错: {str(e)}")
