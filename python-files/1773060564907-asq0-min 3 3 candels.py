...
# root.attributes("-alpha", 0.95)  # removed for PytoExe compatibility
try:
    main_font = tkfont.Font(family=current_font, size=BASE_FONT)
except:
    main_font = tkfont.Font(family="Arial", size=BASE_FONT)  # fallback if font missing
...
def toggle_ai():
    ...
    if ai_enabled:
        ...
        if ai_after_id:
            try: root.after_cancel(ai_after_id)
            except: pass
            ai_after_id = None
