...
def main_app():
    ...
    theme = THEMES.get(settings.get("theme", "Light"), THEMES["Light"])
    ...
    def scan(barcode):
        ...
        cur.execute(
            "SELECT id, item_name, scanned_qty, original_qty FROM items WHERE barcode=?",
            (barcode,)
        )
        item = cur.fetchone()
        ...
        if item:
            cur.execute(
                "UPDATE items SET scanned_qty=? WHERE id=?",
                (item[2] + 1, item[0])
            )
            result = "Matched"
    ...
    def refresh_table():
        ...
        cur.execute(
            "SELECT barcode, item_name, original_qty, scanned_qty, scanned_qty-original_qty FROM items"
        )
    ...
    def export_csv():
        ...
        if file:
            with open(file, "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(
                    ["Barcode", "Item Name", "Original Qty", "Scanned Qty", "Difference"]
                )
                cur.execute(
                    "SELECT barcode, item_name, original_qty, scanned_qty, scanned_qty-original_qty FROM items"
                )
