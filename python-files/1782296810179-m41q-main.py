...
class TypingTest:
    ...
    def check_text(self, event=None):
        ...
        # Обработка ошибок, сохраняем уже допущенные
        for i in range(len(original_text)):
            ...
            if i < len(typed_text):
                if typed_text[i] == original_text[i]:
                    ...
                    # фиксируем ошибку, если еще не фиксировали
                    if i not in self.errors_for_char:
                        self.errors_for_char[i] = original_text[i]
            else:
                # символ еще не введен
                self.text_display.tag_add("default", start_pos, end_pos)

        # Обработка сверх оригинала
        for i in range(len(original_text), len(typed_text)):
            ...
            # фиксируем ошибку, если еще не фиксировали
            if i not in self.errors_for_char:
                self.errors_for_char[i] = ''

        # Обновляем количество ошибок
        self.total_errors = len(self.errors_for_char)

        # Обновляем статистику
        self.correct_letters = sum(1 for i in range(len(original_text))
                                   if (i in self.errors_for_char and self.errors_for_char[i] == '') 
                                   or (i not in self.errors_for_char and i < len(typed_text) and typed_text[i] == original_text[i]))
