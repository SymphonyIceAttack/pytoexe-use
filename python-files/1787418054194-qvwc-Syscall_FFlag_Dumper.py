...
def main():
    ...
    for current_offset in range(scan_start, scan_end, chunk_size):
        ...
        for i in range(0, len(buffer) - 7, 8):
            maybe_map = struct.unpack('Q', buffer[i:i+8])[0]
            
            if not maybe_map or maybe_map < 0x100000:
                continue
            
            is_this_really_a_map_validation = Memory.read_uintptr(maybe_map)
            if not is_this_really_a_map_validation or is_this_really_a_map_validation < 0x10000:
                continue
            ...
            if is_this_really_a_map_validation == 0x3F800000:
                ...
                if map_start < 0x10000 or map_start > 0x7FFFFFFFFFFF:
                    continue
                
                if current < 0x10000 or current > 0x7FFFFFFFFFFF:
                    continue
                
                while current != 0 and current != map_end:
                    ...
                    if name == "BatchThumbnailMinWaitMs":
                        for value_get_set_offset in range(0x20, 0x50, 0x8):
                            ...
                            if test_value_get_set < 0x10000 or test_value_get_set > 0x7FFFFFFFFFFF:
                                continue
                            
                            for flag_to_value_offset in range(0x0, 0x100, 0x8):
                                test_pointer = Memory.read_uintptr(test_value_get_set + flag_to_value_offset)
                                if test_pointer < 0x10000 or test_pointer > 0x7FFFFFFFFFFF:
                                    continue
                    ...
                    if new_current < 0x10000 or new_current > 0x7FFFFFFFFFFF:
                        break
    ...
    while current != 0 and current != last:
        ...
        if mhm != "True" and mhm != "False":
            ...
            if offset < 0x10000 or offset > 0x7FFFFFFFFFFF:
                current = Memory.read_uintptr(current)
                continue
