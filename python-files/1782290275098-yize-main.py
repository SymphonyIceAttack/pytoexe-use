    ...
    def check_text(self, event=None):
        ...
        for i in range(min_length):
            ...
            if typed_text[i] == original_text[i]:
                ...
                correct_letters += 1
                # Если ранее была ошибка, убираем ее
                if i in self.errors_for_char:
                    # ошибка исправлена, убираем из ошибок
                    pass
                # Для учета ошибок по символам, если ошибка была, она исчезает
            else:
        ...
        # Обработка символов, введенных сверх оригинала — считаем ошибками
        for i in range(len(original_text), len(typed_text)):
            start_pos = f"1.0 + {i}c"
            end_pos = f"1.0 + {i+1}c"
            self.text_display.tag_add("error", start_pos, end_pos)
            self.text_display.tag_config("error", foreground="red")
            new_errors_for_char[i] = ''  # нет оригинального символа, считаем ошибкой

        # Обновляем общее число ошибок
        # Количество ошибок — это длина new_errors_for_char, так как каждое отличие — ошибка
        # В отличие от предыдущей логики, здесь мы считаем только текущие ошибки, без учета исправленных ошибок
        # Но если нужно, чтобы ошибки, исправленные ранее, оставались, то нужно их сохранять.

        # Для реализации, чтобы ошибки исчезали при исправлении, используем следующую логику:
        # - сохраняем только ошибки, которые есть сейчас
        # - ошибки, исправленные, исчезают из словаря
