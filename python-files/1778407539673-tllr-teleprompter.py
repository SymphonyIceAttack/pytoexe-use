...
class UrduTeleprompter:
    def __init__(self, root):
        ...
        # Initial display
        self.update_display()
        self.root.after(100, self.reset_scroll)
    ...
    def load_font_simple(self):
        ...
        # Try to use font by its filename (without extension) as family name
        font_name_guess = os.path.splitext(os.path.basename(self.font_path))[0]
        try:
            # Test if tkinter can use this font family
            test_font = tkfont.Font(family=font_name_guess, size=40)
            test_font.measure("ب")  # Test with Urdu character
    ...
    # ---------- GUI Controls (all in one bar, no menu) ----------
