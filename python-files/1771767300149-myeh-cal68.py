...
def start_timer(delay_sec):
    ...
    root.after(delay_sec*1000, lambda: _countdown(60))
...
def calculate(event=None):
    ...
    left_lbl.config(text=l, fg="black")
    right_lbl.config(text=r, fg="black")
    middle_lbl.config(font=(current_font,int(BASE_FONT*current_scale*1.05)), anchor="center", pady=1, bg="white")

    # AI behavior
    if ai_mode and v in AI_NEXT_VALUES:
        # العكس فقط داخل اللبني
...
def draw_ai_lamp():
    ...
    size=int(BASE_FONT*current_scale*0.45*1.3) # زيادة 30%
...
def edit_timer_popup():
    ...
    p.geometry("300x80")
    tk.Label(p,text="Delay (sec)").pack()
    scale = tk.Scale(p,from_=-5,to=5,resolution=0.25,orient="horizontal")
    scale.set(ai_delay_sec)
    scale.pack(fill="x")
    def save_delay():
        global ai_delay_sec
        ai_delay_sec = scale.get()
        p.destroy()
    tk.Button(p,text="X",command=p.destroy).pack()
    tk.Button(p,text="Save",command=save_delay).pack()
...
file_menu.add_command(label="Move Window") # Move logic موجود بالسحب
