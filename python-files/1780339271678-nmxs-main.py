...
class TypingTestApp:
    def __init__(self, root):
        ...
        self.btn_load_file = tk.Button(control_frame, text="Загрузить из .txt...", command=self.load_from_file, font=self.base_font)
        ...
        self.text_display.tag_configure('error', background='#FFB3BA')
        self.text_display.tag_configure('correct', foreground='#2E8B57', background='#E6FFF2')
        ...
        # Привязка горячих клавиш к основному полю ввода
        self.entry_input.bind("<Control-c>", lambda e: self.entry_input.event_generate("<<Copy>>"))
        self.entry_input.bind("<Control-v>", lambda e: self.entry_input.event_generate("<<Paste>>"))
    ...
    def load_from_file(self):
        ...
        if file_path:
            try:
                ...
                if text_from_file:
                    self.custom_phrase = text_from_file
                    messagebox.showinfo("Успех", f"Текст успешно загружен из файла!\nНажмите 'Старт'.")
                else:
    ...
    def set_custom_text(self):
        ...
        lbl_info = tk.Label(dialog_window, text="Скопируйте свой текст сюда:", font=("Arial", 11))
        ...
        custom_text_area.bind("<Control-a>", lambda e: custom_text_area.tag_add('sel', '1.0', 'end'))
        custom_text_area.bind("<Control-c>", lambda e: custom_text_area.event_generate("<<Copy>>"))
        custom_text_area.bind("<Control-v>", lambda e: custom_text_area.event_generate("<<Paste>>"))
        custom_text_area.bind("<Control-x>", lambda e: custom_text_area.event_generate("<<Cut>>"))
